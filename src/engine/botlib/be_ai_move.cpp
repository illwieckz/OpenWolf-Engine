...
typedef struct bot_movestate_s
{
    ...
    //state vars
    S32 areanum;								//area the bot is in
    S32 lastareanum;							//last area the bot was in
    S32 lastgoalareanum;						//last goal area number
    S32 lastreachnum;							//last reachability number
    vec3_t lastorigin;							//origin previous cycle
    S32 reachareanum;							//area number of the reachabilty
    S32 moveflags;								//movement flags
    S32 jumpreach;								//set when jumped
    F32 grapplevisible_time;					//last time the grapple was visible
    F32 lastgrappledist;						//last distance to the grapple end
    F32 reachability_time;					//time to use current reachability
    S32 avoidreach[MAX_AVOIDREACH];				//reachabilities to avoid
    F32 avoidreachtimes[MAX_AVOIDREACH];		//times to avoid the reachabilities
    S32 avoidreachtries[MAX_AVOIDREACH];		//number of tries before avoiding
    //
...
// Changes Globals:		-
//========================================================================
bot_movestate_t* BotMoveStateFromHandle( S32 handle )
{
    if( handle <= 0 || handle > MAX_CLIENTS )
    {
        botimport.Print( PRT_FATAL, "move state handle %d out of range\n", handle );
...
// Changes Globals:		-
//========================================================================
F32 AngleDiff( F32 ang1, F32 ang2 )
{
    F32 diff;
...
// Changes Globals:		-
//===========================================================================
S32 BotFuzzyPointReachabilityArea( vec3_t origin )
{
...
{
    S32 modelnum, modeltype, reachnum, areanum;
...
// Changes Globals:		-
//===========================================================================
S32 BotOnMover( vec3_t origin, S32 entnum, aas_reachability_t* reach )
{
    S32 i, modelnum;
...
// Changes Globals:		-
//===========================================================================
S32 MoverDown( aas_reachability_t* reach )
{
    S32 modelnum;
...
void BotSetBrushModelTypes( void )
{
    ...
    ::memset( modeltypes, 0, MAX_MODELS * sizeof( S32 ) );
    //
...
// Changes Globals:		-
//===========================================================================
S32 BotOnTopOfEntity( bot_movestate_t* ms )
{
...
// Changes Globals:		-
//===========================================================================
S32 BotValidTravel( vec3_t origin, aas_reachability_t* reach, S32 travelflags )
{
...
// Changes Globals:		-
//===========================================================================
void BotAddToAvoidReach( bot_movestate_t* ms, S32 number, F32 avoidtime )
{
    S32 i;
...
// Changes Globals:		-
//===========================================================================
S32 BotAvoidSpots( vec3_t origin, aas_reachability_t* reach, bot_avoidspot_t* avoidspots, S32 numavoidspots )
{
    S32 checkbetween, i, type;
    F32 squareddist, squaredradius;
...
// Changes Globals:		-
//===========================================================================
S32 BotGetReachabilityToGoal( vec3_t origin, S32 areanum,
                              S32 lastgoalareanum, S32 lastareanum,
                              S32* avoidreach, F32* avoidreachtimes, S32* avoidreachtries,
                              bot_goal_t* goal, S32 travelflags, S32 movetravelflags,
                              struct bot_avoidspot_s* avoidspots, S32 numavoidspots, S32* flags )
{
    S32 i, t, besttime, bestreachnum, reachnum;
    aas_reachability_t reach;
...
// Changes Globals:		-
//===========================================================================
S32 BotAddToTarget( vec3_t start, vec3_t end, F32 maxdist, F32* dist, vec3_t target )
{
    vec3_t dir;
    F32 curdist;
...
// Changes Globals:		-
//===========================================================================
S32 BotVisible( S32 ent, vec3_t eye, vec3_t target )
{
    bsp_trace_t trace;
...
{
    aas_reachability_t reach;
    S32 reachnum, lastgoalareanum, lastareanum, i;
    S32 avoidreach[MAX_AVOIDREACH];
    F32 avoidreachtimes[MAX_AVOIDREACH];
    S32 avoidreachtries[MAX_AVOIDREACH];
    vec3_t end;
    ...
    ::memset( avoidreach, 0, MAX_AVOIDREACH * sizeof( S32 ) );
...
// Changes Globals:		-
//===========================================================================
F32 BotGapDistance( vec3_t origin, vec3_t hordir, S32 entnum )
{
    F32 dist, startz;
    vec3_t start, end;
    aas_trace_t trace;
...
// Changes Globals:		-
//===========================================================================
S32 BotCheckBarrierJump( bot_movestate_t* ms, vec3_t dir, F32 speed )
{
...
// Changes Globals:		-
//===========================================================================
S32 BotSwimInDirection( bot_movestate_t* ms, vec3_t dir, F32 speed, S32 type )
{
    vec3_t normdir;
...
// Changes Globals:		-
//===========================================================================
S32 BotWalkInDirection( bot_movestate_t* ms, vec3_t dir, F32 speed, S32 type )
{
    vec3_t hordir, cmdmove, velocity, tmpdir, origin;
    S32 presencetype, maxframes, cmdframes, stopevent;
...
// Changes Globals:		-
//===========================================================================
S32 Intersection( vec2_t p1, vec2_t p2, vec2_t p3, vec2_t p4, vec2_t out )
{
    F32 x1, dx1, dy1, x2, dx2, dy2, d;
    ...
    if( d != 0 )
    {
        ...
        out[0] = ( S32 )( ( dx1 * x2 - dx2 * x1 ) / d );
        out[1] = ( S32 )( ( dy1 * x2 - dy2 * x1 ) / d );
        return true;
    } //end if
    else
    {
...
// Changes Globals:		-
//===========================================================================
void BotCheckBlocked( bot_movestate_t* ms, vec3_t dir, S32 checkbottom, bot_moveresult_t* result )
{
    vec3_t mins, maxs, end, up = {0, 0, 1};
    bsp_trace_t trace;
...
// Changes Globals:		-
//===========================================================================
S32 BotAirControl( vec3_t origin, vec3_t velocity, vec3_t goal, vec3_t dir, F32* speed )
{
    vec3_t org, vel;
    F32 dist;
    S32 i;
...
{
	vec3_t hordir;
	F32 dist, gapdist, speed, horspeed, sv_jumpvel;
	bot_moveresult_t_cleared( result );
...
// Changes Globals:		-
//===========================================================================
S32 GrappleState( bot_movestate_t* ms, aas_reachability_t* reach )
{
    S32 i;
    ...
    {
        if( AAS_EntityType( i ) == ( S32 ) entitytypemissile->value )
        {
            AAS_EntityInfo( i, &entinfo );
            if( entinfo.weapon == ( S32 ) weapindex_grapple->value )
            {
                return 1;
            } //end if
        } //end if
...
bot_moveresult_t BotTravel_RocketJump( bot_movestate_t* ms, aas_reachability_t* reach )
{
    ...
    EA_SelectWeapon( ms->client, ( S32 ) weapindex_rocketlauncher->value );
    //weapon is used for movement
    result.weapon = ( S32 ) weapindex_rocketlauncher->value;
    result.flags |= MOVERESULT_MOVEMENTWEAPON;
    //
    VectorCopy( hordir, result.movedir );
    //
...
bot_moveresult_t BotTravel_BFGJump( bot_movestate_t* ms, aas_reachability_t* reach )
{
    ...
    result.flags |= MOVERESULT_MOVEMENTVIEWSET;
    //select the rocket launcher
    EA_SelectWeapon( ms->client, ( S32 ) weapindex_bfg10k->value );
    //weapon is used for movement
    result.weapon = ( S32 ) weapindex_bfg10k->value;
    result.flags |= MOVERESULT_MOVEMENTWEAPON;
    //
    VectorCopy( hordir, result.movedir );
    //
...
// Changes Globals:		-
//===========================================================================
S32 BotReachabilityTime( aas_reachability_t* reach )
{
    switch( reach->traveltype & TRAVELTYPE_MASK )
    {
        case TRAVEL_WALK:
...
{
    S32 reachnum, lastreachnum, foundjumppad, ent, resultflags;
...
void BotResetAvoidReach( S32 movestate )
{
    ...
    ms = BotMoveStateFromHandle( movestate );
    if( !ms ) return;
    ::memset( ms->avoidreach, 0, MAX_AVOIDREACH * sizeof( S32 ) );
    ::memset( ms->avoidreachtimes, 0, MAX_AVOIDREACH * sizeof( F32 ) );
    ::memset( ms->avoidreachtries, 0, MAX_AVOIDREACH * sizeof( S32 ) );
