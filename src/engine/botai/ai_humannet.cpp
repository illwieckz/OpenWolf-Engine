...
// for the voice chats
#include "../gui/menudef.h" // sos001205 - for q3_ui also
...
//  if (bs->inventory[BI_ROCKETLAUNCHER] <= 0) return false;
//  //if low on rockets
//  if (bs->inventory[BI_ROCKETS] < 3) return false;
//  //never rocket jump with the Quad
//  if (bs->inventory[BI_QUAD]) return false;
    //if low on health
    if( bs->inventory[BI_HEALTH] < 60 ) return false;
    //if not full health
//  if (bs->inventory[BI_HEALTH] < 90) {
//    //if the bot has insufficient armor
//    if (bs->inventory[BI_ARMOR] < 40) return false;
//  }
...
//  //if the bot can use the bfg
//  if (bs->inventory[BI_BFG10K] > 0 &&
//      bs->inventory[BI_BFGAMMO] > 7) return true;
//  //if the bot can use the railgun
//  if (bs->inventory[BI_RAILGUN] > 0 &&
//      bs->inventory[BI_SLUGS] > 5) return true;
//  //if the bot can use the lightning gun
//  if (bs->inventory[BI_LIGHTNING] > 0 &&
//      bs->inventory[BI_LIGHTNINGAMMO] > 50) return true;
//  //if the bot can use the rocketlauncher
//  if (bs->inventory[BI_ROCKETLAUNCHER] > 0 &&
//      bs->inventory[BI_ROCKETS] > 5) return true;
//  //
//  if (bs->inventory[BI_NAILGUN] > 0 &&
//      bs->inventory[BI_NAILS] > 5) return true;
//  //
//  if (bs->inventory[BI_PROXLAUNCHER] > 0 &&
//      bs->inventory[BI_MINES] > 5) return true;
//  //
//  if (bs->inventory[BI_CHAINGUN] > 0 &&
//      bs->inventory[BI_BELT] > 40) return true;
//  //if the bot can use the plasmagun
//  if (bs->inventory[BI_PLASMAGUN] > 0 &&
//      bs->inventory[BI_CELLS] > 20) return true;
    return false;
}
...
void BotDumpNodeSwitches( bot_state_t* bs )
{
    ...
    ClientName( bs->client, model, sizeof( model ) );
    BotAI_Print( PRT_MESSAGE, "%s at %1.1f switched more than %d AI nodes\n", model, FloatTime(), MAX_NODESWITCHES );
