...
S32 bg_numBuildables = sizeof( bg_buildableList ) / sizeof( bg_buildableList[ 0 ] );
...
==============
*/
const buildableAttributes_t* idBothGamesLocal::BuildableByName( StringEntry name )
{
    S32 i;
...
==============
*/
const buildableAttributes_t* idBothGamesLocal::BuildableByEntityName( StringEntry name )
{
    S32 i;
...
{
    S32 stages = Buildable( buildable )->stages;
...
======================
*/
bool idBothGamesLocal::ParseBuildableFile( StringEntry filename, buildableConfig_t* bc )
{
    UTF8* text_p;
    S32 i, len;
    UTF8* token;
...
S32 bg_numClasses = sizeof( bg_classList ) / sizeof( bg_classList[ 0 ] );
...
==============
*/
const classAttributes_t* idBothGamesLocal::ClassByName( StringEntry name )
{
    S32 i;
...
{
    S32 stages = Class( _class )->stages;
...
==============
*/
bool idBothGamesLocal::ClassHasAbility( class_t _class, S32 ability )
{
    S32 abilities = Class( _class )->abilities;
...
==============
*/
S32 idBothGamesLocal::ClassCanEvolveFromTo( class_t fclass, class_t tclass, S32 credits, S32 stage, S32 cost )
{
    S32 i, j, best, value;
...
==============
*/
bool idBothGamesLocal::AlienCanEvolve( class_t pclass, S32 credits, S32 stage )
{
    S32 i, j, tclass;
...
======================
*/
bool idBothGamesLocal::ParseClassFile( StringEntry filename, classConfig_t* cc )
{
    UTF8* text_p;
    S32 i, len;
    UTF8* token;
...
S32 bg_numWeapons = sizeof( bg_weapons ) / sizeof( bg_weapons[ 0 ] );
...
==============
*/
const weaponAttributes_t* idBothGamesLocal::WeaponByName( StringEntry name )
{
    S32 i;
...
{
    S32 stages = Weapon( weapon )->stages;
...
S32 bg_numUpgrades = sizeof( bg_upgrades ) / sizeof( bg_upgrades[ 0 ] );
...
==============
*/
const upgradeAttributes_t* idBothGamesLocal::UpgradeByName( StringEntry name )
{
    S32 i;
...
{
    S32 stages = Upgrade( upgrade )->stages;
...
================
*/
void idBothGamesLocal::EvaluateTrajectory( const trajectory_t* tr, S32 atTime, vec3_t result )
{
    F32 deltaTime, phase;
    ...
    switch( tr->trType )
    {
        ...
        case TR_SINE:
            deltaTime = ( atTime - tr->trTime ) / ( F32 )tr->trDuration;
...
================
*/
void idBothGamesLocal::EvaluateTrajectoryDelta( const trajectory_t* tr, S32 atTime, vec3_t result )
{
    F32 deltaTime, phase;
    ...
    switch( tr->trType )
    {
        ...
        default:
            Com_Error( ERR_DROP, "idBothGamesLocal::EvaluateTrajectoryDelta: unknown trType: %i", tr->trTime );
            break;
    }
}
...
===============
*/
StringEntry idBothGamesLocal::EventName( S32 num )
{
    if( num < 0 || num >= sizeof( eventnames ) / sizeof( UTF8* ) )
    {
        return "UNKNOWN";
    }
...
void idBothGamesLocal::AddPredictableEventToPlayerstate( S32 newEvent, S32 eventParm, playerState_t* ps )
{
#ifdef _DEBUG
    {
        UTF8 buf[ 256 ];
...
========================
*/
void idBothGamesLocal::PlayerStateToEntityStateExtraPolate( playerState_t* ps, entityState_t* s, S32 time, bool snap )
{
    S32 i;
...
========================
*/
bool idBothGamesLocal::WeaponIsFull( weapon_t weapon, S32 stats[ ], S32 ammo, S32 clips )
{
    ...
    {
        maxAmmo = ( S32 )( ( F32 )maxAmmo * BATTPACK_MODIFIER );
    }
...
========================
*/
bool idBothGamesLocal::InventoryContainsWeapon( S32 weapon, S32 stats[ ] )
{
...
========================
*/
S32 idBothGamesLocal::CalculateSlotsForInventory( S32 stats[ ] )
{
    S32 i, slot, slots;
...
========================
*/
void idBothGamesLocal::AddUpgradeToInventory( S32 item, S32 stats[ ] )
{
...
========================
*/
void idBothGamesLocal::RemoveUpgradeFromInventory( S32 item, S32 stats[ ] )
{
    stats[ STAT_ITEMS ] &= ~( 1 << item );
}
...
========================
*/
bool idBothGamesLocal::InventoryContainsUpgrade( S32 item, S32 stats[ ] )
{
...
========================
*/
void idBothGamesLocal::ActivateUpgrade( S32 item, S32 stats[ ] )
{
...
========================
*/
void idBothGamesLocal::DeactivateUpgrade( S32 item, S32 stats[ ] )
{
    stats[ STAT_ACTIVEITEMS ] &= ~( 1 << item );
}
...
========================
*/
bool idBothGamesLocal::UpgradeIsActive( S32 item, S32 stats[ ] )
{
...
===============
*/
void idBothGamesLocal::PositionBuildableRelativeToPlayer( const playerState_t* ps, const vec3_t mins, const vec3_t maxs, void ( *trace )( trace_t*, const vec3_t, const vec3_t, const vec3_t, const vec3_t, S32, S32 ), vec3_t outOrigin, vec3_t outAngles, trace_t* tr )
{
...
===============
*/
S32 idBothGamesLocal::GetValueOfPlayer( playerState_t* ps )
{
    S32 i, worth = Class( ( class_t )ps->stats[ STAT_CLASS ] )->value;
...
=================
*/
S32 idBothGamesLocal::PlayerPoisonCloudTime( playerState_t* ps )
{
    S32 time = LEVEL1_PCLOUD_TIME;
...
===============
*/
F32 idBothGamesLocal::atof_neg( UTF8* token, bool allowNegative )
{
    F32 value;
...
===============
*/
S32 idBothGamesLocal::atoi_neg( UTF8* token, bool allowNegative )
{
    S32 value;
...
===============
*/
void idBothGamesLocal::ParseCSVEquipmentList( StringEntry string, weapon_t* weapons, S32 weaponsSize, upgrade_t* upgrades, S32 upgradesSize )
{
...
===============
*/
void idBothGamesLocal::ParseCSVClassList( StringEntry string, class_t* classes, S32 classesSize )
{
    UTF8 buffer[ MAX_STRING_CHARS ];
    S32 i = 0;
    UTF8* p, *q;
...
===============
*/
void idBothGamesLocal::ParseCSVBuildableList( StringEntry string, buildable_t* buildables, S32 buildablesSize )
{
    UTF8 buffer[ MAX_STRING_CHARS ];
    S32 i = 0;
    UTF8* p, *q;
...
{
    UTF8 cvar[ MAX_CVAR_VALUE_STRING ];
...
============
*/
bool idBothGamesLocal::ClientListTest( clientList_t* list, S32 clientNum )
{
...
============
*/
void idBothGamesLocal::ClientListAdd( clientList_t* list, S32 clientNum )
{
    if( clientNum < 0 || clientNum >= MAX_CLIENTS || !list )
    {
        return;
...
============
*/
void idBothGamesLocal::ClientListRemove( clientList_t* list, S32 clientNum )
{
    if( clientNum < 0 || clientNum >= MAX_CLIENTS || !list )
    {
        return;
...
============
*/
UTF8* idBothGamesLocal::ClientListString( clientList_t* list )
{
    static UTF8 s[ 17 ];
...
============
*/
void idBothGamesLocal::ClientListParse( clientList_t* list, StringEntry s )
{
    if( !list )
    {
        return;
...
============
*/
weapon_t idBothGamesLocal::PrimaryWeapon( S32 stats[ ] )
{
    S32 i;
...
============
*/
S32 idBothGamesLocal::LoadEmoticons( UTF8 names[ ][ MAX_EMOTICON_NAME_LEN ], S32 widths[ ] )
{
    S32 numFiles;
    UTF8 fileList[ MAX_EMOTICONS * ( MAX_EMOTICON_NAME_LEN + 9 ) ] = {""};
    S32 i;
    UTF8* filePtr;
    S32 fileLen;
    UTF8 emoticon[ MAX_EMOTICON_NAME_LEN + 9 ] = {""};
    S32 loaded = 0;
    S32 count;
    S32 width = 0;
...
============
*/
UTF8* idBothGamesLocal::TeamName( team_t team )
{
    if( team == TEAM_NONE )
    {
        return "spectator";
