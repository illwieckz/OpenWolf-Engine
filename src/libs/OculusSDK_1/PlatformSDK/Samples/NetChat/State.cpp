...
void machineState::requestCreateRoom()
{
	...
	case IDLE:
		...
		ovrRequest req;
		req = ovr_Matchmaking_CreateAndEnqueueRoom(POOL_NAME, 8, true, nullptr);
...
void machineState::requestFindFilteredMatch()
{
	switch (currentState)
	{
	case NOT_INIT:
	...
	case IDLE:
		...
		//
		// We're filtering using two different keys setup on the developer portal
		//
		// game_type - A string key that has the options deathmatch,CTF
		// map_index - An integer key that indicates a map ID
		//
		// In the example below we are filtering for matches that are of type deathmatch and on map #1
		//
		// The importance of game_type is set to ovrMatchmaking_ImportanceRequired which means we will only return results that match that value exactly
		// The importance of map_index is set to ovrMatchmaking_ImportanceLow which means we may return results that don't match this value depending on the length of time the query is taking
		//

		ovrKeyValuePair filters[2];
		filters[0].stringValue = "deathmatch";
		filters[0].valueType = ovrKeyValuePairType_String;
		filters[0].key = "game_type";

		filters[1].intValue = 1;
		filters[1].valueType = ovrKeyValuePairType_Int;
		filters[1].key = "map_index";

		ovrMatchmakingCriterion criteria[2];

		criteria[0].key = "game_type";
		criteria[0].importance = ovrMatchmaking_ImportanceRequired;

		criteria[1].key = "map_index";
		criteria[1].importance = ovrMatchmaking_ImportanceLow;

		ovrMatchmakingCustomQueryData customQueryData;
		customQueryData.customQueryDataArray = filters;
		customQueryData.customQueryDataArrayCount = 2;
		customQueryData.customQueryCriterionArray = criteria;
		customQueryData.customQueryCriterionArrayCount = 2;

		ovrRequest req;
		req = ovr_Matchmaking_Enqueue(POOL_NAME, &customQueryData);
		currentState = REQUEST_FIND;
		break;
