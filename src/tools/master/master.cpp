...
static qboolean parseIgnoreAddress( void )
{
    ...
    ignoreAddresses = ( ignoreAddress_t* )malloc( sizeof( ignoreAddress_t ) * numAllocIgnoreAddresses );
    ...
    while( !feof( f ) )
    {
        ...
        if( c != EOF )
        {
            ...
            {
                ignoreAddress_t* _new;
                
                numAllocIgnoreAddresses *= 2;
                _new = ( ignoreAddress_t* )realloc( ignoreAddresses, sizeof( ignoreAddress_t ) * numAllocIgnoreAddresses );
...
#ifdef __LINUX__
        if( select( std::max( inSock, outSock ) + 1, &rfds, NULL, NULL, &tv ) <= 0 )
#else
