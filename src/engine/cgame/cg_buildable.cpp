...
/*
===================
AlienBuildableExplosion

Generated a bunch of gibs launching out from a location
===================
*/
void idCGameLocal::AlienBuildableExplosion( vec3_t origin, vec3_t dir )
{
    particleSystem_t*  ps;
    ...
    //particle system
    ps = SpawnNewParticleSystem( cgs.media.alienBuildableDestroyedPS );
    
    if( IsParticleSystemValid( &ps ) )
    {
        SetAttachmentPoint( &ps->attachment, origin );
        SetParticleSystemNormal( ps, dir );
...
/*
=================
HumanBuildableExplosion

Called for human buildables as they are destroyed
=================
*/
void idCGameLocal::HumanBuildableExplosion( vec3_t origin, vec3_t dir )
{
    particleSystem_t*  ps;
    ...
    //particle system
    ps = SpawnNewParticleSystem( cgs.media.humanBuildableDestroyedPS );
...
==================
*/
void idCGameLocal::Creep( centity_t* cent )
{
    S32           msec;
    F32         size, frac;
    trace_t       tr;
    vec3_t        temp, origin;
    S32           scaleUpTime = bggame->Buildable( ( buildable_t )cent->currentState.modelindex )->buildTime;
    S32           time;
    ...
    else if( time < 0 )
    {
        ...
        if( msec >= 0 && msec < CREEP_SCALEDOWN_TIME )
            frac = 1.0f - ( ( F32 )msec / CREEP_SCALEDOWN_TIME );
        else
            frac = 0.0f;
    }
    ...
    Trace( &tr, cent->lerpOrigin, NULL, NULL, temp, cent->currentState.number, MASK_PLAYERSOLID );
    ...
    if( size > 0.0f && tr.fraction < 1.0f )
        ImpactMark( cgs.media.creepShader, origin, cent->currentState.origin2, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f, false, size, true );
}

/*
======================
ParseBuildableAnimationFile

Read a configuration file containing animation counts and rates
models/buildables/hivemind/animation.cfg, etc
======================
*/
bool idCGameLocal::ParseBuildableAnimationFile( StringEntry filename, buildable_t buildable )
{
    UTF8*          text_p;
    S32           len;
    S32           i;
...
/*
======================
ParseBuildableSoundFile

Read a configuration file containing sound properties
sound/buildables/hivemind/sound.cfg, etc
======================
*/
bool idCGameLocal::ParseBuildableSoundFile( StringEntry filename, buildable_t buildable )
{
    UTF8*          text_p;
    S32           len;
    S32           i;
...
===============
*/
void idCGameLocal::InitBuildables( void )
{
    UTF8          filename[ MAX_QPATH ];
    UTF8          soundfile[ MAX_QPATH ];
    UTF8*          buildableName;
    UTF8*          modelFile;
    S32           i;
    S32           j;
    ...
    for( i = BA_NONE + 1; i < BA_NUM_BUILDABLES; i++ )
    {
        ...
        cg.buildablesFraction = ( F32 )i / ( F32 )( BA_NUM_BUILDABLES - 1 );
        trap_UpdateScreen( );
    }
    
    cgs.media.teslaZapTS = RegisterTrailSystem( "models/buildables/tesla/zap" );
}

/*
===============
SetBuildableLerpFrameAnimation

may include ANIM_TOGGLEBIT
===============
*/
void idCGameLocal::SetBuildableLerpFrameAnimation( buildable_t buildable, lerpFrame_t* lf, S32 newAnimation )
{
    animation_t* anim;
...
===============
*/
void idCGameLocal::RunBuildableLerpFrame( centity_t* cent )
{
...
===============
*/
void idCGameLocal::BuildableAnimation( centity_t* cent, S32* old, S32* now, F32* backLerp )
{
...
/*
===============
PositionAndOrientateBuildable
===============
*/
void idCGameLocal::PositionAndOrientateBuildable( const vec3_t angles, const vec3_t inOrigin, const vec3_t normal, const S32 skipNumber, const vec3_t mins, const vec3_t maxs, vec3_t outAxis[ 3 ], vec3_t outOrigin )
{
    ...
    CapTrace( &tr, start, mins, maxs, end, skipNumber, CONTENTS_SOLID | CONTENTS_PLAYERCLIP );
    
    if( tr.fraction == 1.0f )
    {
        //erm we missed completely - try again with a box trace
        Trace( &tr, start, mins, maxs, end, skipNumber, CONTENTS_SOLID | CONTENTS_PLAYERCLIP );
    }
...
==================
*/
void idCGameLocal::GhostBuildable( buildable_t buildable )
{
    refEntity_t     ent;
    playerState_t*   ps;
    ...
    bggame->PositionBuildableRelativeToPlayer( ps, mins, maxs, Trace, entity_origin, angles, &tr );
...
==================
*/
void idCGameLocal::BuildableParticleEffects( centity_t* cent )
{
    entityState_t*   es = &cent->currentState;
    team_t          team = bggame->Buildable( ( buildable_t )es->modelindex )->team;
    S32             health = es->generic1 & B_HEALTH_MASK;
    F32           healthFrac = ( F32 )health / B_HEALTH_MASK;
    ...
    if( team == TEAM_HUMANS )
    {
        if( healthFrac < 0.33f && !IsParticleSystemValid( &cent->buildablePS ) )
        {
            cent->buildablePS = SpawnNewParticleSystem( cgs.media.humanBuildableDamagedPS );
            
            if( IsParticleSystemValid( &cent->buildablePS ) )
            {
                SetAttachmentCent( &cent->buildablePS->attachment, cent );
                AttachToCent( &cent->buildablePS->attachment );
            }
        }
        else if( healthFrac >= 0.33f && IsParticleSystemValid( &cent->buildablePS ) )
            DestroyParticleSystem( &cent->buildablePS );
    }
    else if( team == TEAM_ALIENS )
    {
        if( healthFrac < 0.33f && !IsParticleSystemValid( &cent->buildablePS ) )
        {
            cent->buildablePS = SpawnNewParticleSystem( cgs.media.alienBuildableDamagedPS );
            
            if( IsParticleSystemValid( &cent->buildablePS ) )
            {
                SetAttachmentCent( &cent->buildablePS->attachment, cent );
                SetParticleSystemNormal( cent->buildablePS, es->origin2 );
                AttachToCent( &cent->buildablePS->attachment );
            }
        }
        else if( healthFrac >= 0.33f && IsParticleSystemValid( &cent->buildablePS ) )
...
/*
==================
BuildableStatusParse
==================
*/
void idCGameLocal::BuildableStatusParse( StringEntry filename, buildStat_t* bs )
{
    pc_token_t token;
    S32        handle;
    StringEntry s;
...
/*
==================
BuildableStatusDisplay
==================
*/
void idCGameLocal::BuildableStatusDisplay( centity_t* cent )
{
    entityState_t*   es = &cent->currentState;
    vec3_t          origin;
    F32           healthScale;
    ...
    if( es->modelindex == BA_A_BARRICADE &&
        ...
        maxs[ 2 ] = ( S32 )( maxs[ 2 ] * BARRICADE_SHRINKPROP );
    ...
    healthScale = ( F32 )health / B_HEALTH_MASK;
    ...
    {
        F32  picH = bs->frameHeight;
        F32  picW = bs->frameWidth;
        F32  picX = x;
        F32  picY = y;
        F32  scale;
        F32  subH, subY;
        ...
        if( bs->overlayShader )
        {
            F32 oW = bs->overlayWidth;
            F32 oH = bs->overlayHeight;
            F32 oX = x;
            F32 oY = y;
        ...
        if( !powered )
        {
            ...
            DrawPic( pX, subY, subH, subH, bs->noPowerShader );
        }
        ...
        if( marked )
        {
            ...
            DrawPic( mX, subY, subH, subH, bs->markedShader );
        }
...
==================
*/
S32 idCGameLocal::SortDistance( const void* a, const void* b )
{
    centity_t*    aent, *bent;
    F32        adist, bdist;
...
==================
*/
bool idCGameLocal::PlayerIsBuilder( buildable_t buildable )
{
    switch( cg.predictedPlayerState.weapon )
    {
        ...
        case WP_ABUILD2:
        case WP_HBUILD:
            return bggame->Buildable( buildable )->team ==
...
/*
==================
BuildableRemovalPending
==================
*/
static bool BuildableRemovalPending( S32 entityNum )
{
    S32           i;
...
==================
*/
void idCGameLocal::DrawBuildableStatus( void )
{
    S32             i;
    centity_t*       cent;
    entityState_t*   es;
    ...
    qsort( buildableList, buildables, sizeof( S32 ), SortDistance );
...
==================
*/
void idCGameLocal::Buildable( centity_t* cent )
{
    refEntity_t     ent;
    entityState_t*   es = &cent->currentState;
    vec3_t          angles;
    ...
    if( es->pos.trType == TR_STATIONARY )
    {
        ...
        {
            PositionAndOrientateBuildable( angles, ent.origin, surfNormal,
    ...
    if( cg_buildables[ es->modelindex ].models[ 1 ] )
    {
        ...
        PositionEntityOnTag( &turretBarrel, &ent, ent.hModel, "tag_turret" );
    ...
    if( cg_buildables[ es->modelindex ].models[ 2 ] )
    {
        ...
        PositionRotatedEntityOnTag( &turretTop, &ent, ent.hModel, "tag_turret" );
    ...
    if( es->eFlags & EF_FIRING )
    {
        ...
        if( cg.time - cent->muzzleFlashTime > MUZZLE_FLASH_TIME ||
                bggame->Buildable( ( buildable_t )es->modelindex )->turretProjType == WP_TESLAGEN )
        {
            ...
            ( es->eFlags & EF_B_SPAWNED ) )
    {
        if( cent->lastBuildableDamageSoundTime + BUILDABLE_SOUND_PERIOD < cg.time )
        {
            if( team == TEAM_HUMANS )
            {
                ...
                {
                    cent->buildableHitPS = SpawnNewParticleSystem( cgs.media.humanBuildableHitLargePS );
                }
                else
                {
                    cent->buildableHitPS = SpawnNewParticleSystem( cgs.media.humanBuildableHitSmallPS );
                }
            }
            else if( team == TEAM_ALIENS )
            {
                ...
                {
                    cent->buildableHitPS = SpawnNewParticleSystem( cgs.media.alienBuildableHitLargePS );
                }
                else
                {
                    cent->buildableHitPS = SpawnNewParticleSystem( cgs.media.alienBuildableHitSmallPS );
                }
            }
            if( IsParticleSystemValid( &cent->buildableHitPS ) )
            {
                SetAttachmentCent( &cent->buildableHitPS->attachment, cent );
                AttachToCent( &cent->buildableHitPS->attachment );
            }
