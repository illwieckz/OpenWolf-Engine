...
/*
================
idGameLocal::SetBuildableAnim

Triggers an animation client side
================
*/
void idGameLocal::SetBuildableAnim( gentity_t* ent, buildableAnimNumber_t anim, bool force )
{
    S32 localAnim = anim | ( ent->s.legsAnim & ANIM_TOGGLEBIT );
...
/*
================
idGameLocal::SetIdleBuildableAnim

Set the animation to use whilst no other animations are running
================
*/
void idGameLocal::SetIdleBuildableAnim( gentity_t* ent, buildableAnimNumber_t anim )
{
...
/*
===============
idGameLocal::CheckSpawnPoint

Check if a spawn at a specified point is valid
===============
*/
gentity_t* idGameLocal::CheckSpawnPoint( S32 spawnNum, vec3_t origin, vec3_t normal, buildable_t spawn, vec3_t spawnOrigin )
{
    F32   displacement;
    vec3_t  mins, maxs;
    vec3_t  cmins, cmaxs;
    ...
    {
        bggame->ClassBoundingBox( PCL_HUMAN, cmins, cmaxs, NULL, NULL, NULL );
...
================
*/
S32 idGameLocal::NumberOfDependants( gentity_t* self )
{
    S32       i, n = 0;
    gentity_t* ent;
...
================
*/
bool idGameLocal::FindPower( gentity_t* self )
{
    S32       i;
    gentity_t* ent;
    gentity_t* closestPower = NULL;
    S32       distance = 0;
    S32       minDistance = REPEATER_BASESIZE + 1;
    vec3_t    temp_v;
...
================
*/
gentity_t* idGameLocal::PowerEntityForPoint( vec3_t origin )
{
    gentity_t dummy;
...
================
*/
bool idGameLocal::FindRepeater( gentity_t* self )
{
    S32       i;
    gentity_t* ent;
    gentity_t* closestPower = NULL;
...
================
*/
gentity_t* idGameLocal::RepeaterEntityForPoint( vec3_t origin )
{
    gentity_t dummy;
...
================
*/
S32 idGameLocal::FindDCC( gentity_t* self )
{
    S32       i;
    gentity_t* ent;
    S32       distance = 0;
...
/*
================
idGameLocal::IsDCCBuilt

simple wrapper to idGameLocal::FindDCC to check for a dcc
================
*/
bool idGameLocal::IsDCCBuilt( void )
{
    gentity_t dummy;
...
/*
================
idGameLocal::FindOvermind

Attempt to find an overmind for self
================
*/
bool idGameLocal::FindOvermind( gentity_t* self )
{
    S32       i;
    gentity_t* ent;
...
/*
================
idGameLocal::FindCreep

attempt to find creep for self, return true if successful
================
*/
bool idGameLocal::FindCreep( gentity_t* self )
{
    S32       i;
    gentity_t* ent;
...
simple wrapper to idGameLocal::FindCreep to check if a location has creep
================
*/
bool idGameLocal::IsCreepHere( vec3_t origin )
{
    gentity_t dummy;
...
================
*/
void idGameLocal::CreepSlow( gentity_t* self )
{
    ...
    F32       creepSize = ( F32 )bggame->Buildable( buildable )->creepSize;
...
================
*/
void idGameLocal::nullDieFunction( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, S32 damage, S32 mod )
{
}
...
================
*/
void idGameLocal::AGeneric_CreepRecede( gentity_t* self )
{
    ...
    if( !( self->s.eFlags & EF_DEAD ) )
    {
        ...
        AddEvent( self, EV_BUILD_DESTROY, 0 );
        
        if( self->spawned )
            self->s.time = -level.time;
        else
            self->s.time = -( level.time -
                              ( S32 )( ( F32 )CREEP_SCALEDOWN_TIME *
                                       ( 1.0f - ( ( F32 )( level.time - self->buildTime ) /
                                                  ( F32 )bggame->Buildable( ( buildable_t )self->s.modelindex )->buildTime ) ) ) );
    }
...
================
*/
void idGameLocal::AGeneric_Blast( gentity_t* self )
{
    vec3_t  dir;
    ...
    SelectiveRadiusDamage( self->s.pos.trBase, self, self->splashDamage, self->splashRadius, self, self->splashMethodOfDeath, TEAM_ALIENS );
    
    //pretty events and item cleanup
    self->s.eFlags |= EF_NODRAW; //don't draw the model once it's destroyed
    AddEvent( self, EV_ALIEN_BUILDABLE_EXPLOSION, DirToByte( dir ) );
...
================
*/
void idGameLocal::AGeneric_Die( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, S32 damage, S32 mod )
{
    RewardAttackers( self );
    ...
    if( attacker && attacker->client )
        LogDestruction( self, attacker, mod );
}
...
================
*/
void idGameLocal::AGeneric_CreepCheck( gentity_t* self )
{
    gentity_t* spawn;
    ...
    if( !FindCreep( self ) )
    {
        if( spawn && self->killedBy != ENTITYNUM_NONE )
            Damage( self, NULL, g_entities + self->killedBy, NULL, NULL, self->health, 0, MOD_NOCREEP );
        else
            Damage( self, NULL, NULL, NULL, NULL, self->health, 0, MOD_NOCREEP );
        return;
    }
    CreepSlow( self );
}
...
================
*/
void idGameLocal::AGeneric_Think( gentity_t* self )
{
...
================
*/
void idGameLocal::AGeneric_Pain( gentity_t* self, gentity_t* attacker, S32 damage )
{
    if( self->health <= 0 )
        return;
...
================
*/
void idGameLocal::ASpawn_Die( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, S32 damage, S32 mod )
{
    S32 i;
...
================
*/
void idGameLocal::ASpawn_Think( gentity_t* self )
{
    gentity_t* ent;
    ...
    if( self->spawned )
    {
        ...
        {
            if( ( ent = CheckSpawnPoint( self->s.number, self->s.origin, self->s.origin2, BA_A_SPAWN, NULL ) ) != NULL )
            {
                // If the thing blocking the spawn is a buildable, kill it.
                // If it's part of the map, kill self.
                if( ent->s.eType == ET_BUILDABLE )
                {
                    Damage( ent, NULL, NULL, NULL, NULL, 10000, 0, MOD_SUICIDE );
                    SetBuildableAnim( self, BANIM_SPAWN1, true );
                }
                else if( ent->s.number == ENTITYNUM_WORLD || ent->s.eType == ET_MOVER )
                {
                    Damage( self, NULL, NULL, NULL, NULL, 10000, 0, MOD_SUICIDE );
                    return;
                }
                
                if( ent->s.eType == ET_CORPSE )
                    FreeEntity( ent ); //quietly remove
            }
        }
    }
...
================
*/
void idGameLocal::AOvermind_Think( gentity_t* self )
{
    ...
    {
        //do some damage
        if( SelectiveRadiusDamage( self->s.pos.trBase, self, self->splashDamage, self->splashRadius, self, MOD_OVERMIND, TEAM_ALIENS ) )
        {
            self->timestamp = level.time;
                ...
                level.time > self->overmindSpawnsTimer )
        {
            ...
            BroadcastEvent( EV_OVERMIND_SPAWNS, 0 );
            ...
            if( !haveBuilder || TimeTilSuddenDeath( ) <= 0 )
                level.overmindMuted = true;
        }
        ...
        if( self->health < ( OVERMIND_HEALTH / 10.0f ) && level.time > self->overmindDyingTimer )
        {
            ...
            BroadcastEvent( EV_OVERMIND_DYING, 0 );
        }
        ...
        if( self->health < self->lastHealth && level.time > self->overmindAttackTimer )
        {
            ...
            BroadcastEvent( EV_OVERMIND_ATTACK, 0 );
        }
...
================
*/
void idGameLocal::ABarricade_Pain( gentity_t* self, gentity_t* attacker, S32 damage )
{
    if( self->health <= 0 )
        return;
    if( !self->shrunkTime )
...
================
*/
void idGameLocal::ABarricade_Shrink( gentity_t* self, bool shrink )
{
    if( !self->spawned || self->health <= 0 )
        shrink = true;
    ...
    if( shrink )
    {
        self->r.maxs[ 2 ] = ( S32 )( self->r.maxs[ 2 ] * BARRICADE_SHRINKPROP );
        self->shrunkTime = level.time;
...
================
*/
void idGameLocal::ABarricade_Die( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, S32 damage, S32 mod )
{
    AGeneric_Die( self, inflictor, attacker, damage, mod );
...
================
*/
void idGameLocal::ABarricade_Think( gentity_t* self )
{
    AGeneric_Think( self );
...
void idGameLocal::ABarricade_Touch( gentity_t* self, gentity_t* other, trace_t* trace )
{
    ...
    min_z = self->s.origin[ 2 ] - 18 +
            ( S32 )( self->r.maxs[ 2 ] * BARRICADE_SHRINKPROP );
    if( client_z < min_z )
        return;
    ABarricade_Shrink( self, true );
}
...
================
*/
void idGameLocal::AAcidTube_Think( gentity_t* self )
{
    S32       entityList[ MAX_GENTITIES ];
    ...
    if( self->spawned && self->health > 0 && FindOvermind( self ) )
    {
        ...
        for( i = 0; i < num; i++ )
        {
            ...
            if( !Visible( self, enemy, CONTENTS_SOLID ) )
                continue;
            ...
            if( enemy->client && enemy->client->ps.stats[ STAT_TEAM ] == TEAM_HUMANS )
            {
                ...
                if( level.time >= self->timestamp + ACIDTUBE_REPEAT_ANIM )
                {
                    self->timestamp = level.time;
                    SetBuildableAnim( self, BANIM_ATTACK1, false );
                    AddEvent( self, EV_ALIEN_ACIDTUBE, DirToByte( self->s.origin2 ) );
                }
                
                SelectiveRadiusDamage( self->s.pos.trBase, self, ACIDTUBE_DAMAGE, ACIDTUBE_RANGE, self, MOD_ATUBE, TEAM_ALIENS );
...
================
*/
bool idGameLocal::AHive_CheckTarget( gentity_t* self, gentity_t* enemy )
{
    trace_t trace;
...
================
*/
void idGameLocal::AHive_Think( gentity_t* self )
{
    self->powered = level.overmindPresent;
    self->nextthink = level.time + bggame->Buildable( ( buildable_t )self->s.modelindex )->nextthink;
    ...
    {
        S32 i, num, entityList[ MAX_GENTITIES ];
        vec3_t mins, maxs,
...
================
*/
void idGameLocal::AHive_Pain( gentity_t* self, gentity_t* attacker, S32 damage )
{
...
================
*/
void idGameLocal::AHive_Die( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, S32 damage, S32 mod )
{
...
/*
================
idGameLocal::AHovel_Blocked

Is this hovel entrance blocked?
================
*/
bool idGameLocal::AHovel_Blocked( gentity_t* hovel, gentity_t* player, bool provideExit )
{
    ...
    bggame->BuildableBoundingBox( BA_A_HOVEL, NULL, hovelMaxs );
    bggame->ClassBoundingBox( ( class_t )player->client->ps.stats[ STAT_CLASS ], mins, maxs, NULL, NULL, NULL );
...
bool idGameLocal::APropHovel_Blocked( vec3_t origin, vec3_t angles, vec3_t normal, gentity_t* player )
{
    gentity_t hovel;
...
================
*/
void idGameLocal::AHovel_Use( gentity_t* self, gentity_t* other, gentity_t* activator )
{
    ...
    if( self->spawned && FindOvermind( self ) )
    {
        if( self->active )
        {
            ...
            TriggerMenu( activator->client->ps.clientNum, MN_A_HOVEL_OCCUPIED );
        }
                 ...
                 activator->health > 0 && self->health > 0 )
        {
            if( AHovel_Blocked( self, activator, false ) )
            {
                ...
                TriggerMenu( activator->client->ps.clientNum, MN_A_HOVEL_BLOCKED );
                return;
            }
            ...
            SetOrigin( activator, hovelOrigin );
...
================
*/
void idGameLocal::AHovel_Think( gentity_t* self )
{
    self->powered = level.overmindPresent;
...
================
*/
void idGameLocal::AHovel_Die( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, S32 damage, S32 mod )
{
    ...
    if( self->active )
    {
        ...
        UnlaggedClear( builder );
        
        SetOrigin( builder, newOrigin );
...
================
*/
void idGameLocal::ABooster_Touch( gentity_t* self, gentity_t* other, trace_t* trace )
{
    gclient_t* client = other->client;
...
================
*/
void idGameLocal::ATrapper_FireOnEnemy( gentity_t* self, S32 firespeed, F32 range )
{
    gentity_t* enemy = self->enemy;
    vec3_t dirToTarget;
    vec3_t halfAcceleration, thirdJerk;
    F32 distanceToTarget = bggame->Buildable( ( buildable_t )self->s.modelindex )->turretRange;
    S32 lowMsec = 0;
    S32 highMsec = ( S32 )( ( ( ( distanceToTarget * LOCKBLOB_SPEED ) + ( distanceToTarget * bggame->Class( ( class_t )enemy->client->ps.stats[ STAT_CLASS ] )->speed ) ) / ( LOCKBLOB_SPEED * LOCKBLOB_SPEED ) ) * 1000.0f );
    ...
    {
        S32   partitionMsec = ( highMsec + lowMsec ) / 2;
        F32 time = ( F32 )partitionMsec / 1000.0f;
        F32 projectileDistance = LOCKBLOB_SPEED * time;
...
================
*/
bool idGameLocal::ATrapper_CheckTarget( gentity_t* self, gentity_t* target, S32 range )
{
    vec3_t    distance;
    trace_t   trace;
...
================
*/
void idGameLocal::ATrapper_FindEnemy( gentity_t* ent, S32 range )
{
    gentity_t* target;
...
================
*/
void idGameLocal::ATrapper_Think( gentity_t* self )
{
    S32 range = bggame->Buildable( ( buildable_t )self->s.modelindex )->turretRange;
    S32 firespeed = bggame->Buildable( ( buildable_t )self->s.modelindex )->turretFireSpeed;
...
================
*/
void idGameLocal::HRepeater_Think( gentity_t* self )
{
    S32       i;
    bool  reactor = false;
    gentity_t* ent;
    ...
    if( NumberOfDependants( self ) == 0 )
    {
        ...
        if( self->count < 0 )
            ...
            Damage( self, NULL, NULL, NULL, NULL, self->health, 0, MOD_SUICIDE );
    }
    else
        self->count = -1;
...
================
*/
void idGameLocal::HRepeater_Use( gentity_t* self, gentity_t* other, gentity_t* activator )
{
    if( self->health <= 0 || !self->spawned )
        return;
        
    if( other && other->client )
        GiveClientMaxAmmo( other, true );
}
...
================
*/
void idGameLocal::HReactor_Think( gentity_t* self )
{
    S32       entityList[ MAX_GENTITIES ];
    ...
    if( self->spawned && ( self->health > 0 ) )
    {
        ...
        for( i = 0; i < num; i++ )
        {
            ...
            tent = TempEntity( enemy->s.pos.trBase, EV_TESLATRAIL );
        ...
        if( fired )
        {
            self->timestamp = level.time;
            if( self->dcc )
                SelectiveRadiusDamage( self->s.pos.trBase, self, REACTOR_ATTACK_DCC_DAMAGE, REACTOR_ATTACK_DCC_RANGE, self, MOD_REACTOR, TEAM_HUMANS );
            else
                SelectiveRadiusDamage( self->s.pos.trBase, self, REACTOR_ATTACK_DAMAGE, REACTOR_ATTACK_RANGE, self, MOD_REACTOR, TEAM_HUMANS );
        }
    }
...
================
*/
void idGameLocal::HArmoury_Activate( gentity_t* self, gentity_t* other, gentity_t* activator )
{
    if( self->spawned )
    {
        ...
        if( self->powered )
            TriggerMenu( activator->client->ps.clientNum, MN_H_ARMOURY );
        else
            TriggerMenu( activator->client->ps.clientNum, MN_H_NOTPOWERED );
    }
}
...
================
*/
void idGameLocal::HArmoury_Think( gentity_t* self )
{
...
================
*/
void idGameLocal::HDCC_Think( gentity_t* self )
{
    //make sure we have power
    self->nextthink = level.time + POWER_REFRESH_TIME;
...
================
*/
void idGameLocal::HMedistat_Die( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, S32 damage, S32 mod )
{
...
================
*/
void idGameLocal::HMedistat_Think( gentity_t* self )
{
    S32       entityList[ MAX_GENTITIES ];
    vec3_t    mins, maxs;
    S32       i, num;
...
================
*/
bool idGameLocal::HMGTurret_CheckTarget( gentity_t* self, gentity_t* target, bool los_check )
{
    trace_t   tr;
    vec3_t    dir, end;
...
================
*/
bool idGameLocal::HMGTurret_TrackEnemy( gentity_t* self )
{
    vec3_t  dirToTarget, dttAdjusted, angleToTarget, angularDiff, xNormal;
    vec3_t  refNormal = { 0.0f, 0.0f, 1.0f };
    F32   temp, rotAngle, angularSpeed;
...
================
*/
void idGameLocal::HMGTurret_FindEnemy( gentity_t* self )
{
    S32       entityList[ MAX_GENTITIES ];
    vec3_t    range;
    vec3_t    mins, maxs;
...
================
*/
void idGameLocal::HMGTurret_Think( gentity_t* self )
{
    F32 temp;
    ...
    if( !self->active && self->count < level.time )
    {
        ...
        AddEvent( self, EV_MGTURRET_SPINUP, 0 );
    }
    ...
    AddEvent( self, EV_FIRE_WEAPON, 0 );
    SetBuildableAnim( self, BANIM_ATTACK1, false );
}
...
================
*/
void idGameLocal::HTeslaGen_Think( gentity_t* self )
{
    self->nextthink = level.time + bggame->Buildable( ( buildable_t )self->s.modelindex )->nextthink;
    ...
    if( self->spawned && self->count < level.time )
    {
        ...
        S32 entityList[ MAX_GENTITIES ], i, num;
...
================
*/
void idGameLocal::HSpawn_Disappear( gentity_t* self )
{
    vec3_t  dir;
...
================
*/
void idGameLocal::HSpawn_Blast( gentity_t* self )
{
    vec3_t  dir;
    ...
    RadiusDamage( self->s.pos.trBase, self, self->splashDamage, self->splashRadius, self, self->splashMethodOfDeath );
    ...
    AddEvent( self, EV_HUMAN_BUILDABLE_EXPLOSION, DirToByte( dir ) );
}
...
================
*/
void idGameLocal::HSpawn_Die( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, S32 damage, S32 mod )
{
    RewardAttackers( self );
    SetBuildableAnim( self, BANIM_DESTROY1, true );
    SetIdleBuildableAnim( self, BANIM_DESTROYED );
...
================
*/
void idGameLocal::HSpawn_Think( gentity_t* self )
{
    gentity_t* ent;
    ...
    if( self->spawned )
    {
        ...
        if( self->s.groundEntityNum )
        {
            if( ( ent = CheckSpawnPoint( self->s.number, self->s.origin, self->s.origin2, BA_H_SPAWN, NULL ) ) != NULL )
            {
                // If the thing blocking the spawn is a buildable, kill it.
                // If it's part of the map, kill self.
                if( ent->s.eType == ET_BUILDABLE )
...
/*
============
idGameLocal::QueueBuildPoints
============
*/
void idGameLocal::QueueBuildPoints( gentity_t* self )
{
...
============
*/
void idGameLocal::BuildableTouchTriggers( gentity_t* ent )
{
    S32       i, num;
...
/*
===============
idGameLocal::BuildableThink

General think function for buildables
===============
*/
void idGameLocal::BuildableThink( gentity_t* ent, S32 msec )
{
    S32 bHealth = bggame->Buildable( ( buildable_t )ent->s.modelindex )->health;
    S32 bRegen = bggame->Buildable( ( buildable_t )ent->s.modelindex )->regenRate;
    S32 bTime = bggame->Buildable( ( buildable_t )ent->s.modelindex )->buildTime;
    ...
    if( ent->time1000 >= 1000 )
    {
        ...
        if( !ent->spawned && ent->health > 0 )
            ent->health += ( S32 )( ceil( ( F32 )bHealth / ( F32 )( bTime * 0.001 ) ) );
    ...
    if( ent->health <= 0 )
        ...
        ent->s.generic1 = ( U8 )( ( B_HEALTH_MASK * ent->health + bHealth - 1 ) / bHealth );
    ...
    gameLocal.BuildableTouchTriggers( ent );
    
    //fall back on normal physics routines
    gameLocal.Physics( ent, msec );
}


/*
===============
idGameLocal::BuildableRange

Check whether a point is within some range of a type of buildable
===============
*/
bool idGameLocal::BuildableRange( vec3_t origin, F32 r, buildable_t buildable )
{
    S32       entityList[ MAX_GENTITIES ];
    vec3_t    range;
    vec3_t    mins, maxs;
...
/*
================
idGameLocal::FindBuildable

Finds a buildable of the specified type
================
*/
gentity_t* idGameLocal::FindBuildable( buildable_t buildable )
{
    S32       i;
    gentity_t* ent;
...
===============
*/
bool idGameLocal::BuildablesIntersect( buildable_t a, vec3_t originA, buildable_t b, vec3_t originB )
{
    vec3_t minsA, maxsA;
    vec3_t minsB, maxsB;
...
static S32 G_CompareBuildablesForRemoval( const void* a, const void* b )
{
    S32       precedence[ ] =
    {
        BA_NONE,
    ...
    S32       i;
    S32       aPrecedence = 0, bPrecedence = 0;
    ...
    aMatches = gameLocal.BuildablesIntersect( cmpBuildable, cmpOrigin, ( buildable_t )buildableA->s.modelindex, buildableA->s.origin );
    bMatches = gameLocal.BuildablesIntersect( cmpBuildable, cmpOrigin, ( buildable_t )buildableB->s.modelindex, buildableB->s.origin );
    ...
    {
        gentity_t* powerEntity = gameLocal.PowerEntityForPoint( cmpOrigin );
...
===============
*/
void idGameLocal::FreeMarkedBuildables( gentity_t* deconner )
{
    S32       i;
    gentity_t* ent;
    ...
    for( i = 0; i < level.numBuildablesForRemoval; i++ )
    {
        ...
        Damage( ent, NULL, deconner, NULL, NULL, ent->health, 0, MOD_DECONSTRUCT );
        FreeEntity( ent );
    }
}
...
===============
*/
itemBuildError_t idGameLocal::SufficientBPAvailable( buildable_t buildable, vec3_t origin )
{
    S32               i;
    S32               numBuildables = 0;
    S32               pointsYielded = 0;
    ...
    S32               buildPoints = bggame->Buildable( buildable )->buildPoints;
    S32               remainingBP, remainingSpawns;
    bool          collision = false;
    S32               collisionCount = 0;
    bool          repeaterInRange = false;
    S32               repeaterInRangeCount = 0;
...
================
*/
void idGameLocal::SetBuildableLinkState( bool link )
{
    S32       i;
    gentity_t* ent;
...
void idGameLocal::SetBuildableMarkedLinkState( bool link )
{
    S32       i;
    gentity_t* ent;
...
/*
================
idGameLocal::CanBuild

Checks to see if a buildable can be built
================
*/
itemBuildError_t idGameLocal::CanBuild( gentity_t* ent, buildable_t buildable, S32 distance, vec3_t origin )
{
    vec3_t            angles;
    ...
    if( ent->client->ps.stats[ STAT_TEAM ] == TEAM_ALIENS )
    {
        ...
        if( buildable == BA_A_HOVEL )
        {
            ...
            bggame->ClassBoundingBox( PCL_ALIEN_BUILDER0_UPG, builderMins, builderMaxs, NULL, NULL, NULL );
    ...
    if( reason == IBE_NONE )
    {
        ...
        if( CheckSpawnPoint( ENTITYNUM_NONE, origin, normal, buildable, NULL ) != NULL )
            reason = IBE_NORMAL;
...
================
*/
gentity_t* idGameLocal::Build( gentity_t* builder, buildable_t buildable, vec3_t origin, vec3_t angles )
{
    gentity_t* built;
    vec3_t    normal;
    ...
    switch( buildable )
    {
        ...
        case BA_A_BARRICADE:
            ...
            built->shrunkTime = 0;
            gameLocal.ABarricade_Shrink( built, true );
            break;
    ...
    AddEvent( built, EV_BUILD_CONSTRUCT, 0 );
...
/*
=================
idGameLocal::BuildIfValid
=================
*/
bool idGameLocal::BuildIfValid( gentity_t* ent, buildable_t buildable )
{
    F32         dist;
    vec3_t        origin;
    ...
    switch( CanBuild( ent, buildable, dist, origin ) )
    {
        ...
        case IBE_NOOVERMIND:
            TriggerMenu( ent->client->ps.clientNum, MN_A_NOOVMND );
            return false;
            
        case IBE_NOCREEP:
            TriggerMenu( ent->client->ps.clientNum, MN_A_NOCREEP );
            return false;
        ...
        case IBE_HOVELEXIT:
            TriggerMenu( ent->client->ps.clientNum, MN_A_HOVEL_EXIT );
            return false;
            
        case IBE_NORMAL:
            TriggerMenu( ent->client->ps.clientNum, MN_B_NORMAL );
            return false;
        ...
        case IBE_NOPOWERHERE:
            TriggerMenu( ent->client->ps.clientNum, MN_H_NOPOWERHERE );
            return false;
            
        case IBE_NOROOM:
            TriggerMenu( ent->client->ps.clientNum, MN_B_NOROOM );
            return false;
        ...
        case IBE_RPTPOWERHERE:
            TriggerMenu( ent->client->ps.clientNum, MN_H_RPTPOWERHERE );
            return false;
...
================
*/
void idGameLocal::FinishSpawningBuildable( gentity_t* ent )
{
    trace_t     tr;
    vec3_t      dest;
    gentity_t*   built;
    ...
    if( tr.startsolid )
    {
        Printf( S_COLOR_YELLOW "idGameLocal::FinishSpawningBuildable: %s startsolid at %s\n", built->classname, vtos( built->s.origin ) );
        FreeEntity( built );
        return;
    }
...
============
*/
void idGameLocal::SpawnBuildable( gentity_t* ent, buildable_t buildable )
{
    ent->s.modelindex = buildable;
...
/*
============
idGameLocal::LayoutSave
============
*/
void idGameLocal::LayoutSave( UTF8* name )
{
    UTF8 map[ MAX_QPATH ];
    ...
    if( !map[ 0 ] )
    {
        Printf( "LayoutSave( ): no map is loaded\n" );
        return;
    }
    ...
    if( len < 0 )
    {
        Printf( "layoutsave: could not open %s\n", fileName );
        return;
    }
    
    Printf( "layoutsave: saving layout to %s\n", fileName );
...
/*
============
idGameLocal::LayoutList
============
*/
S32 idGameLocal::LayoutList( StringEntry map, UTF8* list, S32 len )
{
    // up to 128 single character layout names could fit in layouts
    UTF8 fileList[( MAX_CVAR_VALUE_STRING / 2 ) * 5 ] = {""};
    UTF8 layouts[ MAX_CVAR_VALUE_STRING ] = {""};
    S32 numFiles, i, fileLen = 0, listLen;
    S32  count = 0;
    UTF8* filePtr;
    ...
    if( count != numFiles )
    {
        Printf( S_COLOR_YELLOW "WARNING: layout list was truncated to %d "
...
/*
============
idGameLocal::LayoutSelect

set level.layout based on g_layouts or g_layoutAuto
============
*/
void idGameLocal::LayoutSelect( void )
{
    UTF8 fileName[ MAX_OSPATH ];
    UTF8 layouts[ MAX_CVAR_VALUE_STRING ];
    UTF8 layouts2[ MAX_CVAR_VALUE_STRING ];
    UTF8* l;
    UTF8 map[ MAX_QPATH ];
    UTF8* s;
    S32 cnt = 0;
    ...
    while( 1 )
    {
        ...
        if( trap_FS_FOpenFile( fileName, NULL, FS_READ ) > 0 )
        {
            ...
            Printf( S_COLOR_YELLOW "idGameLocal::LayoutSelect: WARNING: layout \"%s\" does not exist\n", s );
    }
    if( !cnt )
    {
        Printf( S_COLOR_RED "idGameLocal::LayoutSelect: ERROR: none of the specified layouts could be "
    ...
    Printf( "idGameLocal::LayoutSelect: Using layout \"%s\" from list (%s)\n", level.layout, layouts );
}
...
============
*/
void idGameLocal::LayoutBuildItem( buildable_t buildable, vec3_t origin, vec3_t angles, vec3_t origin2, vec3_t angles2 )
{
    gentity_t* builder;
...
void idGameLocal::LayoutLoad( void )
{
    ...
    if( len < 0 )
    {
        Printf( "ERROR: layout %s could not be opened\n", level.layout );
        return;
    }
    layout = ( UTF8* )bggame->Alloc( len + 1 );
    ...
    *( layout + len ) = '\0';
    ...
    while( *layout )
    {
        if( i >= sizeof( line ) - 1 )
        {
            Printf( S_COLOR_RED "ERROR: line overflow in %s before \"%s\"\n",
        ...
        if( *layout == '\n' )
        {
            ...
            if( buildable > BA_NONE && buildable < BA_NUM_BUILDABLES )
                ...
                Printf( S_COLOR_YELLOW "WARNING: bad buildable number (%d) in layout.  skipping\n", buildable );
        }
        layout++;
    }
}
...
/*
============
idGameLocal::BaseSelfDestruct
============
*/
void idGameLocal::BaseSelfDestruct( team_t team )
{
    S32       i;
    gentity_t* ent;
