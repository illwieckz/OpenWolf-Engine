...
static  S32     cg_numSolidEntities;
static  centity_t* cg_solidEntities[MAX_ENTITIES_IN_SNAPSHOT];
static  S32     cg_numTriggerEntities;
...
====================
*/
void idCGameLocal::BuildSolidList( void )
{
    S32           i;
    centity_t*     cent;
    snapshot_t*    snap;
...
====================
*/
void idCGameLocal::ClipMoveToEntities( const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, S32 skipNumber, S32 mask, trace_t* tr, traceType_t collisionType )
{
    S32           i, j, x, zd, zu;
    trace_t       trace;
    entityState_t* ent;
    ...
    for( i = 0; i < j; i++ )
    {
        ...
        else
        {
            ...
            if( i == cg_numSolidEntities )
                bggame->ClassBoundingBox( ( class_t )( ( ent->misc >> 8 ) & 0xFF ), bmins, bmaxs, NULL, NULL, NULL );
        ...
        if( trace.allsolid || trace.fraction < tr->fraction )
        {
            ...
            {
                F32 oldLateralFraction = tr->lateralFraction;
                *tr = trace;
...
================
*/
void idCGameLocal::Trace( trace_t* result, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, S32 skipNumber, S32 mask )
{
    trace_t t;
...
================
*/
void idCGameLocal::CapTrace( trace_t* result, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, S32 skipNumber, S32 mask )
{
    trace_t t;
...
================
*/
void idCGameLocal::BiSphereTrace( trace_t* result, const vec3_t start, const vec3_t end, const F32 startRadius, const F32 endRadius, S32 skipNumber, S32 mask )
{
    trace_t t;
    vec3_t  mins, maxs;
...
================
*/
S32 idCGameLocal::PointContents( const vec3_t point, S32 passEntityNum )
{
    S32           i;
    entityState_t* ent;
    centity_t*     cent;
...
========================
*/
void idCGameLocal::InterpolatePlayerState( bool grabAngles )
{
    F32         f;
    S32           i;
    playerState_t* out;
    ...
    *out = cg.snap->ps;
    ...
    f = ( F32 )( cg.time - prev->serverTime ) / ( next->serverTime - prev->serverTime );
...
=========================
*/
void idCGameLocal::TouchTriggerPrediction( void )
{
    S32           i;
    trace_t       trace;
    entityState_t* ent;
...
S32 idCGameLocal::IsUnacceptableError( playerState_t* ps, playerState_t* pps )
{
    vec3_t delta;
    S32 i;
...
=================
*/
void idCGameLocal::PredictPlayerState( void )
{
    S32     cmdNum, current, i;
    playerState_t oldPlayerState;
    bool  moved;
    usercmd_t oldestCmd;
    usercmd_t latestCmd;
    S32 stateIndex = 0, predictCmd = 0;
    ...
    cg_pmove.trace = cgameLocal.Trace;
    cg_pmove.pointcontents = cgameLocal.PointContents;
    ...
    for( cmdNum = current - CMD_BACKUP + 1; cmdNum <= current; cmdNum++ )
    {
        ...
        if( cg.predictedPlayerState.commandTime == oldPlayerState.commandTime )
        {
            ...
            {
                vec3_t  adjusted;
                AdjustPositionForMover( cg.predictedPlayerState.origin, cg.predictedPlayerState.groundEntityNum, cg.physicsTime, cg.oldTime, adjusted );
    ...
    AdjustPositionForMover( cg.predictedPlayerState.origin, cg.predictedPlayerState.groundEntityNum, cg.physicsTime, cg.time, cg.predictedPlayerState.origin );
