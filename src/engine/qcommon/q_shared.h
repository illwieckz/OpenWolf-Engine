...
S32 Q_vsnprintf( UTF8* str, U64 size, StringEntry format, va_list args );
#else // not using MSVC
...
void* Hunk_AllocDebug( S32 size, ha_pref preference, UTF8* label, UTF8* file, S32 line );
#else
...
void			Q_strncpyz2( UTF8* dst, StringEntry src, S32 dstSize );
...
// parse "\n" into '\n'
void Q_ParseNewlines( UTF8* dest, StringEntry src, S32 destsize );
...
void Com_ClientListAdd( clientList_t* list, S32 clientNum );
void Com_ClientListRemove( clientList_t* list, S32 clientNum );
