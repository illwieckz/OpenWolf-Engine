...
// Changes Globals:		-
//===========================================================================
UTF8* AAS_LoadAASLump( fileHandle_t fp, S32 offset, S32 length, S32* lastoffset, S32 size )
{
    UTF8* buf;
    //
    if( !length )
    {
        //just alloc a dummy
        return ( UTF8* ) GetClearedHunkMemory( size + 1 );
    } //end if
    //seek to the data
    ...
    //allocate memory
    buf = ( UTF8* ) GetClearedHunkMemory( length + 1 );
    //read the data
    if( length )
    {
...
// Changes Globals:		-
//===========================================================================
void AAS_DData( U8* data, S32 size )
{
    S32 i;
...
// Changes Globals:		-
//===========================================================================
static S32 AAS_WriteAASLump_offset;

S32 AAS_WriteAASLump( fileHandle_t fp, aas_header_t* h, S32 lumpnum, void* data, S32 length )
{
    aas_lump_t* lump;
