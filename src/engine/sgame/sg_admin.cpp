...
g_admin_cmd_t g_admin_cmds[ ] =
{
    ...
    {
        "listchars", &idGameLocal::ListChars, "v",
        "display bot characters available to you or info on a specific character",
        "(charactor)",
    },
};

static S32 adminNumCmds = sizeof( g_admin_cmds ) / sizeof( g_admin_cmds[ 0 ] );
static S32 admin_level_maxname = 0;
...
===============
*/
bool idAdminLocal::AdminPermission( gentity_t* ent, UTF8 flag )
{
    S32 i, l = 0;
    UTF8* flags;
...
===============
*/
bool idAdminLocal::AdminNameCheck( gentity_t* ent, UTF8* name, UTF8* err, S32 len )
{
    S32 i;
    gclient_t* client;
    UTF8 testName[ MAX_NAME_LENGTH ] = {""};
    UTF8 name2[ MAX_NAME_LENGTH ] = {""};
    
    gameLocal.SanitiseString( name, name2, sizeof( name2 ) );
    ...
    for( i = 0; i < level.maxclients; i++ )
    {
        ...
        gameLocal.SanitiseString( client->pers.netname, testName, sizeof( testName ) );
    ...
    for( i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[ i ]; i++ )
    {
        ...
        gameLocal.SanitiseString( g_admin_admins[ i ]->name, testName, sizeof( testName ) );
...
===============
*/
bool idAdminLocal::AdminHigherGuid( UTF8* admin_guid, UTF8* victim_guid )
{
    S32 i, alevel = 0;
...
===============
*/
void idAdminLocal::AdminWriteConfigString( UTF8* s, fileHandle_t f )
{
...
===============
*/
void idAdminLocal::AdminWriteConfigInt( S32 v, fileHandle_t f )
{
    UTF8 buf[ 32 ];
...
void idAdminLocal::AdminWriteConfig( void )
{
    fileHandle_t f;
    S32 len, i, j, t;
    UTF8 levels[ MAX_STRING_CHARS ] = {""};
    
    if( !g_admin.string[ 0 ] )
    {
        gameLocal.Printf( S_COLOR_YELLOW "WARNING: g_admin is not set. configuration will not be saved to a file.\n" );
        return;
    }
    ...
    if( len < 0 )
    {
        gameLocal.Printf( "admin_writeconfig: could not open g_admin file \"%s\"\n", g_admin.string );
        return;
    }
...
===============
*/
void idAdminLocal::AdminReadConfigString( UTF8** cnf, UTF8* s, S32 size )
{
    UTF8* t;
...
===============
*/
void idAdminLocal::AdminReadConfigInt( UTF8** cnf, S32* v )
{
    UTF8* t;
...
===============
*/
bool idAdminLocal::AdminCommandPermission( gentity_t* ent, UTF8* command )
{
    S32 i, j, level;
...
===============
*/
void idAdminLocal::AdminLog( gentity_t* admin, UTF8* cmd, S32 skiparg )
{
    fileHandle_t f;
    S32 len, i, j, min, tens, sec, pids[MAX_CLIENTS];
    UTF8 string[ MAX_STRING_CHARS ];
    g_admin_admin_t* a;
    g_admin_level_t* l;
    UTF8 flags[ MAX_ADMIN_FLAGS * 2 ];
    ...
    if( len < 0 )
    {
        gameLocal.Printf( "idAdminLocal::AdminLog: error could not open %s\n", g_adminLog.string );
        return;
    }
    ...
    *flags = '\0';
    ...
    if( gameLocal.SayArgc() > 1 + skiparg )
    {
        gameLocal.SayArgv( 1 + skiparg, name, sizeof( name ) );
        if( gameLocal.ClientNumbersFromString( name, pids, MAX_CLIENTS ) == 1 )
        {
    ...
    if( victim && Q_stricmp( cmd, "attempted" ) )
    {
        Com_sprintf( string, sizeof( string ),
                     ...
                     gameLocal.SayConcatArgs( 2 + skiparg ) );
    }
    else
    {
        Com_sprintf( string, sizeof( string ),
                     ...
                     ( admin ) ? admin->client->pers.netname : "console",
                     flags, cmd,
                     gameLocal.SayConcatArgs( 1 + skiparg ) );
    }
...
===============
*/
S32 idAdminLocal::AdminListAdmins( gentity_t* ent, S32 start, UTF8* search )
{
    S32 drawn = 0;
    UTF8 guid_stub[9];
    UTF8 name[ MAX_NAME_LENGTH ] = {""};
    UTF8 name2[ MAX_NAME_LENGTH ] = {""};
    UTF8 lname[ MAX_NAME_LENGTH ] = {""};
    S32 i, j;
    gentity_t* vic;
    S32 l = 0;
    bool dup = false;
    ...
    for( i = 0; i < level.maxclients && search[ 0 ]; i++ )
    {
        ...
        gameLocal.SanitiseString( vic->client->pers.netname, name, sizeof( name ) );
    ...
    for( i = start; i < MAX_ADMIN_ADMINS && g_admin_admins[ i ] && drawn < MAX_ADMIN_LISTITEMS; i++ )
    {
        if( search[ 0 ] )
        {
            gameLocal.SanitiseString( g_admin_admins[ i ]->name, name, sizeof( name ) );
            if( !strstr( name, search ) )
                continue;
            ...
            for( j = 0; j < level.maxclients; j++ )
            {
                ...
                gameLocal.SanitiseString( vic->client->pers.netname, name2, sizeof( name2 ) );
...
===============
*/
void idAdminLocal::AdminDuration( S32 secs, UTF8* duration, S32 dursize )
{
...
===============
*/
bool idAdminLocal::AdminBanCheck( UTF8* userinfo, UTF8* reason, S32 rlen )
{
    UTF8* guid, *ip;
    S32 i, t;
    ...
    *reason = '\0';
    ...
    for( i = 0; i < MAX_ADMIN_BANS && g_admin_bans[ i ]; i++ )
    {
        ...
        if( strstr( ip, g_admin_bans[ i ]->ip ) )
        {
            ...
            gameLocal.Printf( "Banned player tried to connect from IP %s\n", ip );
            
            if( trap_Database_RunQuery( va( "INSERT INTO banconnect (ip) VALUES (\"%s\")", ip ) ) == true )
            {
                UTF8 data[255];
                
                trap_Database_FinishQuery();
                trap_Database_RunQuery( "SELECT id FROM banconnect ORDER BY id DESC LIMIT 1" );
                trap_Database_FetchRow();
                trap_Database_FetchFieldByName( "id", data, sizeof( data ) );
                trap_Database_RunQuery( va( "INSERT into mainlog (idaction,typeaction,date) VALUES (%d,\"%d\",NOW())", atoi( data ), 0 ) );
                trap_Database_FinishQuery();
            }
        ...
        if( *guid && !Q_stricmp( g_admin_bans[ i ]->guid, guid ) )
        {
            ...
            AdminDuration( ( g_admin_bans[ i ]->expires - t ), duration, sizeof( duration ) );
            Com_sprintf( reason, rlen, "You have been banned by %s^7 reason: %s^7 expires: %s", g_admin_bans[ i ]->banner, g_admin_bans[ i ]->reason, duration );
            gameLocal.Printf( "Banned player tried to connect with GUID %s\n", guid );
            return true;
        }
    }
    return false;
...
bool idAdminLocal::AdminCmdCheck( gentity_t* ent, bool say )
{
    ...
    command[ 0 ] = '\0';
    gameLocal.SayArgv( 0, command, sizeof( command ) );
    ...
    {
        skip = 1;
        gameLocal.SayArgv( 1, command, sizeof( command ) );
    }
    ...
    for( i = 0; i < MAX_ADMIN_COMMANDS && g_admin_commands[ i ]; i++ )
    {
        ...
        if( AdminCommandPermission( ent, cmd ) )
        {
            ...
            if( !say && gameLocal.FloodLimited( ent ) )
            {
                return true;
            }
...
void idAdminLocal::AdminNamelogUpdate( gclient_t* client, bool disconnect )
{
    ...
    UTF8 n1[ MAX_NAME_LENGTH ];
    UTF8 n2[ MAX_NAME_LENGTH ];
    S32 clientNum = ( client - level.clients );
    
    gameLocal.SanitiseString( client->pers.netname, n1, sizeof( n1 ) );
    for( i = 0; i < MAX_ADMIN_NAMELOGS && g_admin_namelog[ i ]; i++ )
    {
        ...
        if( !Q_stricmp( client->pers.ip, g_admin_namelog[ i ]->ip ) && !Q_stricmp( client->pers.guid, g_admin_namelog[ i ]->guid ) )
        {
            ...
            {
                gameLocal.SanitiseString( g_admin_namelog[ i ]->name[ j ], n2, sizeof( n2 ) );
    ...
    {
        gameLocal.Printf( "idAdminLocal::AdminNamelogUpdate: warning, g_admin_namelogs overflow\n" );
        return;
    }
...
bool idAdminLocal::AdminReadconfig( gentity_t* ent, S32 skiparg )
{
    ...
    if( len < 0 )
    {
        gameLocal.Printf( "^3!readconfig: ^7could not open admin config file %s\n", g_admin.string );
        adminLocal.AdminDefaultLevels();
        return false;
    }
    
    cnf = ( UTF8* )bggame->Alloc( len + 1 );
    cnf2 = cnf;
...
===============
*/
bool idAdminLocal::AdminTime( gentity_t* ent, S32 skiparg )
{
    qtime_t qt;
...
===============
*/
bool idAdminLocal::AdminSetlevel( gentity_t* ent, S32 skiparg )
{
    UTF8 name[ MAX_NAME_LENGTH ] = {""};
    UTF8 lstr[ 11 ]; // 10 is max strlen() for 32-bit S32
    UTF8 adminname[ MAX_NAME_LENGTH ] = {""};
    UTF8 testname[ MAX_NAME_LENGTH ] = {""};
    UTF8 guid[ 33 ];
    S32 l, i, matches = 0, id = -1;
    gentity_t* vic = NULL;
    bool updated = false;
    g_admin_admin_t* a;
    bool found = false;
    ...
    if( gameLocal.SayArgc() < 3 + skiparg )
    {
        adminLocal.ADMP( "^3!setlevel: ^7usage: !setlevel [name|slot#] [level]\n" );
        return false;
    }
    
    gameLocal.SayArgv( 1 + skiparg, testname, sizeof( testname ) );
    gameLocal.SayArgv( 2 + skiparg, lstr, sizeof( lstr ) );
    
    l = atoi( lstr );
    gameLocal.SanitiseString( testname, name, sizeof( name ) );
    ...
    else
    {
        ...
        for( i = 0; i < level.maxclients && matches < 2; i++ )
        {
            ...
            gameLocal.SanitiseString( level.clients[ i ].pers.netname, testname, sizeof( testname ) );
...
===============
*/
bool idAdminLocal::AdminCreateBan( gentity_t* ent, UTF8* netname, UTF8* guid, UTF8* ip, S32 seconds, UTF8* reason )
{
    g_admin_ban_t* b = NULL;
    qtime_t qt;
    S32 t, i;
...
===============
*/
S32 idAdminLocal::AdminParseTime( StringEntry time )
{
    S32 seconds = 0, num = 0;
    while( *time )
    {
...
===============
*/
bool idAdminLocal::AdminKick( gentity_t* ent, S32 skiparg )
{
    S32 pids[ MAX_CLIENTS ], found, minargc;
    UTF8 name[ MAX_NAME_LENGTH ], *reason, err[ MAX_STRING_CHARS ];
    gentity_t* vic;
    ...
    gameLocal.SayArgv( 1 + skiparg, name, sizeof( name ) );
    reason = gameLocal.SayConcatArgs( 2 + skiparg );
    
    if( ( found = gameLocal.ClientNumbersFromString( name, pids, MAX_CLIENTS ) ) != 1 )
    {
        gameLocal.MatchOnePlayer( pids, found, err, sizeof( err ) );
...
===============
*/
bool idAdminLocal::AdminBan( gentity_t* ent, S32 skiparg )
{
    S32 seconds;
    UTF8 search[ MAX_NAME_LENGTH ];
    UTF8 secs[ MAX_TOKEN_CHARS ];
    UTF8* reason;
    S32 minargc;
    UTF8 duration[ 32 ];
    S32 logmatch = -1, logmatches = 0;
    S32 i, j;
    ...
    gameLocal.SayArgv( 1 + skiparg, search, sizeof( search ) );
    gameLocal.SanitiseString( search, s2, sizeof( s2 ) );
    gameLocal.SayArgv( 2 + skiparg, secs, sizeof( secs ) );
    ...
    {
        reason = gameLocal.SayConcatArgs( 3 + skiparg );
    }
    ...
    {
        S32 slot = g_admin_namelog[ logmatch ]->slot;
        if( g_entities[slot].r.svFlags & SVF_BOT )
        {
            adminLocal.ADMP( "^3!ban: ^7banning a bot would cause bad things\n" );
            return false;
        }
    }
...
===============
*/
bool idAdminLocal::AdminUnBan( gentity_t* ent, S32 skiparg )
{
    S32 bnum;
    UTF8 bs[ 5 ];
    
    if( gameLocal.SayArgc() < 2 + skiparg )
    {
        adminLocal.ADMP( "^3!unban: ^7usage: !unban [ban#]\n" );
        return false;
    }
    
    gameLocal.SayArgv( 1 + skiparg, bs, sizeof( bs ) );
...
===============
*/
bool idAdminLocal::AdminAdjustBan( gentity_t* ent, S32 skiparg )
{
    S32 bnum, length, expires, time = trap_RealTime( NULL );
    UTF8 duration[ 32 ] = {""};
    UTF8* reason;
    UTF8 bs[ 5 ];
    UTF8 secs[ MAX_TOKEN_CHARS ];
...
===============
*/
bool idAdminLocal::AdminPutTeam( gentity_t* ent, S32 skiparg )
{
    S32 pids[ MAX_CLIENTS ], found;
    UTF8 name[ MAX_NAME_LENGTH ], team[ 7 ], err[ MAX_STRING_CHARS ];
    gentity_t* vic;
    team_t teamnum = TEAM_NONE;
    
    gameLocal.SayArgv( 1 + skiparg, name, sizeof( name ) );
    gameLocal.SayArgv( 2 + skiparg, team, sizeof( team ) );
    ...
    vic = &g_entities[ pids[ 0 ] ];
    teamnum = gameLocal.TeamFromString( team );
    ...
    gameLocal.ChangeTeam( vic, teamnum );
...
===============
*/
bool idAdminLocal::AdminMap( gentity_t* ent, S32 skiparg )
{
    UTF8 map[ MAX_QPATH ];
    ...
    if( gameLocal.SayArgc( ) > 2 + skiparg )
    {
        gameLocal.SayArgv( skiparg + 2, layout, sizeof( layout ) );
...
===============
*/
bool idAdminLocal::AdminMute( gentity_t* ent, S32 skiparg )
{
    S32 pids[ MAX_CLIENTS ], found;
    UTF8 name[ MAX_NAME_LENGTH ], err[ MAX_STRING_CHARS ];
    UTF8 command[ MAX_ADMIN_CMD_LEN ], *cmd;
    gentity_t* vic;
    
    gameLocal.SayArgv( skiparg, command, sizeof( command ) );
...
===============
*/
bool idAdminLocal::AdminDenyBuild( gentity_t* ent, S32 skiparg )
{
    S32 pids[ MAX_CLIENTS ], found;
    UTF8 name[ MAX_NAME_LENGTH ], err[ MAX_STRING_CHARS ];
    UTF8 command[ MAX_ADMIN_CMD_LEN ], *cmd;
...
===============
*/
bool idAdminLocal::AdminListAdmins( gentity_t* ent, S32 skiparg )
{
    S32 i, found = 0;
    UTF8 search[ MAX_NAME_LENGTH ] = {""};
    UTF8 s[ MAX_NAME_LENGTH ] = {""};
    S32 start = 0;
    bool numeric = true;
    S32 drawn = 0;
    ...
    if( gameLocal.SayArgc() == 2 + skiparg )
    {
        gameLocal.SayArgv( 1 + skiparg, s, sizeof( s ) );
        ...
        {
            gameLocal.SanitiseString( s, search, sizeof( search ) );
        }
    }
...
===============
*/
bool idAdminLocal::AdminListLayouts( gentity_t* ent, S32 skiparg )
{
    UTF8 list[ MAX_CVAR_VALUE_STRING ];
    UTF8 map[ MAX_QPATH ];
    S32 count = 0, i = 0;
    UTF8* s;
    UTF8 layout[ MAX_QPATH ] = { "" };
    ...
    if( gameLocal.SayArgc() == 2 + skiparg )
    {
        gameLocal.SayArgv( 1 + skiparg, map, sizeof( map ) );
    }
    else
    {
    ...
    count = gameLocal.LayoutList( map, list, sizeof( list ) );
...
===============
*/
bool idAdminLocal::AdminListPlayers( gentity_t* ent, S32 skiparg )
{
    S32 i, j, l;
    gclient_t* p;
    UTF8 c[ 3 ], t[ 2 ]; // color and team letter
    UTF8 n[ MAX_NAME_LENGTH ] = {""};
    UTF8 n2[ MAX_NAME_LENGTH ] = {""};
    UTF8 n3[ MAX_NAME_LENGTH ] = {""};
    UTF8 lname[ MAX_NAME_LENGTH ];
    UTF8 guid_stub[ 9 ];
    UTF8 muted[ 2 ], denied[ 2 ];
    ...
    for( i = 0; i < level.maxclients; i++ )
    {
        ...
        l = 0;
        gameLocal.SanitiseString( p->pers.netname, n2, sizeof( n2 ) );
        ...
        for( j = 0; j < MAX_ADMIN_ADMINS && g_admin_admins[ j ]; j++ )
        {
            if( !Q_stricmp( g_admin_admins[ j ]->guid, p->pers.guid ) )
            {
                ...
                gameLocal.SanitiseString( g_admin_admins[ j ]->name, n3, sizeof( n3 ) );
...
===============
*/
bool idAdminLocal::AdminShowBans( gentity_t* ent, S32 skiparg )
{
    S32 i, found = 0, max = -1, count, t, j, k;
    UTF8 duration[ 32 ];
    S32 max_name = 1, max_banner = 1, colorlen;
    S32 len, secs, start = 0;
    UTF8 skip[ 11 ];
    UTF8 date[ 11 ];
    UTF8* made;
    UTF8 n1[ MAX_NAME_LENGTH ] = {""};
    ...
    if( gameLocal.SayArgc() == 2 + skiparg )
    {
        gameLocal.SayArgv( 1 + skiparg, skip, sizeof( skip ) );
        start = atoi( skip );
...
===============
*/
bool idAdminLocal::AdminHelp( gentity_t* ent, S32 skiparg )
{
    S32 i;
    ...
    {
        //!help param
        UTF8 param[ MAX_ADMIN_CMD_LEN ];
        UTF8* cmd;
        
        gameLocal.SayArgv( 1 + skiparg, param, sizeof( param ) );
...
===============
*/
bool idAdminLocal::AdminAdminTest( gentity_t* ent, S32 skiparg )
{
    S32 i, l = 0;
    bool found = false;
    bool lname = false;
...
===============
*/
bool idAdminLocal::AdminAllReady( gentity_t* ent, S32 skiparg )
{
    S32 i = 0;
    gclient_t* cl;
...
===============
*/
bool idAdminLocal::AdminCancelVote( gentity_t* ent, S32 skiparg )
{
    ...
    gameLocal.CheckTeamVote( TEAM_HUMANS );
    
    level.teamVoteNo[ 1 ] = level.numConnectedClients;
    level.teamVoteYes[ 1 ] = 0;
    
    gameLocal.CheckTeamVote( TEAM_ALIENS );
...
===============
*/
bool idAdminLocal::AdminPassVote( gentity_t* ent, S32 skiparg )
{
    if( !level.voteTime && !level.teamVoteTime[ 0 ] && !level.teamVoteTime[ 1 ] )
    {
...
===============
*/
bool idAdminLocal::AdminSpec999( gentity_t* ent, S32 skiparg )
{
    S32 i;
    gentity_t* vic;
    ...
    for( i = 0; i < level.maxclients; i++ )
    {
        ...
        {
            gameLocal.ChangeTeam( vic, TEAM_NONE );
...
===============
*/
bool idAdminLocal::AdminRename( gentity_t* ent, S32 skiparg )
{
    S32 pids[ MAX_CLIENTS ], found;
    UTF8 name[ MAX_NAME_LENGTH ];
    ...
    s = gameLocal.SayConcatArgs( 2 + skiparg );
    ...
    gameLocal.ClientUserinfoChanged( pids[ 0 ] );
...
===============
*/
bool idAdminLocal::AdminRestart( gentity_t* ent, S32 skiparg )
{
    UTF8 layout[MAX_CVAR_VALUE_STRING] = { "" };
    UTF8 teampref[MAX_STRING_CHARS] = { "" };
    S32 i;
    gclient_t* cl;
...
===============
*/
bool idAdminLocal::AdminNextmap( gentity_t* ent, S32 skiparg )
{
    ...
    gameLocal.LogExit( va( "nextmap was run by %s", ( ent ) ? ent->client->pers.netname : "console" ) );
...
===============
*/
bool idAdminLocal::AdminNameLog( gentity_t* ent, S32 skiparg )
{
    S32 i, j;
    UTF8 search[ MAX_NAME_LENGTH ] = {""};
    UTF8 s2[ MAX_NAME_LENGTH ] = {""};
...
===============
*/
bool idAdminLocal::AdminLock( gentity_t* ent, S32 skiparg )
{
    UTF8 teamName[2] = {""};
    team_t team;
    ...
    gameLocal.SayArgv( 1 + skiparg, teamName, sizeof( teamName ) );
    team = gameLocal.TeamFromString( teamName );
...
===============
*/
bool idAdminLocal::AdminUnlock( gentity_t* ent, S32 skiparg )
{
    UTF8 teamName[2] = {""};
    team_t team;
...
/*
===============
idAdminLocal::AdminBot
===============
*/
bool idAdminLocal::AdminBot( gentity_t* ent, S32 skiparg )
{
    // add [name] (team) (skill)
    // del [name]
    gentity_t* bot;
    S32 minargc, matches, team_int;
    ...
    minargc = 3 + skiparg;
    if( gameLocal.SayArgc() < skiparg + 1 )
    {
        adminLocal.ADMP( "^7Incorrect number of arguments. Use 'bot removeall' to remove all bots, 'bot add [name] [team]' to add a bot, 'bot del [name]' to remove a bot\n" );
        //adminLocal.ADMP( "^3!bot: ^7usage: !bot [add/del] [name] (team) (skill)\n" );
        adminLocal.ADMP( "^3!bot: ^7usage: !bot add [name] [h/a] (character) (skill)\n" );
        return false;
    }
    
    gameLocal.SayArgv( 1 + skiparg, command, sizeof( command ) );
    gameLocal.SayArgv( 2 + skiparg, name, sizeof( name ) );
    ...
    if( !Q_stricmp( command, "add" ) )
    {
        ...
        if( gameLocal.SayArgc() < minargc )
        {
            adminLocal.ADMP( "^7Please have at least name and team.\n" );
            //adminLocal.ADMP( "^3!bot: ^7usage: !bot [add/del] [name] [humans/aliens] \n" );
            adminLocal.ADMP( "^3!bot: ^7usage: !bot add [name] [h/a] (character) (skill)\n" );
            return false;
        }
        
        gameLocal.SayArgv( 3 + skiparg, team, sizeof( team ) );
        ...
        {
            adminLocal.ADMP( "^7Invalid bot team.\n" );
            adminLocal.ADMP( "^3!bot: ^7usage: !bot add [name] [h/a] (character) (skill)\n" );
            return false;
        }
        
        //if( team_int == TEAM_HUMANS)
        switch( gameLocal.SayArgc() - skiparg )
        {
            case 4: //no skill or charactor name
                adminLocal.ADMP( "^3!bot add: ^7bot charater not specified, using random\n" );
                skill_float = 2;
                gameLocal.AddRandomBot( ent, team_int, name , &skill_float );
                return true;
                
            case 5: // either charactor name or skill level
                gameLocal.SayArgv( 4 + skiparg, charname, sizeof( charname ) );
                gameLocal.SayArgv( 4 + skiparg, skill, sizeof( skill ) );
                botinfo = gameLocal.GetBotInfoByName( charname );
                skill_float = atof( skill );
                
                if( botinfo )
                {
                    skill_float = -1;
                    gameLocal.BotAdd( ent, name, team_int, &skill_float, botinfo );
                    return true;
                }
                ...
                {
                    adminLocal.ADMP( "^3!bot add: ^1ERROR:^7 Invalid charactor or skill specifed\n" );
                    adminLocal.ADMP( "^3!bot add: ^1See !listchars for bot characters and skill level must be between 1.0-5.0\n" );
                    return false;
                }
                break;
            ...
            case 6:
                gameLocal.SayArgv( 4 + skiparg, charname, sizeof( charname ) );
                gameLocal.SayArgv( 5 + skiparg, skill, sizeof( skill ) );
                botinfo = gameLocal.GetBotInfoByName( charname );
                skill_float = atof( skill );
                
                if( skill_float > 5.0 || skill_float < 1.0 )
                {
                    adminLocal.ADMP( "^3!bot add: ^7bot skill level must be between 1 and 5\n" );
                    return false;
                }
    ...
    else if( !Q_stricmp( command, "del" ) )
    {
        ...
        if( gameLocal.SayArgc() < minargc )
        {
            adminLocal.ADMP( "^7Please have at least name\n" );
            adminLocal.ADMP( "^3!bot: ^7usage: !bot del [name] (team)\n" );
            return false;
        }
        ...
        matches = gameLocal.ClientNumbersFromString( name, pids, MAX_CLIENTS );
        for( i = 0; i < matches; i++ )
        {
    ...
    {
        gameLocal.BotRemoveAll( ent );
        return true;
    }
    
    adminLocal.ADMP( "^3!bot: ^7usage: !bot [add/del] [name] (team) (charactor) (skill)\n" );
...
================
*/
void idAdminLocal::AdminPrint( gentity_t* ent, UTF8* m )
{
    if( ent )
    {
    ...
    {
        UTF8 m2[ MAX_STRING_CHARS ];
        if( !trap_Cvar_VariableIntegerValue( "com_ansiColor" ) )
        {
            gameLocal.DecolorString( m, m2, sizeof( m2 ) );
            trap_Print( m2 );
        }
        else
        {
...
===============
*/
void idAdminLocal::AdminBufferPrint( gentity_t* ent, UTF8* m )
{
