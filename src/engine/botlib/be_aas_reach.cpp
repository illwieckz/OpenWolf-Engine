...
S32 reach_swim;			//swim
S32 reach_equalfloor;	//walk on floors with equal height
S32 reach_step;			//step up
S32 reach_walk;			//walk of step
S32 reach_barrier;		//jump up to a barrier
S32 reach_waterjump;	//jump out of water
S32 reach_walkoffledge;	//walk of a ledge
S32 reach_jump;			//jump
S32 reach_ladder;		//climb or descent a ladder
S32 reach_teleport;		//teleport
S32 reach_elevator;		//use an elevator
S32 reach_funcbob;		//use a func bob
S32 reach_grapple;		//grapple hook
S32 reach_doublejump;	//double jump
S32 reach_rampjump;		//ramp jump
S32 reach_strafejump;	//strafe jump (just normal jump but further)
S32 reach_rocketjump;	//rocket jump
S32 reach_bfgjump;		//bfg jump
S32 reach_jumppad;		//jump pads
//if true grapple reachabilities are skipped
S32 calcgrapplereach;
//linked reachability
typedef struct aas_lreachability_s
{
    S32 areanum;					//number of the reachable area
    S32 facenum;					//number of the face towards the other area
    S32 edgenum;					//number of the edge towards the other area
    vec3_t start;					//start point of inter area movement
    vec3_t end;						//end point of inter area movement
    S32 traveltype;					//type of travel required to get to the area
    U16 traveltime;	//travel time of the inter area movement
    //
...
// Changes Globals:		-
//===========================================================================
F32 AAS_FaceArea( aas_face_t* face )
{
    S32 i, edgenum, side;
    F32 total;
    vec_t* v;
...
// Changes Globals:		-
//===========================================================================
F32 AAS_AreaVolume( S32 areanum )
{
    S32 i, edgenum, facenum, side;
    vec_t d, a, volume;
    vec3_t corner;
    aas_plane_t* plane;
    aas_edge_t* edge;
...
// Changes Globals:		-
//===========================================================================
S32 AAS_GetJumpPadInfo( S32 ent, vec3_t areastart, vec3_t absmins, vec3_t absmaxs, vec3_t velocity )
{
    S32 modelnum, ent2;
    F32 speed, height, gravity, time, dist, forward;
    vec3_t origin, angles, teststart, ent2origin;
    aas_trace_t trace;
    UTF8 model[MAX_EPAIRKEY];
    UTF8 target[MAX_EPAIRKEY], targetname[MAX_EPAIRKEY];
...
{
    S32 area2num, ent, bot_visualizejumppads, bestareanum;
    F32 volume, bestareavolume;
...
// Changes Globals:		-
//===========================================================================
void AAS_FaceCenter( S32 facenum, vec3_t center )
{
    S32 i;
    F32 scale;
    aas_face_t* face;
...
// Changes Globals:		-
//===========================================================================
S32 AAS_FallDamageDistance( void )
{
    F32 maxzvelocity, gravity, t;
...
// Changes Globals:		-
//===========================================================================
F32 AAS_FallDelta( F32 distance )
{
    F32 t, delta, gravity;
...
// Changes Globals:		-
//===========================================================================
F32 AAS_MaxJumpHeight( F32 phys_jumpvel )
{
    F32 phys_gravity;
...
// Changes Globals:		-
//===========================================================================
F32 AAS_MaxJumpDistance( F32 phys_jumpvel )
{
    F32 phys_gravity, phys_maxvelocity, t;
...
// Changes Globals:		-
//===========================================================================
S32 AAS_AreaTeleporter( S32 areanum )
{
...
// Changes Globals:		-
//===========================================================================
S32 AAS_AreaClusterPortal( S32 areanum )
{
...
// Changes Globals:		-
//===========================================================================
U16 AAS_BarrierJumpTravelTime( void )
{
...
// Changes Globals:		-
//===========================================================================
bool AAS_ReachabilityExists( S32 area1num, S32 area2num )
{
    aas_lreachability_t* r;
...
// Changes Globals:		-
//===========================================================================
S32 AAS_NearbySolidOrGap( vec3_t start, vec3_t end )
{
...
// Changes Globals:		-
//===========================================================================
S32 AAS_Reachability_Swim( S32 area1num, S32 area2num )
{
    S32 i, j, face1num, face2num, side1;
...
// Changes Globals:		-
//===========================================================================
S32 AAS_Reachability_EqualFloorHeight( S32 area1num, S32 area2num )
{
    S32 i, j, edgenum, edgenum1, edgenum2, foundreach, side;
    F32 height, bestheight, length, bestlength;
...
// Changes Globals:		-
//===========================================================================
S32 AAS_Reachability_Step_Barrier_WaterJump_WalkOffLedge( S32 area1num, S32 area2num )
{
    S32 i, j, k, l, edge1num, edge2num, areas[10], numareas;
    S32 ground_bestarea2groundedgenum, ground_foundreach;
    S32 water_bestarea2groundedgenum, water_foundreach;
    S32 side1, area1swim, faceside1, groundface1num;
    F32 dist, dist1, dist2, diff, invgravitydot, ortdot;
    F32 x1, x2, x3, x4, y1, y2, y3, y4, tmp, y;
    F32 length, ground_bestlength, water_bestlength, ground_bestdist, water_bestdist;
            ...
            if( !aassettings.rs_maxfallheight || fabs( ground_bestdist ) < aassettings.rs_maxfallheight )
            {
                ...
                if( !trace.startsolid && trace.fraction >= 1.0 )
                {
                    ...
                    if( AAS_PointAreaNum( trace.endpos ) == area2num )
                    {
                        ...
                        numareas = AAS_TraceAreas( start, end, areas, NULL, sizeof( areas ) / sizeof( S32 ) );
...
// Changes Globals:		-
//===========================================================================
S32 VectorBetweenVectors( vec3_t v, vec3_t v1, vec3_t v2 )
{
    vec3_t dir1, dir2;
...
//===========================================================================
/*
F32 AAS_ClosestEdgePoints(vec3_t v1, vec3_t v2, vec3_t v3, vec3_t v4,
							aas_plane_t *plane1, aas_plane_t *plane2,
							vec3_t beststart, vec3_t bestend, F32 bestdist)
{
	vec3_t dir1, dir2, p1, p2, p3, p4;
	F32 a1, a2, b1, b2, dist;
	S32 founddist;
...
F32 AAS_ClosestEdgePoints( vec3_t v1, vec3_t v2, vec3_t v3, vec3_t v4,
                           ...
                           vec3_t beststart2, vec3_t bestend2, F32 bestdist )
{
    vec3_t dir1, dir2, p1, p2, p3, p4;
    F32 a1, a2, b1, b2, dist, dist1, dist2;
    S32 founddist;
...
// Changes Globals:		-
//===========================================================================
S32 AAS_Reachability_Jump( S32 area1num, S32 area2num )
{
    S32 i, j, k, l, face1num, face2num, edge1num, edge2num, traveltype;
    S32 stopevent, areas[10], numareas;
    F32 phys_jumpvel, maxjumpdistance, maxjumpheight, height, bestdist, speed;
    vec_t* v1, *v2, *v3, *v4;
...
//		Log_Write("shortest distance between %d and %d is %f\r\n", area1num, area2num, bestdist);
        ...
        for( i = 0; i < 3; i++ )
        {
            ...
            teststart[2] += 1;
            numareas = AAS_TraceAreas( move.endpos, teststart, areas, NULL, sizeof( areas ) / sizeof( S32 ) );
...
// Changes Globals:		-
//===========================================================================
S32 AAS_Reachability_Ladder( S32 area1num, S32 area2num )
{
    S32 i, j, k, l, edge1num, edge2num, sharededgenum = 0, lowestedgenum = 0;
    S32 face1num, face2num, ladderface1num = 0, ladderface2num = 0;
    S32 ladderface1vertical, ladderface2vertical, firstv;
    F32 face1area, face2area, bestface1area = -9999, bestface2area = -9999;
    F32 phys_jumpvel, maxjumpheight;
...
// Changes Globals:		-
//===========================================================================
S32 AAS_TravelFlagsForTeam( S32 ent )
{
    S32 notteam;
...
{
    S32 area1num, area2num;
    UTF8 target[MAX_EPAIRKEY], targetname[MAX_EPAIRKEY];
    UTF8 classname[MAX_EPAIRKEY], model[MAX_EPAIRKEY];
    S32 ent, dest;
    F32 angle;
...
{
    S32 area1num, area2num, modelnum, i, j, k, l, n, p;
    F32 lip, height, speed;
    UTF8 model[MAX_EPAIRKEY], classname[MAX_EPAIRKEY];
    S32 ent;
...
// Changes Globals:		-
//===========================================================================
aas_lreachability_t* AAS_FindFaceReachabilities( vec3_t* facepoints, S32 numpoints, aas_plane_t* plane, S32 towardsface )
{
    S32 i, j, k, l;
    S32 facenum, edgenum, bestfacenum;
    F32* v1, *v2, *v3, *v4;
    F32 bestdist, speed, hordist, dist;
...
{
    S32 ent, spawnflags, modelnum, axis;
    S32 i, numareas, areas[10];
    UTF8 classname[MAX_EPAIRKEY], model[MAX_EPAIRKEY];
        ...
        for( i = 0; i < 2; i++ )
        {
            ...
            for( startreach = firststartreach; startreach; startreach = nextstartreach )
            {
                ...
                for( endreach = firstendreach; endreach; endreach = nextendreach )
                {
                    ...
                    if( i == 0 ) lreach->edgenum = ( ( S32 )move_start[axis] << 16 ) | ( ( S32 ) move_end[axis] & 0x0000ffff );
                    else lreach->edgenum = ( ( S32 )move_end[axis] << 16 ) | ( ( S32 ) move_start[axis] & 0x0000ffff );
...
{
    S32 face2num, i, ret, area2num, visualize, ent, bot_visualizejumppads;
    //S32 modelnum, ent2;
    //F32 dist, time, height, gravity, forward;
    F32 speed, zvel, hordist;
    aas_face_t* face2;
    aas_area_t* area2;
    aas_lreachability_t* lreach;
    ...
    //aas_trace_t trace;
    aas_link_t* areas, *link;
    //UTF8 target[MAX_EPAIRKEY], targetname[MAX_EPAIRKEY], model[MAX_EPAIRKEY];
    UTF8 classname[MAX_EPAIRKEY];
...
// Changes Globals:		-
//===========================================================================
S32 AAS_Reachability_Grapple( S32 area1num, S32 area2num )
{
    S32 face2num, i, j, areanum, numareas, areas[20];
    F32 mingrappleangle, z, hordist;
...
void AAS_SetWeaponJumpAreaFlags( void )
{
    ...
    vec3_t origin;
    S32 areanum, weaponjumpareas, spawnflags;
    UTF8 classname[MAX_EPAIRKEY];
...
// Changes Globals:		-
//===========================================================================
S32 AAS_Reachability_WeaponJump( S32 area1num, S32 area2num )
{
    S32 face2num, i, n, ret, visualize;
    F32 speed, zvel, hordist;
    aas_face_t* face2;
    aas_area_t* area1, *area2;
    aas_lreachability_t* lreach;
...
// Changes Globals:		-
//===========================================================================
void AAS_Reachability_WalkOffLedge( S32 areanum )
{
    S32 i, j, k, l, m, n, p, areas[10], numareas;
    S32 face1num, face2num, face3num, edge1num, edge2num, edge3num;
    S32 otherareanum, gap, reachareanum, side;
            ...
            for( j = 0; j < area->numfaces; j++ )
            {
                ...
                for( l = 0; l < face2->numedges; l++ )
                {
                    ...
                    if( abs( edge1num ) == abs( edge2num ) )
                    {
                        ...
                        //if not going through a cluster portal
                        numareas = AAS_TraceAreas( mid, testend, areas, NULL, sizeof( areas ) / sizeof( S32 ) );
...
{
    S32 i, j, todo, start_time;
    static F32 framereachability, reachability_delay;
    static S32 lastpercentage;
    ...
    todo = aasworld.numreachabilityareas + ( S32 ) framereachability;
    ...
    for( i = aasworld.numreachabilityareas; i < aasworld.numareas && i < todo; i++ )
    {
        ...
        if( Sys_MilliSeconds() - start_time > ( S32 ) reachability_delay ) break;
        //
    ...
    {
        botimport.Print( PRT_MESSAGE, "\r%6.1f%%", ( F32 ) 100.0 );
    ...
    else
    {
        ...
        botimport.Print( PRT_MESSAGE, "\r%6.1f%%", ( F32 ) lastpercentage / 10 );
    } //end else
    //not yet finished
    return true;
...
void AAS_InitReachability( void )
{
    ...
    {
#ifndef BSPC
        if( !( ( S32 )LibVarGetValue( "forcereachability" ) ) )
        {
