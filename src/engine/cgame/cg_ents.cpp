...
======================
*/
void idCGameLocal::DrawBoxFace( vec3_t a, vec3_t b, vec3_t c, vec3_t d )
{
    polyVert_t  verts[ 4 ];
...
======================
*/
void idCGameLocal::DrawBoundingBox( vec3_t origin, vec3_t mins, vec3_t maxs )
{
...
======================
*/
void idCGameLocal::PositionEntityOnTag( refEntity_t* entity, const refEntity_t* parent, qhandle_t parentModel, UTF8* tagName )
{
    S32           i;
...
======================
*/
void idCGameLocal::PositionRotatedEntityOnTag( refEntity_t* entity, const refEntity_t* parent, qhandle_t parentModel, UTF8* tagName )
{
    S32           i;
    orientation_t lerped;
    vec3_t        tempAxis[ 3 ];
...
======================
*/
void idCGameLocal::SetEntitySoundPosition( centity_t* cent )
{
...
==================
*/
void idCGameLocal::EntityEffects( centity_t* cent )
{
    S32 i;
    ...
    if( IsTrailSystemValid( &cent->muzzleTS ) )
    {
        ...
        if( cent->currentState.eType == ET_BUILDABLE )
        {
            ...
            AttachmentPoint( &cent->muzzleTS->frontAttachment, front );
            AttachmentPoint( &cent->muzzleTS->backAttachment, back );
            
            if( Distance( front, back ) > ( TESLAGEN_RANGE * M_ROOT3 ) )
                DestroyTrailSystem( &cent->muzzleTS );
        }
        
        if( cg.time > cent->muzzleTSDeathTime && IsTrailSystemValid( &cent->muzzleTS ) )
            DestroyTrailSystem( &cent->muzzleTS );
    }
    ...
    if( cent->currentState.eType == ET_PLAYER )
    {
        ...
        for( i = 0; i <= 2; i++ )
        {
            if( IsTrailSystemValid( &pcent->level2ZapTS[ i ] ) )
            {
                vec3_t  front, back;
                
                AttachmentPoint( &pcent->level2ZapTS[ i ]->frontAttachment, front );
                AttachmentPoint( &pcent->level2ZapTS[ i ]->backAttachment, back );
                
                if( cg.time - pcent->level2ZapTime > 100 || Distance( front, back ) > LEVEL2_AREAZAP_CUTOFF )
                {
                    DestroyTrailSystem( &pcent->level2ZapTS[ i ] );
                }
            }
        }
    }
...
==================
*/
void idCGameLocal::General( centity_t* cent )
{
    refEntity_t     ent;
    entityState_t*   s1;
...
==================
*/
void idCGameLocal::Speaker( centity_t* cent )
{
...
===============
*/
void idCGameLocal::LaunchMissile( centity_t* cent )
{
    entityState_t*       es;
    ...
    {
        ps = SpawnNewParticleSystem( wi->wim[ weaponMode ].missileParticleSystem );
    ...
    {
        ts = SpawnNewTrailSystem( wi->wim[ weaponMode ].missileTrailSystem );
        
        if( IsTrailSystemValid( &ts ) )
        {
            SetAttachmentCent( &ts->frontAttachment, cent );
            AttachToCent( &ts->frontAttachment );
        }
    }
}
...
===============
*/
void idCGameLocal::Missile( centity_t* cent )
{
    refEntity_t             ent;
    entityState_t*           es;
    const weaponInfo_t*      wi;
    ...
    else
    {
        ...
        {
            S32 timeSinceStart = cg.time - es->time;
            
            if( wim->missileAnimLooping )
            {
                ent.frame = wim->missileAnimStartFrame +
                            ( S32 )( ( timeSinceStart / 1000.0f ) * wim->missileAnimFrameRate ) %
...
===============
*/
void idCGameLocal::Mover( centity_t* cent )
{
    refEntity_t     ent;
    entityState_t*   s1;
...
===============
*/
void idCGameLocal::Beam( centity_t* cent )
{
    refEntity_t     ent;
    entityState_t*   s1;
...
===============
*/
void idCGameLocal::Portal( centity_t* cent )
{
    refEntity_t     ent;
    entityState_t*   s1;
...
=========================
*/
void idCGameLocal::LightFlare( centity_t* cent )
{
    refEntity_t   flare;
    entityState_t* es;
    ...
    F32         srcRadius, srLocal, ratio = 1.0f;
    S32           entityNum;
    ...
    Trace( &tr, cg.refdef.vieworg, NULL, NULL, es->angles2, entityNum, MASK_SHOT );
    ...
    if( !( cg.clientFrame % 2 ) )
    {
        ...
        if( maxAngle > 0.0f )
        {
            F32 radiusMod = 1.0f - ( 180.0f - RAD2DEG(
        ...
        {
            //"correct" flares
            BiSphereTrace( &tr, cg.refdef.vieworg, end, 1.0f, srcRadius, entityNum, MASK_SHOT );
        ...
        else if( cg_lightFlare.integer == FLARE_TIMEFADE )
        {
            ...
            Trace( &tr, start, mins, maxs, end, entityNum, MASK_SHOT );
            ...
            if( cent->lfs.status )
            {
                if( cent->lfs.lastTime + es->time > cg.time )
                    ratio = ( F32 )( cg.time - cent->lfs.lastTime ) / es->time;
            }
    ...
    {
        flare.radius *= ratio;
        flare.shaderRGBA[ 3 ] = ( U8 )( ( F32 )flare.shaderRGBA[ 3 ] * ratio );
    }
...
=========================
*/
void idCGameLocal::AdjustPositionForMover( const vec3_t in, S32 moverNum, S32 fromTime, S32 toTime, vec3_t out )
{
    centity_t* cent;
...
=============================
*/
void idCGameLocal::InterpolateEntityPosition( centity_t* cent )
{
    vec3_t    current, next;
    F32     f;
    ...
    if( cg.nextSnap == NULL )
        Error( "idCGameLocal::InterpoateEntityPosition: cg.nextSnap == NULL" );
...
===============
*/
void idCGameLocal::CalcEntityLerpPositions( centity_t* cent )
{
    ...
    if( timeshift )
    {
        ...
        Trace( &tr, lastOrigin, vec3_origin, vec3_origin, cent->lerpOrigin, cent->currentState.number, MASK_SHOT );
...
===============
*/
void idCGameLocal::CEntityPVSEnter( centity_t* cent )
{
...
===============
*/
void idCGameLocal::CEntityPVSLeave( centity_t* cent )
{
    if( cg_debugPVS.integer )
...
===============
*/
void idCGameLocal::AddCEntity( centity_t* cent )
{
...
===============
*/
void idCGameLocal::AddPacketEntities( void )
{
    S32             num;
    centity_t*       cent;
    playerState_t*   ps;
    ...
    if( cg.nextSnap )
    {
        ...
        if( delta == 0 )
            ...
            cg.frameInterpolation = ( F32 )( cg.time - cg.snap->serverTime ) / delta;
    }
    else
    {
...
void idCGameLocal::LinkLocation( centity_t* cent )
{
    centity_t* tempent;
