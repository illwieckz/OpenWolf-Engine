...
=============
*/
void idCGameLocal::UpdateEntityPositions( void )
{
    centity_t* cent = NULL;
    S32       i;
    ...
    for( i = 0; i < cg.snap->numEntities; i++ )
    {
        ...
        {
            S32 team = cent->currentState.misc & 0x00FF;
...
=============
*/
void idCGameLocal::DrawBlips( rectDef_t* rect, vec3_t origin, vec4_t colour )
{
    vec3_t  drawOrigin;
    vec3_t  up = { 0, 0, 1 };
    F32   alphaMod = 1.0f;
    F32   timeFractionSinceRefresh = 1.0f -
                                     ( ( F32 )( cg.time - entityPositions.lastUpdateTime ) /
                                       ( F32 )HUMAN_SCANNER_UPDATE_PERIOD );
    vec4_t  localColour;
    ...
    if( drawOrigin[ 2 ] > 0 )
        DrawPic( rect->x + ( rect->w / 2 ) - ( STALKWIDTH / 2 ) - drawOrigin[ 0 ],
    ...
    DrawPic( rect->x + ( rect->w / 2 ) - ( BLIPX / 2 ) - drawOrigin[ 0 ],
...
=============
*/
void idCGameLocal::DrawDir( rectDef_t* rect, vec3_t origin, vec4_t colour )
{
    vec3_t  drawOrigin;
    vec3_t  noZOrigin;
    ...
    DrawPic( rect->x + ( rect->w / 2 ) - ( BLIPX2 / 2 ) - drawOrigin[ 0 ] * ( rect->w / 2 ), rect->y + ( rect->h / 2 ) - ( BLIPY2 / 2 ) + drawOrigin[ 1 ] * ( rect->h / 2 ), BLIPX2, BLIPY2, cgs.media.scannerBlipShader );
    trap_R_SetColor( NULL );
}
...
=============
*/
void idCGameLocal::AlienSense( rectDef_t* rect )
{
    S32     i;
    vec3_t  origin;
    vec3_t  relOrigin;
...
=============
*/
void idCGameLocal::Scanner( rectDef_t* rect, qhandle_t shader, vec4_t color )
{
    S32     i;
    vec3_t  origin;
    vec3_t  relOrigin;
