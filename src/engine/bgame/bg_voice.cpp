...
============
*/
void idBothGamesLocal::VoiceParseError( fileHandle_t handle, UTF8* err )
{
    S32 line;
...
{
    UTF8 fileList[ MAX_VOICES * ( MAX_VOICE_NAME_LEN + 8 ) ] = {""};
    S32 numFiles, i, fileLen = 0;
    S32 count = 0;
    UTF8* filePtr;
    voice_t* voices = NULL;
    voice_t* top = NULL;
...
============
*/
bool idBothGamesLocal::VoiceParseTrack( S32 handle, voiceTrack_t* voiceTrack )
{
    pc_token_t token;
    bool found = false;
    ...
    while( foundToken )
    {
        ...
        else if( !Q_stricmp( token.string, "text" ) )
        {
            ...
            voiceTrack->text = ( UTF8* )Alloc( strlen( token.string ) + 1 );
...
============
*/
voiceTrack_t* idBothGamesLocal::VoiceParseCommand( S32 handle )
{
    pc_token_t token;
...
============
*/
voiceCmd_t* idBothGamesLocal::VoiceParse( UTF8* name )
{
...
============
*/
void idBothGamesLocal::PrintVoices( voice_t* voices, S32 debugLevel )
{
    voice_t* voice = voices;
    voiceCmd_t* voiceCmd;
    voiceTrack_t* voiceTrack;
    
    S32 cmdCount, trackCount;
...
============
*/
voice_t* idBothGamesLocal::VoiceByName( voice_t* head, UTF8* name )
{
    voice_t* v = head;
...
============
*/
voiceCmd_t* idBothGamesLocal::VoiceCmdFind( voiceCmd_t* head, UTF8* name, S32* cmdNum )
{
    voiceCmd_t* vc = head;
    S32 i = 0;
...
============
*/
voiceCmd_t* idBothGamesLocal::VoiceCmdByNum( voiceCmd_t* head, S32 num )
{
    voiceCmd_t* vc = head;
    S32 i = 0;
...
============
*/
voiceTrack_t* idBothGamesLocal::VoiceTrackByNum( voiceTrack_t* head, S32 num )
{
...
============
*/
voiceTrack_t* idBothGamesLocal::VoiceTrackFind( voiceTrack_t* head, team_t team, class_t _class, weapon_t weapon, S32 enthusiasm, S32* trackNum )
{
    voiceTrack_t* vt = head;
    S32 highestMatch = 0;
    S32 matchCount = 0;
