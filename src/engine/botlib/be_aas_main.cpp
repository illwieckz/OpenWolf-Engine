...
// Changes Globals:		-
//===========================================================================
UTF8* AAS_StringFromIndex( UTF8* indexname, UTF8* stringindex[], S32 numindexes, S32 index )
{
...
// Changes Globals:		-
//===========================================================================
S32 AAS_IndexFromString( UTF8* indexname, UTF8* stringindex[], S32 numindexes, UTF8* string )
{
    S32 i;
    if( !aasworld.indexessetup )
    {
...
// Changes Globals:		-
//===========================================================================
void AAS_UpdateStringIndexes( S32 numconfigstrings, UTF8* configstrings[] )
{
    S32 i;
    ...
    for( i = 0; i < numconfigstrings; i++ )
    {
        if( configstrings[i] )
        {
            ...
            aasworld.configstrings[i] = ( UTF8* ) GetMemory( strlen( configstrings[i] ) + 1 );
...
// Changes Globals:		-
//===========================================================================
void AAS_ContinueInit( F32 time )
{
    ...
    if( aasworld.savefile || ( ( S32 )LibVarGetValue( "forcewrite" ) ) )
    {
        //optimize the AAS data
        if( ( S32 )LibVarValue( "bot_aasoptimize", "1" ) ) AAS_Optimize();
        //save the AAS file
...
//	vec3_t origin;
    ...
    for( revlink = revreach->first; revlink; revlink = revlink->next )
    {
        ...
        AAS_ShowReachability( reach );
        AAS_ShowArea( revlink->areanum, true );
...
S32 AAS_StartFrame( F32 time )
{
    ...
    {
        AAS_WriteRouteCache();
        LibVarSet( "bot_saveroutingcache", "0" );
    } //end if
    //
    aasworld.numframes++;
...
// Changes Globals:		-
//===========================================================================
S32 AAS_LoadFiles( StringEntry mapname )
{
    S32 errnum;
    UTF8 aasfile[MAX_PATH];
//	UTF8 bspfile[MAX_PATH];
...
S32 AAS_Setup( void )
{
    aasworld.maxclients = ( S32 ) LibVarValue( "maxclients", "128" );
    aasworld.maxentities = ( S32 ) LibVarValue( "maxentities", "1024" );
    // as soon as it's set to 1 the routing cache will be saved
    saveroutingcache = LibVar( "bot_saveroutingcache", "0" );
