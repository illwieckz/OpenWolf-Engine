...
//-----------------------------------------------------------------------------
S64 RllDecodeMonoToMono( U8* from, S16* to, U32 size, char signedOutput , U16 flag )
{
    U32 z;
    S32 prev;
...
// Returns:     Number of samples placed in output buffer
//-----------------------------------------------------------------------------
S64 RllDecodeMonoToStereo( U8* from, S16* to, U32 size, char signedOutput, U16 flag )
{
    U32 z;
    S32 prev;
...
// Returns:     Number of samples placed in output buffer
//-----------------------------------------------------------------------------
S64 RllDecodeStereoToStereo( U8* from, S16* to, U32 size, char signedOutput, U16 flag )
{
    U32 z;
    U8* zz = from;
    S32	prevL, prevR;
...
// Returns:     Number of samples placed in output buffer
//-----------------------------------------------------------------------------
S64 RllDecodeStereoToMono( U8* from, S16* to, U32 size, char signedOutput, U16 flag )
{
    U32 z;
    S32 prevL, prevR;
...
*/
void Frame_yuv_to_rgb24( const U8* y, const U8* u, const U8* v,
                         S32 width, S32 height, S32 y_stride, S32 uv_stride,
                         S32 yWShift, S32 uvWShift, S32 yHShift, S32 uvHShift, U32* output )
{
    S32             i, j, uvI;
    S64            r, g, b, YY;
    ...
    for( j = 0; j < height; ++j )
    {
        for( i = 0; i < width; ++i )
        {
            ...
            YY = ( S64 )( ROQ_YY_tab[( y[( i >> yWShift ) + ( j >> yHShift ) * y_stride] )] );
    ...
    switch( cinTable[currentHandle].roq_id )
    {
        ...
        case	ZA_SOUND_MONO:
            if( !cinTable[currentHandle].silent )
            {
                ...
                soundSystem->RawSamples( ssize, 22050, 2, 1, ( U8* )sbuf, 1.0f, 1.0f, CIN_STREAM );
                //soundSystem->RawSamples( 0, ssize, 22050, 2, 1, ( U8* )sbuf, 1.0f, 1.0f );
            }
            break;
        case	ZA_SOUND_STEREO:
            if( !cinTable[currentHandle].silent )
            {
                ...
                //soundSystem->RawSamples( 0, ssize, 22050, 2, 2, ( U8* )sbuf, 1.0f, 1.0f );
                soundSystem->RawSamples( ssize, 22050, 2, 2, ( U8* )sbuf, 1.0f, 1.0f, CIN_STREAM );
            }
            break;
        case	ROQ_QUAD_INFO:
            if( cinTable[currentHandle].numQuads == -1 )
...
=================
*/
static char* S_FileExtension( StringEntry fni )
{
