...
// cmd history
static UTF8 qconsole_history[ QCONSOLE_HISTORY ][ MAX_EDIT_LINE ];
static S32 qconsole_history_pos = -1;
static S32 qconsole_history_oldest = 0;

// current edit buffer
static UTF8 qconsole_line[ MAX_EDIT_LINE ];
static S32 qconsole_linelen = 0;
...
UTF8* CON_Input( void )
{
    ...
    for( i = 0; i < count; i++ )
    {
        ...
        {
            UTF8 c = buff[ i ].Event.KeyEvent.uChar.AsciiChar;
            
            if( key == VK_BACK )
            {
                S32 pos = ( qconsole_linelen > 0 ) ?
...
==================
*/
void CON_Print( StringEntry msg )
{
    fputs( msg, stderr );
