...
// get pixel size
    int getHeight()
    {
        return m_pWnd->allocation.height;
    }
    int getWidth()
    {
        return m_pWnd->allocation.width;
    }
// set pixel size and other parameters before showing it
    void setSizeParm( int width, int height )
    {
        m_nWidthParam = width;
        m_nHeightParam = height;
    }
// set the IWindowListener (implemented by the plugin using this window)
    void setListener( IWindowListener* pListen )
    {
        m_pListen = pListen;
        m_pListen->IncRef();
    }
