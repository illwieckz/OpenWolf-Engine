...
cvar_t*         Cvar_Set2( StringEntry var_name, StringEntry value, bool force );
...
============
*/
static bool Cvar_ValidateString( StringEntry s )
{
    if( !s )
    {
        return false;
...
============
*/
static cvar_t*  Cvar_FindVar( StringEntry var_name )
{
    cvar_t*         var;
    S64            hash;
...
============
*/
UTF8*           Cvar_ClearForeignCharacters( StringEntry value )
{
...
============
*/
cvar_t*         Cvar_Get( StringEntry var_name, StringEntry var_value, S32 flags )
{
    cvar_t*         var;
    S64            hash;
    ...
    if( var )
    {
        ...
        {
            UTF8*           cleaned = Cvar_ClearForeignCharacters( var->string );	// NOTE: it is probably harmless to call Cvar_Set2 in all cases, but I don't want to risk it
...
cvar_t*         Cvar_Set2( StringEntry var_name, StringEntry value, bool force )
{
    ...
    {
        UTF8*           cleaned = Cvar_ClearForeignCharacters( value );
...
void Cvar_Toggle_f( void )
{
    ...
    if( c == 2 )
    {
        Cvar_Set2( varname, va( "%d", !Cvar_VariableValue( varname ) ), false );
        return;
    }
    ...
    for( i = 2; i < c - 1; ++i )
    {
        ...
        {
            Cvar_Set2( varname, Cmd_Argv( i + 1 ), false );
            return;
        }
    }
    
    // fallback
    Cvar_Set2( varname, Cmd_Argv( 2 ), false );
}
...
void Cvar_Set_f( void )
{
    ...
    Cvar_Set2( Cmd_Argv( 1 ), Com_UnquoteStr( value ), false );
    free( value );
}
...
=====================
*/
void Cvar_CheckRange( cvar_t* var, F32 min, F32 max, bool integral )
{
    var->validate = true;
    var->min = min;
    var->max = max;
...
void Cvar_Update( vmCvar_t* vmCvar )
{
    ...
    {
        Com_Error( ERR_DROP, "Cvar_Update: handle %d out of range", ( U32 )vmCvar->handle );
    }
