...
=================
*/
void idCGameLocal::RegisterUpgrade( S32 upgradeNum )
{
...
/*
===============
idCGameLocal::InitUpgrades

Precaches upgrades
===============
*/
void idCGameLocal::InitUpgrades( void )
{
    S32   i;
...
======================
*/
bool idCGameLocal::ParseWeaponAnimationFile( StringEntry filename, weaponInfo_t* weapon )
{
    UTF8*          text_p;
    S32           len;
    S32           i;
...
===============
*/
bool idCGameLocal::ParseWeaponModeSection( weaponInfoMode_t* wim, UTF8** text_p )
{
    UTF8*  token;
    S32   i;
    ...
    while( 1 )
    {
        ...
        else if( !Q_stricmp( token, "missileParticleSystem" ) )
        {
            ...
            wim->missileParticleSystem = RegisterParticleSystem( token );
        ...
        else if( !Q_stricmp( token, "missileTrailSystem" ) )
        {
            ...
            wim->missileTrailSystem = RegisterTrailSystem( token );
        ...
        else if( !Q_stricmp( token, "muzzleParticleSystem" ) )
        {
            ...
            wim->muzzleParticleSystem = RegisterParticleSystem( token );
        ...
        else if( !Q_stricmp( token, "impactParticleSystem" ) )
        {
            ...
            wim->impactParticleSystem = RegisterParticleSystem( token );
...
======================
*/
bool idCGameLocal::ParseWeaponFile( StringEntry filename, weaponInfo_t* wi )
{
    UTF8*          text_p;
    S32           len;
    UTF8*          token;
...
=================
*/
void idCGameLocal::RegisterWeapon( S32 weaponNum )
{
...
===============
*/
void idCGameLocal::InitWeapons( void )
{
    S32   i;
    ...
    cgs.media.level2ZapTS = RegisterTrailSystem( "models/weapons/lev2zap/lightning" );
    cgs.media.massDriverTS = RegisterTrailSystem( "models/weapons/mdriver/fireTS" );
}
...
===============
*/
void idCGameLocal::SetWeaponLerpFrameAnimation( weapon_t weapon, lerpFrame_t* lf, S32 newAnimation )
{
    animation_t* anim;
...
===============
*/
void idCGameLocal::WeaponAnimation( centity_t* cent, S32* old, S32* now, F32* backLerp )
{
...
=================
*/
S32 idCGameLocal::MapTorsoToWeaponFrame( clientInfo_t* ci, S32 frame )
{
    // change weapon
...
==============
*/
void idCGameLocal::CalculateWeaponPosition( vec3_t origin, vec3_t angles )
{
    F32 scale, fracsin, bob;
    S32 delta;
    weaponInfo_t* weapon;
...
F32 idCGameLocal::MachinegunSpinAngle( centity_t* cent, bool firing )
{
    S32 delta;
    F32 angle, speed;
    ...
    else
    {
        ...
        speed = 0.5 * ( SPIN_SPEED + ( F32 )( COAST_TIME - delta ) / COAST_TIME );
...
=============
*/
void idCGameLocal::AddPlayerWeapon( refEntity_t* parent, playerState_t* ps, centity_t* cent )
{
    ...
    if( !noGunModel )
    {
        PositionEntityOnTag( &gun, parent, parent->hModel, "tag_weapon" );
        ...
        if( barrel.hModel )
        {
            ...
            PositionRotatedEntityOnTag( &barrel, &gun, gun.hModel, "tag_barrel" );
    ...
    if( IsParticleSystemValid( &cent->muzzlePS ) )
    {
        ...
        {
            if( noGunModel )
                SetAttachmentTag( &cent->muzzlePS->attachment, *parent, parent->hModel, "tag_weapon" );
            else
                SetAttachmentTag( &cent->muzzlePS->attachment, gun, gun.hModel, "tag_flash" );
        }
        
        //if the PS is infinite disable it when not firing
        if( !firing && IsParticleSystemInfinite( cent->muzzlePS ) )
    ...
    if( flash.hModel )
    {
        ...
        if( noGunModel )
            PositionRotatedEntityOnTag( &flash, parent, parent->hModel, "tag_weapon" );
        else
            PositionRotatedEntityOnTag( &flash, &gun, gun.hModel, "tag_flash" );
            ...
            cent->currentState.number != cg.predictedPlayerState.clientNum )
    {
        ...
        {
            cent->muzzlePS = SpawnNewParticleSystem( weapon->wim[ weaponMode ].muzzleParticleSystem );
            ...
            if( IsParticleSystemValid( &cent->muzzlePS ) )
            {
                ...
                SetAttachmentCent( &cent->muzzlePS->attachment, cent );
                AttachToTag( &cent->muzzlePS->attachment );
            }
...
void idCGameLocal::AddViewWeapon( playerState_t* ps )
{
    refEntity_t   hand;
    centity_t*     cent;
    clientInfo_t*  ci;
    ...
    if( ( ps->stats[ STAT_BUILDABLE ] & ~SB_VALID_TOGGLEBIT ) > BA_NONE )
        GhostBuildable( ( buildable_t )( ps->stats[ STAT_BUILDABLE ] & ~SB_VALID_TOGGLEBIT ) );
    ...
    if( !cg_drawGun.integer )
    {
        ...
        if( cent->muzzlePS )
            SetAttachmentPoint( &cent->muzzlePS->attachment, origin );
            
        //check for particle systems
        if( wi->wim[ weaponMode ].muzzleParticleSystem && cent->muzzlePsTrigger )
        {
            cent->muzzlePS = SpawnNewParticleSystem( wi->wim[ weaponMode ].muzzleParticleSystem );
            
            if( IsParticleSystemValid( &cent->muzzlePS ) )
            {
                SetAttachmentPoint( &cent->muzzlePS->attachment, origin );
                SetAttachmentCent( &cent->muzzlePS->attachment, cent );
                AttachToPoint( &cent->muzzlePS->attachment );
            }
            cent->muzzlePsTrigger = false;
        }
    ...
    if( weapon == WP_LUCIFER_CANNON && ps->stats[ STAT_MISC ] > 0 )
    {
        ...
        fraction = ( F32 )ps->stats[ STAT_MISC ] / LCANNON_CHARGE_TIME_MAX;
...
===============
*/
bool idCGameLocal::WeaponSelectable( weapon_t weapon )
{
...
===============
*/
bool idCGameLocal::UpgradeSelectable( upgrade_t upgrade )
{
...
===================
*/
void idCGameLocal::DrawItemSelect( rectDef_t* rect, vec4_t color )
{
    S32           i;
    F32         x = rect->x;
    F32         y = rect->y;
    ...
    for( i = 0; i < length; i++ )
    {
        S32 item = i - length / 2 + selectedItem;
        ...
        if( item >= 0 && item < numItems )
        {
            ...
            if( items[ item ] <= 32 )
                DrawPic( x, y, iconWidth, iconHeight, cg_weapons[ items[ item ] ].weaponIcon );
            else if( items[ item ] > 32 )
                DrawPic( x, y, iconWidth, iconHeight, cg_upgrades[ items[ item ] - 32 ].upgradeIcon );
        }
...
===================
*/
void idCGameLocal::DrawItemSelectText( rectDef_t* rect, F32 scale, S32 textStyle )
{
    S32 x, w;
    UTF8*  name;
    F32* color;
    
    color = FadeColor( cg.weaponSelectTime, WEAPON_SELECT_TIME );
    if( !color )
        return;
...
===============
*/
void idCGameLocal::NextWeapon_f( void )
{
    S32 i, original;
...
===============
*/
void idCGameLocal::PrevWeapon_f( void )
{
    S32 i, original;
...
===============
*/
void idCGameLocal::Weapon_f( void )
{
    S32   num;
...
================
*/
void idCGameLocal::FireWeapon( centity_t* cent, weaponMode_t weaponMode )
{
    entityState_t*     es;
    S32               c;
    weaponInfo_t*      wi;
    ...
    {
        if( !IsParticleSystemValid( &cent->muzzlePS ) || !IsParticleSystemInfinite( cent->muzzlePS ) )
        {
...
=================
*/
void idCGameLocal::HandleAlienFeedback( centity_t* cent, alienFeedback_t feedbackType )
{
    entityState_t*     es;
...
=================
*/
void idCGameLocal::MissileHitWall( weapon_t weaponNum, weaponMode_t weaponMode, S32 clientNum, vec3_t origin, vec3_t dir, impactSound_t soundType, S32 charge )
{
    ...
    if( ps )
    {
        particleSystem_t* partSystem = SpawnNewParticleSystem( ps );
        
        if( IsParticleSystemValid( &partSystem ) )
        {
            SetAttachmentPoint( &partSystem->attachment, origin );
            SetParticleSystemNormal( partSystem, dir );
            AttachToPoint( &partSystem->attachment );
    ...
    // impact mark
    if( radius > 0.0f )
        ImpactMark( mark, origin, dir, random( ) * 360, 1, 1, 1, 1, false, radius, false );
}
...
=================
*/
void idCGameLocal::MissileHitPlayer( weapon_t weaponNum, weaponMode_t weaponMode, vec3_t origin, vec3_t dir, S32 entityNum, S32 charge )
{
    vec3_t normal;
    weaponInfo_t* weapon = &cg_weapons[ weaponNum ];
...
void idCGameLocal::MassDriverFire( entityState_t* es )
{
    vec3_t front, frontToBack;
    trailSystem_t* ts;
    F32 length;
    
    ts = SpawnNewTrailSystem( cgs.media.massDriverTS );
    ...
    SetAttachmentPoint( &ts->frontAttachment, front );
    AttachToPoint( &ts->frontAttachment );
    
    // trail back attaches to the impact point
    SetAttachmentPoint( &ts->backAttachment, es->pos.trBase );
    AttachToPoint( &ts->backAttachment );
}
...
===============
*/
void idCGameLocal::Tracer( vec3_t source, vec3_t dest )
{
...
======================
*/
bool idCGameLocal::CalcMuzzlePoint( S32 entityNum, vec3_t muzzle )
{
    vec3_t forward;
    centity_t* cent;
    S32 anim;
...
======================
*/
void idCGameLocal::Bullet( vec3_t end, S32 sourceEntityNum, vec3_t normal, bool flesh, S32 fleshEntityNum )
{
    vec3_t  start;
    ...
    if( flesh )
        Bleed( end, normal, fleshEntityNum );
    else
...
================
*/
void idCGameLocal::ShotgunPattern( vec3_t origin, vec3_t origin2, S32 seed, S32 otherEntNum )
{
    S32       i;
    F32     r, u;
    vec3_t    end;
    ...
    for( i = 0; i < SHOTGUN_PELLETS; i++ )
    {
        ...
        Trace( &tr, origin, NULL, NULL, end, otherEntNum, MASK_SHOT );
...
==============
*/
void idCGameLocal::ShotgunFire( entityState_t* es )
{
    vec3_t  v;
