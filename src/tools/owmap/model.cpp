    ...
    for( s = 0; s < numSurfaces; s++ )
    {
        ...
        for( i = 0; i < ds->numVerts; i++ )
        {
            ...
            for( j = 0; j < MAX_LIGHTMAPS; j++ )
            {
                ...
                {
                    dv->lightColor[j][0] = 255.0f;
                    dv->lightColor[j][1] = 255.0f;
                    dv->lightColor[j][2] = 255.0f;
                    dv->lightColor[j][3] = color[0] * 0.3f + color[1] * 0.59f + color[2] * 0.11f;
                }
                else
                {
                    dv->lightColor[j][0] = color[0];
                    dv->lightColor[j][1] = color[1];
                    dv->lightColor[j][2] = color[2];
                    dv->lightColor[j][3] = color[3];
                }
            }
        }
