...
static void CL_RazorHydra( usercmd_t* cmd )
{
    ...
    if( HydraManagerLocal.Peak( 0, &joyx, &joyy, &pos, &trigger, &yaw, &pitch, &roll, &buttons ) != 0 )
    {
        static U32 lockbtn = 0;
        static F32 accjoyx = 0.0f;
        accjoyx -= ( joyx );
    ...
    if( HydraManagerLocal.Peak( 1, &joyx, &joyy, &pos, &trigger, &yaw, &pitch, &roll, &buttons ) != 0 )
    {
        static U32 lockbtn = 0;
        if( ( ( buttons & SIXENSE_BUTTON_BUMPER ) != 0 ) && ( ( lockbtn & SIXENSE_BUTTON_BUMPER ) == 0 ) )
        {
        ...
        cmd->forwardmove = ClampChar( cmd->forwardmove + ( S32 )( joyy * movespeed ) );
        cmd->rightmove = ClampChar( cmd->rightmove + ( S32 )( joyx * movespeed ) );
        
        /*if (roll > 0.2 || roll < -0.2)
        cmd->rightmove   = ClampChar(cmd->rightmove   + (S32)(-roll * movespeed));*/
        lockbtn = buttons;
    }
