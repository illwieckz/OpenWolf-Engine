...
void* idCGameLocal::Alloc( S32 size )
{
    ...
    for( fmn = freehead; fmn; fmn = fmn->next )
    {
        if( fmn->cookie != FREEMEMCOOKIE )
            Error( "CG_Alloc: Memory corruption detected!\n" );
    ...
    if( ptr )
    {
        ...
        if( cg_debugAlloc.integer )
            Printf( "CG_Alloc of %i bytes (%i left)\n", allocsize, freemem );
        ::memset( ptr, 0, allocsize );
        *ptr++ = allocsize;       // Store a copy of size for deallocation
        return( ( void* ) ptr );
    }
    
    Error( "CG_Alloc: failed on allocation of %i bytes\n", size );
    return( NULL );
}

void idCGameLocal::Free( void* ptr )
{
    ...
    freemem += *freeptr;
    if( cg_debugAlloc.integer )
        Printf( "CG_Free of %i bytes (%i left)\n", *freeptr, freemem );
...
void idCGameLocal::InitMemory( void )
{
...
void idCGameLocal::DefragmentMemory( void )
{
    ...
    for( startfmn = freehead; startfmn; )
    {
        ...
        {
            if( fmn->cookie != FREEMEMCOOKIE )
                Error( "CG_DefragmentMemory: Memory corruption detected!\n" );
