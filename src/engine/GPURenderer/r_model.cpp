...
//  ::memcpy(mod->md3[lod], buffer, LittleLong(md3Model->ofsEnd));
    ...
    for( i = 0; i < md3Model->numSurfaces; i++ )
    {
        ...
        surf->shaderIndexes = shaderIndex = ( S32* )Hunk_Alloc( sizeof( *shaderIndex ) * md3Surf->numShaders, h_low );
        ...
        surf->indexes = tri = ( U32* )Hunk_Alloc( sizeof( *tri ) * 3 * md3Surf->numTriangles, h_low );
    ...
    
    {
        ...
        mdvModel->vaoSurfaces = ( srfVaoMdvMesh_t* )Hunk_Alloc( sizeof( *mdvModel->vaoSurfaces ) * mdvModel->numSurfaces, h_low );
...
static bool R_LoadMDR( model_t* mod, void* buffer, S32 filesize, StringEntry mod_name )
{
    ...
    mod->dataSize += size;
    mod->modelData = mdr = ( mdrHeader_t* )Hunk_Alloc( size, h_low );
