...
S32 g_numArmourRegions[ UP_NUM_UPGRADES ];

/*
============
idGameLocal::AddScore

Adds score to the client
============
*/
void idGameLocal::AddScore( gentity_t* ent, S32 score )
{
    if( !ent->client )
        return;
    ...
    {
        score = rint( ( F64 ) score / 2.0 );
    }
    
    // scale values down to fit the scoreboard better
    score = rint( ( F64 ) score / 10.0 );
...
==================
*/
void idGameLocal::LookAtKiller( gentity_t* self, gentity_t* inflictor, gentity_t* attacker )
{
...
/*
==================
idGameLocal::RewardAttackers

Function to distribute rewards to entities that killed this one.
Returns the total damage dealt.
==================
*/
F32 idGameLocal::RewardAttackers( gentity_t* self )
{
    F32 value, totalDamage = 0;
    S32 team, i, maxHealth = 0;
    
    // Total up all the damage done by every client
    for( i = 0; i < MAX_CLIENTS; i++ )
        totalDamage += ( F32 )self->credits[ i ];
    if( totalDamage <= 0.0f )
        return 0.f;
    ...
    else if( self->s.eType == ET_BUILDABLE )
    {
        ...
        if( !self->spawned )
            value *= ( F32 )( level.time - self->buildTime ) /
    ...
    for( i = 0; i < MAX_CLIENTS; i++ )
    {
        ...
        S16 num = value * self->credits[ i ] / totalDamage;
        S32 stageValue = num;
        ...
        {
            AddCreditToClient( player->client, num, true );
...
/*
==================
idGameLocal::player_die
==================
*/
void idGameLocal::player_die( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, S32 damage, S32 meansOfDeath )
{
    gentity_t* ent;
    S32       anim;
    S32       killer;
    S32       i, idk;
    UTF8*      killerName, *obit;
    ...
    if( attacker )
    {
        ...
        if( attacker->client )
        {
            idk = attacker->client->pers.mysqlid; //Used to insert in kills mysql table
    ...
    LogPrintf( "Kill: %i %i %i: %s^7 killed %s^7 by %s\n", killer, self->s.number, meansOfDeath, killerName, self->client->pers.netname, obit );
    
    if( trap_Database_RunQuery( va( "INSERT INTO kills (idkiller,idtarget,idhow) VALUES (%d,%d,%d)", idk, self->client->pers.mysqlid, meansOfDeath ) ) == true )
    {
        UTF8 data[255];
        trap_Database_FinishQuery();
        trap_Database_RunQuery( "SELECT id FROM kills ORDER BY id DESC LIMIT 1" );
        trap_Database_FetchRow();
        trap_Database_FetchFieldByName( "id", data, sizeof( data ) );
        trap_Database_RunQuery( va( "INSERT into mainlog (idaction,typeaction,date) VALUES (%d,\"%d\",NOW())", atoi( data ), 8 ) );
        trap_Database_FinishQuery();
    }
    
    // close any menus the client has open
    CloseMenus( self->client->ps.clientNum );
    ...
    ent = TempEntity( self->r.currentOrigin, EV_OBITUARY );
    ...
    if( attacker && attacker->client )
    {
        ...
        if( attacker == self || OnSameTeam( self, attacker ) )
        {
            ...
            {
                AddCreditToClient( attacker->client, -ALIEN_TK_SUICIDE_PENALTY, true );
                AddScore( attacker, -ALIEN_TK_SUICIDE_PENALTY );
            }
            else if( attacker->client->ps.stats[ STAT_TEAM ] == TEAM_HUMANS )
            {
                AddCreditToClient( attacker->client, -HUMAN_TK_SUICIDE_PENALTY, true );
    ...
    
    {
        ...
        AddEvent( self, EV_DEATH1 + i, ( killer < MAX_CLIENTS ) ? killer : self - g_entities );
...
/*
===============
idGameLocal::ParseDmgScript
===============
*/
S32 idGameLocal::ParseDmgScript( damageRegion_t* regions, UTF8* buf )
{
    UTF8*  token;
    F32 angleSpan, heightSpan;
    S32   count;
    ...
    for( count = 0; ; count++ )
    {
        ...
        if( strcmp( token, "{" ) )
        {
            Printf( "Missing { in damage region file\n" );
            break;
        }
        
        if( count >= MAX_DAMAGE_REGIONS )
        {
            Printf( "Max damage regions exceeded in damage region file\n" );
            break;
        }
        ...
        while( 1 )
        {
            ...
            if( !token[0] )
            {
                Printf( "Unexpected end of damage region file\n" );
                break;
            }
...
============
*/
F32 idGameLocal::GetRegionDamageModifier( gentity_t* targ, S32 _class, S32 piece )
{
    damageRegion_t* regions, *overlap;
    F32 modifier = 0.f, areaSum = 0.f;
    S32 j, i;
    bool crouch;
    ...
    if( g_debugDamage.integer > 2 )
        Printf( "GetRegionDamageModifier():\n"
    ...
    for( j = UP_NONE + 1; j < UP_NUM_UPGRADES; j++ )
    {
        ...
        {
            F32 overlapMaxA, regionMinA, regionMaxA, angleSpan, heightSpan, area;
...
============
*/
F32 idGameLocal::GetNonLocDamageModifier( gentity_t* targ, S32 _class )
{
    F32 modifier = 0., area = 0.f, scale = 0.f;
    S32 i;
    bool crouch;
    ...
    if( g_debugDamage.integer > 1 )
        Printf( "GetNonLocDamageModifier() modifier:%f, area:%f, scale:%f\n", modifier, area, scale );
...
============
*/
F32 idGameLocal::GetPointDamageModifier( gentity_t* targ, damageRegion_t* regions, S32 len, F32 angle, F32 height )
{
    F32 modifier = 1.f;
    S32 i;
    ...
    if( g_debugDamage.integer )
        Printf( "GetDamageRegionModifier(angle = %f, height = %f): %f\n", angle, height, modifier );
...
============
*/
F32 idGameLocal::CalcDamageModifier( vec3_t point, gentity_t* targ, gentity_t* attacker, S32 _class, S32 dflags )
{
    vec3_t  targOrigin, bulletPath, bulletAngle, pMINUSfloor, floor, normal;
    F32   clientHeight, hitRelative, hitRatio, modifier;
    S32     hitRotation, i;
...
/*
============
idGameLocal::InitDamageLocations
============
*/
void idGameLocal::InitDamageLocations( void )
{
    UTF8*          modelName;
    ...
    UTF8          buffer[ MAX_DAMAGE_REGION_TEXT ];
    ...
    for( i = PCL_NONE + 1; i < PCL_NUM_CLASSES; i++ )
    {
        ...
        {
            Printf( S_COLOR_RED "file too large: %s is %i, max allowed is %i", filename, len, MAX_DAMAGE_REGION_TEXT );
...
void idGameLocal::Damage( gentity_t* targ, gentity_t* inflictor, gentity_t* attacker, vec3_t dir, vec3_t point, S32 damage, S32 dflags, S32 mod )
{
    gclient_t* client;
    S32     take;
    S32     save;
    ...
    if( targ->s.eType == ET_MOVER )
    {
        ...
        if( attacker->client->pers.teamSelection == TEAM_ALIENS )
        {
            if( mod == MOD_LEVEL3_BOUNCEBALL ||
                    ...
                    mod == MOD_LEVEL2_ZAP )
                AddEvent( attacker, EV_ALIENRANGED_HIT, targ->s.number );
            else
                AddEvent( attacker, EV_ALIEN_HIT, targ->s.number );
        }
        return;
    }
    ...
    if( knockback && targ->client )
    {
        ...
        VectorScale( dir, g_knockback.value * ( F32 )knockback / mass, kvel );
    ...
    if( !( dflags & DAMAGE_NO_PROTECTION ) )
    {
            ...
            else if( !g_friendlyFire.integer )
            {
                        ...
                        targ->client->ps.stats[ STAT_TEAM ] == TEAM_ALIENS )
                {
                    if( mod == MOD_LEVEL3_BOUNCEBALL ||
                            ...
                            mod == MOD_LEVEL1_PCLOUD     ||
                            mod == MOD_LEVEL2_ZAP )
                        AddEvent( attacker, EV_ALIENRANGED_TEAMHIT, targ->s.number );
                    else
                        AddEvent( attacker, EV_ALIEN_TEAMHIT, targ->s.number );
                    return;
                }
                else
                {
                    if( mod == MOD_LEVEL3_BOUNCEBALL ||
                            ...
                            mod == MOD_LEVEL1_PCLOUD     ||
                            mod == MOD_LEVEL2_ZAP )
                        AddEvent( attacker, EV_ALIENRANGED_MISS, targ->s.number );
                    else
                        AddEvent( attacker, EV_ALIEN_MISS, targ->s.number );
                }
            }
        }
        ...
        if( targ->s.eType == ET_BUILDABLE && attacker->client )
        {
            ...
            if( targ->buildableTeam == TEAM_HUMANS && FindDCC( targ ) && level.time > level.humanBaseAttackTimer && mod != MOD_DECONSTRUCT && mod != MOD_SUICIDE )
            {
                level.humanBaseAttackTimer = level.time + DC_ATTACK_PERIOD;
                BroadcastEvent( EV_DCC_ATTACK, 0 );
            }
        }
    ...
    if( client )
    {
        ...
        take = ( S32 )( take * CalcDamageModifier( point, targ, attacker, client->ps.stats[ STAT_CLASS ], dflags ) + 0.5f );
...
/*
============
idGameLocal::CanDamage

Returns true if the inflictor can directly damage the target.  Used for
explosions and melee attacks.
============
*/
bool idGameLocal::CanDamage( gentity_t* targ, vec3_t origin )
{
    vec3_t  dest;
    trace_t tr;
    vec3_t  midpoint;
...
/*
============
idGameLocal::SelectiveRadiusDamage
============
*/
bool idGameLocal::SelectiveRadiusDamage( vec3_t origin, gentity_t* attacker, F32 damage, F32 radius, gentity_t* ignore, S32 mod, S32 team )
{
    F32     points, dist;
    gentity_t* ent;
    S32       entityList[ MAX_GENTITIES ];
    S32       numListedEntities;
    vec3_t    mins, maxs;
    vec3_t    v;
    vec3_t    dir;
    S32       i, e;
    ...
    for( e = 0; e < numListedEntities; e++ )
    {
        ...
        if( CanDamage( ent, origin ) && ent->client && ent->client->ps.stats[ STAT_TEAM ] != team )
        {
            ...
            dir[ 2 ] += 24;
            hitClient = true;
            Damage( ent, NULL, attacker, dir, origin, ( S32 )points, DAMAGE_RADIUS | DAMAGE_NO_LOCDAMAGE, mod );
        }
    }
...
============
*/
bool idGameLocal::RadiusDamage( vec3_t origin, gentity_t* attacker, F32 damage, F32 radius, gentity_t* ignore, S32 mod )
{
    F32     points, dist;
    gentity_t* ent;
    S32       entityList[ MAX_GENTITIES ];
...
/*
================
idGameLocal::LogDestruction

Log deconstruct/destroy events
================
*/
void idGameLocal::LogDestruction( gentity_t* self, gentity_t* actor, S32 mod )
{
    ...
    {
        TeamCommand( ( team_t )actor->client->ps.stats[ STAT_TEAM ],
    ...
    if( mod == MOD_DECONSTRUCT )
        LogPrintf( "Decon: %d %d %d: %s^7 deconstructed %s\n",
        ...
        LogPrintf( "Decon: %d %d %d: %s^7 destroyed %s by %s\n",
                   actor->client->ps.clientNum,
                   self->s.modelindex,
                   mod,
                   actor->client->pers.netname,
    ...
    if( trap_Database_RunQuery( va( "INSERT INTO decons (idwho,idwhat,idhow) VALUES (\"%d\",\"%d\",\"%d\")",  actor->client->pers.mysqlid, self->s.modelindex, mod ) ) == true )
    {
        UTF8 data[255];
        trap_Database_FinishQuery();
        trap_Database_RunQuery( "SELECT id FROM decons ORDER BY id DESC LIMIT 1" );
        trap_Database_FetchRow();
        trap_Database_FetchFieldByName( "id", data, sizeof( data ) );
        trap_Database_RunQuery( va( "INSERT into mainlog (idaction,typeaction,date) VALUES (%d,\"%d\",NOW())", atoi( data ), 5 ) );
        trap_Database_FinishQuery();
    }
}
