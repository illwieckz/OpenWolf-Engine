...
UTF8 lastValidBase[MAX_OSPATH];
UTF8 lastValidGame[MAX_OSPATH];

#ifdef FS_MISSING
FILE*       missingFiles = NULL;
#endif

bool legacy_mp_bin = false;
...
================
*/
static S64 FS_HashFileName( StringEntry fname, S32 hashSize )
{
    S32 i;
    S64 hash;
    UTF8 letter;
...
====================
*/
static void FS_ReplaceSeparators( UTF8* path )
{
    UTF8*    s;
...
=====================
*/
void FS_BuildOSHomePath( UTF8* ospath, S32 size, S32 qpath )
{
...
============
*/
S32 FS_CreatePath( StringEntry OSPath_ )
{
...
===========
*/
/*static void FS_Remove( const UTF8 *osPath ) {
	remove( osPath );
}*/
...
===========
*/
static bool FS_Remove( StringEntry osPath )
{
...
bool FS_OS_FileExists( StringEntry file )
{
    FILE* f;
    f = fopen( file, "rb" );
    if( f )
...
===========
*/
fileHandle_t FS_FOpenFileWrite( StringEntry filename )
{
    UTF8*            ospath;
    fileHandle_t f;
...
===========
*/
fileHandle_t FS_FOpenFileAppend( StringEntry filename )
{
    UTF8*            ospath;
    fileHandle_t f;
...
===========
*/
S32 FS_FOpenFileDirect( StringEntry filename, fileHandle_t* f )
{
    S32   r;
    UTF8* ospath;
...
===========
*/
fileHandle_t FS_FOpenFileUpdate( StringEntry filename, S32* length )
{
    UTF8*			ospath;
    fileHandle_t	f;
...
S32 FS_FOpenFileRead( StringEntry filename, fileHandle_t* file, bool uniqueFILE )
{
    searchpath_t*   search;
    UTF8*           netpath;
    pack_t*         pak;
...
==============
*/
bool FS_AllowDeletion( UTF8* filename )
{
...
==============
*/
S32 FS_DeleteDir( UTF8* dirname, bool nonEmpty, bool recursive )
{
    UTF8* ospath;
    UTF8** pFiles = NULL;
    S32 i, nFiles = 0;
    ...
    if( recursive )
    {
        ...
        pFiles = Sys_ListFiles( ospath, root, NULL, &nFiles, false );
        for( i = 0; i < nFiles; i++ )
        {
            UTF8 temp[MAX_OSPATH];
    ...
    {
        ospath = FS_BuildOSPath( fs_homepath->string, fs_gamedir, dirname );
        pFiles = Sys_ListFiles( ospath, NULL, NULL, &nFiles, false );
        for( i = 0; i < nFiles; i++ )
        {
...
=================
*/
S32 FS_Write( const void* buffer, S32 len, fileHandle_t h )
{
    S32 block, remaining;
    S32 written;
    U8*    buf;
...
void FS_Printf( fileHandle_t h, StringEntry fmt, ... )
{
    va_list argptr;
...
=================
*/
static pack_t* FS_LoadZipFile( StringEntry zipfile, StringEntry basename )
{
    ...
    S32				i, len;
    S64			hash;
    S32				fs_numHeaderLongs;
    S32*				fs_headerLongs;
    UTF8*			namePtr;
    S32             sizeOfHeaderLongs;
...
static S32 FS_ReturnPath( StringEntry zname, UTF8* zpath, S32* depth )
{
    S32 len, at, newdep;
...
==================
*/
static S32 FS_AddFileToList( UTF8* name, UTF8* list[MAX_FOUND_FILES], S32 nfiles )
{
    S32 i;
...
===============
*/
UTF8** FS_ListFilteredFiles( StringEntry path, StringEntry extension, UTF8* filter, S32* numfiles )
{
    S32 nfiles;
    UTF8**            listCopy;
...
=======================
 */
static U32 Sys_CountFileList( UTF8** list )
{
    S32 i = 0;
...
static UTF8** Sys_ConcatenateFileLists( UTF8** list0, UTF8** list1, UTF8** list2 )
{
    S32 totalLength = 0;
...
===========
*/
S32 FS_PathCmp( StringEntry s1, StringEntry s2 )
{
    S32 c1, c2;
...
================
*/
void FS_SortFileList( UTF8** filelist, S32 numfiles )
{
...
===========
*/
bool FS_IsExt( StringEntry filename, StringEntry ext, S32 namelen )
{
    S32 extlen;
...
================
*/
void FS_AddGameDirectory( StringEntry path, StringEntry dir )
{
    searchpath_t*   sp;
    searchpath_t*   search;
    pack_t*         pak;
    ...
    qsort( pakfiles, numfiles, sizeof( UTF8* ), paksort );
    qsort( pakdirs, numdirs, sizeof( UTF8* ), paksort );
            ...
            else
            {
                ...
                if( FS_SV_FileExists( va( "%s.pk3", fs_serverReferencedPakNames[i] ) ) )
                {
                    ...
                    if( CL_WWWBadChecksum( va( "%s.pk3", fs_serverReferencedPakNames[i] ) ) )
                    {
                        ...
                        UTF8* rmv = FS_BuildOSPath( fs_homepath->string, va( "%s.pk3", fs_serverReferencedPakNames[i] ), "" );
                        rmv[strlen( rmv ) - 1] = '\0';
                        FS_Remove( rmv );
                    }
#endif
...
================
*/
static void FS_Startup( StringEntry gameName )
{
...
static StringEntry pak_checksums = "-137448799 131270674 125907563 -1023558518 764840216 1886207346";
static StringEntry pak_names = "mp_pak4 mp_pak3 mp_pak2 mp_pak1 mp_pak0 pak0";
...
static const S32 feeds[5] = {
  0x14d48835, 0xc44ed670, 0xd1c8da0d, 0x98df0626, 0xb4e51e7a
};

static StringEntry pak_purechecksums[5] = {
...
static S32 feed_index = -1;

static S32 lookup_randomized[5] = { 0, 1, 2, 3, 4 };
...
void FS_InitRandomFeed()
{
    ...
    for( i = 0; i < 5; i++ )
    {
        swap = ( S32 )( 5.0 * rand() / ( RAND_MAX + 1.0 ) );
...
void	FS_FilenameCompletion( StringEntry dir, StringEntry ext,
                               bool stripExt, void( *callback )( StringEntry s ) )
{
    UTF8**	filenames;
    S32		nfiles;
    S32		i;
...
bool FS_OW_RemoveFile( StringEntry qpath )
{
    UTF8 stripped[MAX_OSPATH];
    UTF8* ospath;
    StringEntry c;
    UTF8* ext;
