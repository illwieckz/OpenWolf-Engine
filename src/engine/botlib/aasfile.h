...
typedef struct aas_areasettings_s
{
    ...
    S32 contents;						//contents of the area
    S32 areaflags;						//several area flags
    S32 presencetype;					//how a bot can be present in this area
    S32 cluster;						//cluster the area belongs to, if negative it's a portal
    S32 clusterareanum;				//number of the area in the cluster
    S32 numreachableareas;			//number of reachable areas from this one
    S32 firstreachablearea;			//first reachable area in the reachable area index
} aas_areasettings_t;

//cluster portal
typedef struct aas_portal_s
{
    S32 areanum;						//area that is the actual portal
    S32 frontcluster;					//cluster at front of portal
    S32 backcluster;					//cluster at back of portal
    S32 clusterareanum[2];			//number of the area in the front and back cluster
} aas_portal_t;

//cluster portal index
typedef S32 aas_portalindex_t;

//cluster
typedef struct aas_cluster_s
{
    S32 numareas;						//number of areas in the cluster
    S32 numreachabilityareas;			//number of areas with reachabilities
    S32 numportals;						//number of cluster portals
    S32 firstportal;					//first cluster portal in the index
} aas_cluster_t;
...
typedef struct aas_plane_s
{
    ...
    F32 dist;							//distance of the plane (normal vector * distance = point in plane)
    S32 type;
} aas_plane_t;
...
{
    S32 v[2];							//numbers of the vertexes of this edge
} aas_edge_t;

//edge index, negative if vertexes are reversed
typedef S32 aas_edgeindex_t;

//a face bounds an area, often it will also seperate two areas
typedef struct aas_face_s
{
    S32 planenum;						//number of the plane this face is in
    S32 faceflags;						//face flags (no use to create face settings for just this field)
    S32 numedges;						//number of edges in the boundary of the face
    S32 firstedge;						//first edge in the edge index
    S32 frontarea;						//area at the front of this face
    S32 backarea;						//area at the back of this face
} aas_face_t;

//face index, stores a negative index if backside of face
typedef S32 aas_faceindex_t;

//area with a boundary of faces
typedef struct aas_area_s
{
    S32 areanum;						//number of this area
    //3d definition
    S32 numfaces;						//number of faces used for the boundary of the area
    S32 firstface;						//first face in the face index used for the boundary of the area
...
{
    S32 planenum;
    S32 children[2];					//child nodes of this node, or areas as leaves when negative
