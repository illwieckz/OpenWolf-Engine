...
struct gameImports_t
{
    ...
    void( *FS_CopyFile )( UTF8* from, UTF8* to );
    ...
    void( *Cvar_Register )( vmCvar_t* cvar, StringEntry var_name, StringEntry value, S32 flags );
    ...
    void( *LocateGameData )( gentity_t* gEnts, S32 numGEntities, S32 sizeofGEntity_t, playerState_t* clients, S32 sizeofGClient );
    
    void( *DropClient )( S32 clientNum, StringEntry reason, S32 length );
    void( *SendServerCommand )( S32 clientNum, StringEntry text );
    ...
    void( *GetServerinfo )( UTF8* buffer, S32 bufferSize );
    void( *SetBrushModel )( gentity_t* ent, StringEntry name );
    
    void( *Trace )( trace_t* results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, S32 passEntityNum, S32 contentmask, traceType_t type );
    ...
    S32( *EntitiesInBox )( const vec3_t mins, const vec3_t maxs, S32* list, S32 maxcount );
    
    bool( *EntityContact )( const vec3_t mins, const vec3_t maxs, const sharedEntity_t* gEnt, traceType_t type );
    
    S32( *BotAllocateClient )( S32 clientNum );
    void( *BotFreeClient )( S32 clientNum );
    
    void( *GetUsercmd )( S32 clientNum, usercmd_t* cmd );
    bool( *GetEntityToken )( UTF8* buffer, S32 bufferSize );
