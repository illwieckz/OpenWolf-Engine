...
=============
*/
void idCGameLocal::Obituary( entityState_t* ent )
{
    S32           mod;
    S32           target, attacker;
    UTF8*          message;
    UTF8*          message2;
    StringEntry    targetInfo;
    StringEntry    attackerInfo;
    UTF8          targetName[ MAX_NAME_LENGTH ];
    UTF8          attackerName[ MAX_NAME_LENGTH ];
    UTF8          className[ 64 ];
    gender_t      gender;
    clientInfo_t*  ci;
    bool      teamKill = false;
    ...
    {
        attackerInfo = ConfigString( CS_PLAYERS + attacker );
        if( ci && cgs.clientinfo[ attacker ].team == ci->team )
            teamKill = true;
    }
    
    targetInfo = ConfigString( CS_PLAYERS + target );
    ...
    if( attacker != ENTITYNUM_WORLD )
    {
        ...
        if( message )
        {
            ...
            {
                CenterPrint( va( "You killed " S_COLOR_RED "TEAMMATE " S_COLOR_WHITE "%s", targetName ), SCREEN_HEIGHT * 0.30, BIGCHAR_WIDTH );
            }
            return;
        }
    }
...
================
*/
void idCGameLocal::PainEvent( centity_t* cent, S32 health )
{
    UTF8*  snd;
    ...
    trap_S_StartSound( NULL, cent->currentState.number, CHAN_VOICE, CustomSound( cent->currentState.number, snd ) );
...
=========================
*/
void idCGameLocal::Level2Zap( entityState_t* es )
{
    S32 i;
    ...
    for( i = 0; i <= 2; i++ )
    {
        ...
        if( !IsTrailSystemValid( &source->level2ZapTS[ i ] ) )
            source->level2ZapTS[ i ] = SpawnNewTrailSystem( cgs.media.level2ZapTS );
            
        if( IsTrailSystemValid( &source->level2ZapTS[ i ] ) )
        {
            SetAttachmentCent( &source->level2ZapTS[ i ]->frontAttachment, source );
            SetAttachmentCent( &source->level2ZapTS[ i ]->backAttachment, target );
            AttachToCent( &source->level2ZapTS[ i ]->frontAttachment );
            AttachToCent( &source->level2ZapTS[ i ]->backAttachment );
        }
    }
...
==============
*/
void idCGameLocal::EntityEvent( centity_t* cent, vec3_t position )
{
    entityState_t* es;
    S32           event;
    vec3_t        dir;
    ...
    switch( event )
    {
        ...
        case EV_FALL_MEDIUM:
            ...
            trap_S_StartSound( NULL, es->number, CHAN_VOICE, CustomSound( es->number, "*pain100_1.wav" ) );
        ...
        case EV_FALL_FAR:
            trap_S_StartSound( NULL, es->number, CHAN_AUTO, CustomSound( es->number, "*fall1.wav" ) );
        ...
        case EV_FALLING:
            trap_S_StartSound( NULL, es->number, CHAN_AUTO, CustomSound( es->number, "*falling1.wav" ) );
            break;
        ...
        case EV_JUMP:
            trap_S_StartSound( NULL, es->number, CHAN_VOICE, CustomSound( es->number, "*jump1.wav" ) );
            ...
            if( bggame->ClassHasAbility( ( class_t )cg.predictedPlayerState.stats[ STAT_CLASS ], SCA_WALLJUMPER ) )
            {
                ...
                if( !VectorCompare( surfNormal, cg.lastNormal ) && surfNormal[ 2 ] != 1.0f )
                {
                    ...
                    //add the op
                    addSmoothOp( rotAxis, 15.0f, 1.0f );
                }
        ...
        case EV_TAUNT:
            if( !cg_noTaunt.integer )
                trap_S_StartSound( NULL, es->number, CHAN_VOICE, CustomSound( es->number, "*taunt.wav" ) );
            break;
        ...
        case EV_FIRE_WEAPON:
            FireWeapon( cent, WPM_PRIMARY );
            break;
            
        case EV_FIRE_WEAPON2:
            FireWeapon( cent, WPM_SECONDARY );
            break;
            
        case EV_FIRE_WEAPON3:
            FireWeapon( cent, WPM_TERTIARY );
            break;
            
        case EV_ALIEN_HIT:
            HandleAlienFeedback( cent, AFEEDBACK_HIT );
            break;
            
        case EV_ALIEN_MISS:
            HandleAlienFeedback( cent, AFEEDBACK_MISS );
            break;
            
        case EV_ALIEN_TEAMHIT:
            HandleAlienFeedback( cent, AFEEDBACK_TEAMHIT );
            break;
            
        case EV_ALIENRANGED_HIT:
            HandleAlienFeedback( cent, AFEEDBACK_RANGED_HIT );
            break;
            
        case EV_ALIENRANGED_MISS:
            HandleAlienFeedback( cent, AFEEDBACK_RANGED_MISS );
            break;
        ...
        case EV_MISSILE_MISS:
            ByteToDir( es->eventParm, dir );
            MissileHitWall( ( weapon_t )es->weapon, ( weaponMode_t )es->generic1, 0, position, dir, IMPACTSOUND_DEFAULT, es->torsoAnim );
            break;
            
        case EV_MISSILE_MISS_METAL:
            ByteToDir( es->eventParm, dir );
            MissileHitWall( ( weapon_t )es->weapon, ( weaponMode_t )es->generic1, 0, position, dir, IMPACTSOUND_METAL, es->torsoAnim );
            break;
            
        case EV_HUMAN_BUILDABLE_EXPLOSION:
            ByteToDir( es->eventParm, dir );
            HumanBuildableExplosion( position, dir );
            break;
            
        case EV_ALIEN_BUILDABLE_EXPLOSION:
            ByteToDir( es->eventParm, dir );
            AlienBuildableExplosion( position, dir );
            break;
        ...
        case EV_TESLATRAIL:
            cent->currentState.weapon = WP_TESLAGEN;
            {
                ...
                if( !IsTrailSystemValid( &source->muzzleTS ) )
                {
                    source->muzzleTS = SpawnNewTrailSystem( cgs.media.teslaZapTS );
                    
                    if( IsTrailSystemValid( &source->muzzleTS ) )
                    {
                        SetAttachmentCent( &source->muzzleTS->frontAttachment, source );
                        SetAttachmentCent( &source->muzzleTS->backAttachment, target );
                        AttachToCent( &source->muzzleTS->frontAttachment );
                        AttachToCent( &source->muzzleTS->backAttachment );
                        SetAttachmentOffset( &source->muzzleTS->frontAttachment, sourceOffset );
        ...
        case EV_BULLET_HIT_WALL:
            ByteToDir( es->eventParm, dir );
            Bullet( es->pos.trBase, es->otherEntityNum, dir, false, ENTITYNUM_WORLD );
            break;
            
        case EV_BULLET_HIT_FLESH:
            Bullet( es->pos.trBase, es->otherEntityNum, dir, true, es->eventParm );
            break;
        ...
        case EV_GENERAL_SOUND:
            ...
            {
                s = ConfigString( CS_SOUNDS + es->eventParm );
                trap_S_StartSound( NULL, es->number, CHAN_VOICE, CustomSound( es->number, s ) );
            }
            break;
        ...
        case EV_GLOBAL_SOUND: // play from the player's head so it never diminishes
            ...
            {
                s = ConfigString( CS_SOUNDS + es->eventParm );
                trap_S_StartSound( NULL, cg.snap->ps.clientNum, CHAN_AUTO, CustomSound( es->number, s ) );
            }
            break;
        ...
        case EV_DEATH2:
        case EV_DEATH3:
            trap_S_StartSound( NULL, es->number, CHAN_VOICE, CustomSound( es->number, va( "*death%i.wav", event - EV_DEATH1 + 1 ) ) );
            break;
        ...
        case EV_OVERMIND_ATTACK:
            if( cg.predictedPlayerState.stats[ STAT_TEAM ] == TEAM_ALIENS )
            {
                ...
                CenterPrint( "The Overmind is under attack!", 200, GIANTCHAR_WIDTH * 4 );
            }
            break;
        ...
        case EV_OVERMIND_DYING:
            if( cg.predictedPlayerState.stats[ STAT_TEAM ] == TEAM_ALIENS )
            {
                ...
                CenterPrint( "The Overmind is dying!", 200, GIANTCHAR_WIDTH * 4 );
            }
            break;
        ...
        case EV_DCC_ATTACK:
            if( cg.predictedPlayerState.stats[ STAT_TEAM ] == TEAM_HUMANS )
            {
                ...
                CenterPrint( "Our base is under attack!", 200, GIANTCHAR_WIDTH * 4 );
            }
            break;
        ...
        case EV_OVERMIND_SPAWNS:
            if( cg.predictedPlayerState.stats[ STAT_TEAM ] == TEAM_ALIENS )
            {
                ...
                CenterPrint( "The Overmind needs spawns!", 200, GIANTCHAR_WIDTH * 4 );
            }
            break;
        ...
        case EV_ALIEN_EVOLVE:
            ...
            {
                particleSystem_t* ps = SpawnNewParticleSystem( cgs.media.alienEvolvePS );
                
                if( IsParticleSystemValid( &ps ) )
                {
                    SetAttachmentCent( &ps->attachment, cent );
                    AttachToCent( &ps->attachment );
                }
            }
        ...
        {
            particleSystem_t* ps = SpawnNewParticleSystem( cgs.media.alienAcidTubePS );
...
==============
*/
void idCGameLocal::CheckEvents( centity_t* cent )
{
    ...
    if( cent->currentState.eType > ET_EVENTS )
    {
        ...
        if( cent->currentState.eFlags & EF_PLAYER_EVENT )
        {
            ...
            cent->currentState.event = _event;
        }
    }
    else
    {
