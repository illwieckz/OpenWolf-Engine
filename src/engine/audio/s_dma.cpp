...
Disables sounds until the next S_Base_BeginRegistration.
...
==================
*/
sfxHandle_t S_Base_RegisterSound( StringEntry name )
{
    sfx_t*          sfx;
...
=================
*/
void S_SpatializeOrigin( vec3_t origin, S32 master_vol, int* left_vol, int* right_vol )
{
    vec_t           dot;
    vec_t           dist;
...
void S_Base_StartSound( vec3_t origin, S32 entityNum, S32 entchannel, sfxHandle_t sfxHandle )
{
    ...
    {
        Com_Error( ERR_DROP, "S_Base_StartSound: bad entitynum %i", entityNum );
    }
    
    if( sfxHandle < 0 || sfxHandle >= s_numSfx )
    {
        Com_Printf( S_COLOR_YELLOW "S_Base_StartSound: handle %i out of range\n", sfxHandle );
        return;
    }
...
void S_Base_StartLocalSound( sfxHandle_t sfxHandle, S32 channelNum )
{
    ...
    if( sfxHandle < 0 || sfxHandle >= s_numSfx )
    {
        Com_Printf( S_COLOR_YELLOW "S_Base_StartLocalSound: handle %i out of range\n", sfxHandle );
        return;
    }
...
void S_Base_AddLoopingSound( S32 entityNum, const vec3_t origin, const vec3_t velocity, sfxHandle_t sfxHandle )
{
    ...
    if( sfxHandle < 0 || sfxHandle >= s_numSfx )
    {
        Com_Printf( S_COLOR_YELLOW "S_Base_AddLoopingSound: handle %i out of range\n", sfxHandle );
        return;
    }
...
============
*/
void S_Base_RawSamples( S32 stream, S32 samples, S32 rate, S32 width, S32 s_channels, const U8* data, F32 volume )
{
    S32             i;
    S32             src, dst;
    F32           scale;
    ...
    if( s_channels == 2 && width == 2 )
    {
        if( scale == 1.0 )
        {
            ...
            for( i = 0; i < samples; i++ )
            {
                ...
                s_rawend[stream]++;
                rawsamples[dst].left = ( ( S16* )data )[i * 2] * intVolume;
                rawsamples[dst].right = ( ( S16* )data )[i * 2 + 1] * intVolume;
            }
        }
        else
        {
            for( i = 0;; i++ )
            {
                ...
                dst = s_rawend[stream] & ( MAX_RAW_SAMPLES - 1 );
                s_rawend[stream]++;
                rawsamples[dst].left = ( ( S16* )data )[src * 2] * intVolume;
                rawsamples[dst].right = ( ( S16* )data )[src * 2 + 1] * intVolume;
            }
        }
    }
    else if( s_channels == 1 && width == 2 )
    {
        for( i = 0;; i++ )
        {
            ...
            dst = s_rawend[stream] & ( MAX_RAW_SAMPLES - 1 );
            s_rawend[stream]++;
            rawsamples[dst].left = ( ( S16* )data )[src] * intVolume;
            rawsamples[dst].right = ( ( S16* )data )[src] * intVolume;
        }
    }
    else if( s_channels == 2 && width == 1 )
    {
        ...
        for( i = 0;; i++ )
        {
            ...
            dst = s_rawend[stream] & ( MAX_RAW_SAMPLES - 1 );
            s_rawend[stream]++;
            rawsamples[dst].left = ( ( UTF8* )data )[src * 2] * intVolume;
            rawsamples[dst].right = ( ( UTF8* )data )[src * 2 + 1] * intVolume;
        }
    }
    else if( s_channels == 1 && width == 1 )
    {
        ...
        for( i = 0;; i++ )
        {
            ...
            dst = s_rawend[stream] & ( MAX_RAW_SAMPLES - 1 );
            s_rawend[stream]++;
            rawsamples[dst].left = ( ( ( U8* ) data )[src] - 128 ) * intVolume;
            rawsamples[dst].right = ( ( ( U8* ) data )[src] - 128 ) * intVolume;
        }
    }
...
void S_Base_UpdateEntityPosition( S32 entityNum, const vec3_t origin )
{
    ...
    {
        Com_Error( ERR_DROP, "S_Base_UpdateEntityPosition: bad entitynum %i", entityNum );
    }
    VectorCopy( origin, loopSounds[entityNum].origin );
}
...
void S_GetSoundtime( void )
{
    ...
    // calls to S_Base_Update.  Oh well.
...
void S_Base_StartBackgroundTrack( StringEntry intro, StringEntry loop )
{
    ...
    Com_DPrintf( "S_Base_StartBackgroundTrack( %s, %s )\n", intro, loop );
