...
{
    UTF8*           cmd, *map, smapname[MAX_QPATH], mapname[MAX_QPATH], expanded[MAX_QPATH], *cl_profileStr = Cvar_VariableString( "cl_profile" );
    ...
    buildScript = ( bool )Cvar_VariableIntegerValue( "com_buildScript" );
...
================
*/
static bool SV_TransitionGameState( gamestate_t new_gs, gamestate_t old_gs, S32 delay )
{
...
static void SV_MapRestart_f( void )
{
    ...
    if( Cvar_VariableIntegerValue( "savegame_loading" ) )
    {
        ...
        UTF8    savemap[MAX_QPATH], *cl_profileStr = Cvar_VariableString( "cl_profile" );
        U8*    buffer;
...
{
    UTF8            filename[MAX_QPATH], mapname[MAX_QPATH], savedir[MAX_QPATH], *cl_profileStr = Cvar_VariableString( "cl_profile" );
    U8*             buffer;
    S32             size;
...
//  if(sv_reloading->integer && sv_reloading->integer != RELOAD_FAILED )    // game is in 'reload' mode, don't allow starting new maps yet.
    ...
    Q_strncpyz( mapname, ( const UTF8* )( buffer + sizeof( S32 ) ), sizeof( mapname ) );
...
==================
*/
static void SV_CompleteMapName( UTF8* args, S32 argNum )
{
    if( argNum == 2 )
    {
