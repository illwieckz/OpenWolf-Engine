...
void AAS_WriteRouteCache( void )
{
    ...
    for (i = 0; i < aasworld.numareas; i++)
    {
    	if (!aasworld.areavisibility[i]) {
    		size = 0;
    		botimport.FS_Write(&size, sizeof(S32), fp);
    		continue;
    	}
...
aas_routingcache_t* AAS_ReadCache( fileHandle_t fp )
{
    ...
    cache->size = size;
    botimport.FS_Read( ( U8* )cache + sizeof( size ), size - sizeof( size ), fp );
    cache->reachabilities = ( U8* ) cache + sizeof( aas_routingcache_t ) - sizeof( U16 ) +
                            ( size - sizeof( aas_routingcache_t ) + sizeof( U16 ) ) / 3 * 2;
    return cache;
