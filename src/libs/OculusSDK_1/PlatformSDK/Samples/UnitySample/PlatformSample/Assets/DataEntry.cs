...
public class DataEntry : MonoBehaviour
{
    ...
    void Start()
    {
        Core.Initialize("987072391351510");
        checkEntitlement();
    }
    ...
    void createAndJoinPrivateRoom(string joinPolicy, string maxUsers)
    {
        ...
        Rooms.CreateAndJoinPrivate((JoinPolicy)Convert.ToUInt32(joinPolicy), Convert.ToUInt32(maxUsers)).OnComplete(createAndJoinPrivateRoomCallback);
    }
    ...
    {
        printOutputLine("Trying to get friends of user " + userID);
        Users.GetUserFriends(Convert.ToUInt64(userID)).OnComplete(getFriendsCallback);
    }
