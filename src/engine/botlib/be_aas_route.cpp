...
#ifdef ROUTING_DEBUG
S32 numareacacheupdates;
S32 numportalcacheupdates;
#endif //ROUTING_DEBUG

S32 routingcachesize;
S32 max_routingcachesize;
...
// Changes Globals:		-
//===========================================================================
ID_INLINE S32 AAS_ClusterAreaNum( S32 cluster, S32 areanum )
{
    S32 side, areacluster;
...
// Changes Globals:		-
//===========================================================================
ID_INLINE S32 AAS_TravelFlagForType_inline( S32 traveltype )
{
    S32 tfl;
...
// Changes Globals:		-
//===========================================================================
void AAS_RemoveRoutingCacheInCluster( S32 clusternum )
{
    S32 i;
...
// Changes Globals:		-
//===========================================================================
void AAS_RemoveRoutingCacheUsingArea( S32 areanum )
{
    S32 i, clusternum;
    aas_routingcache_t* cache, *nextcache;
...
// Changes Globals:		-
//===========================================================================
ID_INLINE F32 AAS_RoutingTime( void )
{
    return AAS_Time();
...
// Changes Globals:		-
//===========================================================================
S32 AAS_GetAreaContentsTravelFlags( S32 areanum )
{
    S32 contents, tfl;
...
// Changes Globals:		-
//===========================================================================
ID_INLINE S32 AAS_AreaContentsTravelFlags_inline( S32 areanum )
{
...
void AAS_InitAreaContentsTravelFlags( void )
{
    ...
    aasworld.areacontentstravelflags = ( S32* ) GetClearedMemory( aasworld.numareas * sizeof( S32 ) );
    //
    for( i = 0; i < aasworld.numareas; i++ )
    {
...
void AAS_CreateReversedReachability( void )
{
    ...
    ptr = ( UTF8* ) GetClearedMemory( aasworld.numareas * sizeof( aas_reversedreachability_t ) +
...
void AAS_CalculateAreaTravelTimes( void )
{
    ...
    size = aasworld.numareas * sizeof( U16** );
    for( i = 0; i < aasworld.numareas; i++ )
    {
        ...
        settings = &aasworld.areasettings[i];
        //
        size += settings->numreachableareas * sizeof( U16* );
        //
        size += settings->numreachableareas *
                PAD( revreach->numlinks, sizeof( S64 ) ) * sizeof( U16 );
    } //end for
    //allocate memory for the area travel times
    ptr = ( UTF8* ) GetClearedMemory( size );
    aasworld.areatraveltimes = ( U16*** ) ptr;
    ptr += aasworld.numareas * sizeof( U16** );
    ...
    for( i = 0; i < aasworld.numareas; i++ )
    {
        ...
        settings = &aasworld.areasettings[i];
        //
        aasworld.areatraveltimes[i] = ( U16** ) ptr;
        ptr += settings->numreachableareas * sizeof( U16* );
        //
        for( l = 0; l < settings->numreachableareas; l++ )
        {
            aasworld.areatraveltimes[i][l] = ( U16* ) ptr;
            ptr += PAD( revreach->numlinks, sizeof( S64 ) ) * sizeof( U16 );
            //reachability link
...
// Changes Globals:		-
//===========================================================================
S32 AAS_PortalMaxTravelTime( S32 portalnum )
{
    S32 l, n, t, maxt;
    aas_portal_t* portal;
    aas_reversedreachability_t* revreach;
...
void AAS_InitPortalMaxTravelTimes( void )
{
    ...
    aasworld.portalmaxtraveltimes = ( S32* ) GetClearedMemory( aasworld.numportals * sizeof( S32 ) );
...
//===========================================================================
/*
S32 AAS_FreeOldestCache(void)
{
	S32 i, j, bestcluster, bestarea, freed;
	F32 besttime;
...
// Changes Globals:		-
//===========================================================================
aas_routingcache_t* AAS_AllocRoutingCache( S32 numtraveltimes )
{
    aas_routingcache_t* cache;
    S32 size;
    ...
    size = sizeof( aas_routingcache_t )
           + numtraveltimes * sizeof( U16 )
           + numtraveltimes * sizeof( U8 );
    //
    routingcachesize += size;
    //
    cache = ( aas_routingcache_t* ) GetClearedMemory( size );
    cache->reachabilities = ( U8* ) cache + sizeof( aas_routingcache_t )
                            + numtraveltimes * sizeof( U16 );
    cache->size = size;
    return cache;
...
//void AAS_DecompressVis(U8 *in, S32 numareas, U8 *decompressed);
//S32 AAS_CompressVis(U8 *vis, S32 numareas, U8 *dest);

void AAS_WriteRouteCache( void )
{
    S32 i, j, numportalcache, numareacache, totalsize;
    aas_routingcache_t* cache;
    aas_cluster_t* cluster;
    fileHandle_t fp;
    ...
    routecacheheader.areacrc = CRC_ProcessString( ( U8* )aasworld.areas, sizeof( aas_area_t ) * aasworld.numareas );
    routecacheheader.clustercrc = CRC_ProcessString( ( U8* )aasworld.clusters, sizeof( aas_cluster_t ) * aasworld.numclusters );
    routecacheheader.numportalcache = numportalcache;
    routecacheheader.numareacache = numareacache;
    //write the header
    fileSystem->Write( &routecacheheader, sizeof( routecacheheader_t ), fp );
    //
    totalsize = 0;
    //write all the cache
    for( i = 0; i < aasworld.numareas; i++ )
    {
        for( cache = aasworld.portalcache[i]; cache; cache = cache->next )
        {
            fileSystem->Write( cache, cache->size, fp );
    ...
    for (i = 0; i < aasworld.numareas; i++)
    {
    	if (!aasworld.areavisibility[i]) {
    		size = 0;
    		fileSystem->Write(&size, sizeof(S32), fp);
    		continue;
    	}
    	...
    	fileSystem->Write(&size, sizeof(S32), fp);
    	fileSystem->Write(aasworld.decompressedvis, size, fp);
    }
    */
    //
...
aas_routingcache_t* AAS_ReadCache( fileHandle_t fp )
{
    ...
    fileSystem->Read2( &size, sizeof( size ), fp );
    cache = ( aas_routingcache_t* ) GetMemory( size );
    cache->size = size;
    fileSystem->Read2( ( U8* )cache + sizeof( size ), size - sizeof( size ), fp );
    cache->reachabilities = ( U8* ) cache + sizeof( aas_routingcache_t ) - sizeof( U16 ) + ( size - sizeof( aas_routingcache_t ) + sizeof( U16 ) ) / 3 * 2;
    return cache;
...
// Changes Globals:		-
//===========================================================================
S32 AAS_ReadRouteCache( void )
{
    S32 i, clusterareanum;//, size;
    fileHandle_t fp;
    UTF8 filename[MAX_QPATH];
    routecacheheader_t routecacheheader;
    aas_routingcache_t* cache;
    ...
    fileSystem->Read2( &routecacheheader, sizeof( routecacheheader_t ), fp );
    ...
    if( routecacheheader.areacrc !=
            CRC_ProcessString( ( U8* )aasworld.areas, sizeof( aas_area_t ) * aasworld.numareas ) )
    {
        //AAS_Error("route cache dump area CRC incorrect\n");
        return false;
    } //end if
    if( routecacheheader.clustercrc !=
            CRC_ProcessString( ( U8* )aasworld.clusters, sizeof( aas_cluster_t ) * aasworld.numclusters ) )
    {
    ...
    // read the visareas
    /*
    aasworld.areavisibility = (U8 **) GetClearedMemory(aasworld.numareas * sizeof(U8 *));
    aasworld.decompressedvis = (U8 *) GetClearedMemory(aasworld.numareas * sizeof(U8));
    for (i = 0; i < aasworld.numareas; i++)
    {
    	fileSystem->Read2(&size, sizeof(size), fp );
    	if (size) {
    		aasworld.areavisibility[i] = (U8 *) GetMemory(size);
    		fileSystem->Read2(aasworld.areavisibility[i], size, fp );
    	}
    }
    */
    //
...
{
    S32 i, j, numareas, areas[MAX_REACHABILITYPASSAREAS];
    S32 numreachareas;
    aas_reachability_t* reach;
    vec3_t start, end;
    ...
    aasworld.reachabilityareaindex = ( S32* )
                                     GetClearedMemory( aasworld.reachabilitysize * MAX_REACHABILITYPASSAREAS * sizeof( S32 ) );
    numreachareas = 0;
...
void AAS_InitRouting( void )
{
    ...
    //
    routingcachesize = 0;
    max_routingcachesize = 1024 * ( S32 ) LibVarValue( "max_routingcache", "4096" );
...
{
    S32 i, nextareanum, cluster, badtravelflags, clusterareanum, linknum;
    S32 numreachabilityareas;
    U16 t, startareatraveltimes[128]; //NOTE: not more than 128 reachabilities per area allowed
...
// Changes Globals:		-
//===========================================================================
aas_routingcache_t* AAS_GetAreaRoutingCache( S32 clusternum, S32 areanum, S32 travelflags )
{
    S32 clusterareanum;
...
{
    S32 i, portalnum, clusterareanum, clusternum;
    U16 t;
    aas_portal_t* portal;
    aas_cluster_t* cluster;
    aas_routingcache_t* cache;
...
// Changes Globals:		-
//===========================================================================
aas_routingcache_t* AAS_GetPortalRoutingCache( S32 clusternum, S32 areanum, S32 travelflags )
{
    aas_routingcache_t* cache;
...
// Changes Globals:		-
//===========================================================================
S32 AAS_AreaRouteToGoalArea( S32 areanum, vec3_t origin, S32 goalareanum, S32 travelflags, S32* traveltime, S32* reachnum )
{
    S32 clusternum, goalclusternum, portalnum, i, clusterareanum, bestreachnum;
    U16 t, besttime;
    aas_portal_t* portal;
    aas_cluster_t* cluster;
...
// Changes Globals:		-
//===========================================================================
S32 AAS_AreaReachabilityToGoalArea( S32 areanum, vec3_t origin, S32 goalareanum, S32 travelflags )
{
    S32 traveltime, reachnum;
...
{
    S32 curareanum, reachnum, i, j, testareanum;
    vec3_t curorigin;
    aas_reachability_t* reach;
...
// Changes Globals:		-
//===========================================================================
S32 AAS_BridgeWalkable( S32 areanum )
{
    return false;
...
// Changes Globals:		-
//===========================================================================
S32 AAS_AreaVisible( S32 srcarea, S32 destarea )
{
    return false;
...
// Changes Globals:		-
//===========================================================================
F32 DistancePointToLine( vec3_t v1, vec3_t v2, vec3_t point )
{
    vec3_t vec, p2;
...
// Changes Globals:		-
//===========================================================================
S32 AAS_NearestHideArea( S32 srcnum, vec3_t origin, S32 areanum, S32 enemynum, vec3_t enemyorigin, S32 enemyareanum, S32 travelflags )
{
    S32 i, j, nextareanum, badtravelflags, numreach, bestarea;
    U16 t, besttraveltime, enemytraveltime;
    static U16* hidetraveltimes;
    aas_routingupdate_t* updateliststart, *updatelistend, *curupdate, *nextupdate;
    aas_reachability_t* reach;
    F32 dist1, dist2;
    vec3_t enemyVec;
...
#define MAX_HIDEAREA_LOOPS 3000
    ...
    if( !hidetraveltimes )
    {
        hidetraveltimes = ( U16* ) GetClearedMemory( aasworld.numareas * sizeof( U16 ) );
    } //end if
    else
    {
        ::memset( hidetraveltimes, 0, aasworld.numareas * sizeof( U16 ) );
    } //end else
    besttraveltime = 0;
    bestarea = 0;
    //assume visible
    ...
    startVisible = botimport.BotVisibleFromPos( enemyorigin, enemynum, origin, srcnum, false );
