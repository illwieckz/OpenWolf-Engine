...
StringEntry	NET_AdrToStringwPort( netadr_t a )
{
    ...
    if( a.type == NA_LOOPBACK )
        ...
        Com_sprintf( s, sizeof( s ), "%s:%lu", NET_AdrToString( a ), ( U64 )ntohs( a.port ) );
    else if( a.type == NA_IP6 )
        Com_sprintf( s, sizeof( s ), "[%s]:%lu", NET_AdrToString( a ), ( U64 )ntohs( a.port ) );
...
====================
*/
S32 NET_IPSocket( UTF8* net_interface, S32 port, S32* err )
{
    SOCKET				newsocket;
...
====================
*/
S32 NET_IP6Socket( UTF8* net_interface, S32 port, struct sockaddr_in6* bindto, S32* err )
{
    SOCKET				newsocket;
