...
=================
*/
static int S_ReadChunkInfo( fileHandle_t f, UTF8* name )
{
    S32 len, r;
...
=================
*/
static int S_FindRIFFChunk( fileHandle_t f, UTF8* chunk )
{
    UTF8 name[5];
    S32 len;
...
void* S_WAV_CodecLoad( StringEntry filename, snd_info_t* info )
{
    ...
    S_ByteSwapRawSamples( info->samples, info->width, info->channels, ( U8* ) buffer );
...
=================
*/
int S_WAV_CodecReadStream( snd_stream_t* stream, S32 bytes, void* buffer )
{
    ...
    S_ByteSwapRawSamples( samples, stream->info.width, stream->info.channels, ( U8* )buffer );
    return bytes;
}
