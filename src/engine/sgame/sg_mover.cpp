...
/*
============
idGameLocal::TestEntityPosition
============
*/
gentity_t* idGameLocal::TestEntityPosition( gentity_t* ent )
{
    trace_t tr;
    S32   mask;
...
/*
================
idGameLocal::CreateRotationMatrix
================
*/
void idGameLocal::CreateRotationMatrix( vec3_t angles, vec3_t matrix[ 3 ] )
{
...
/*
================
idGameLocal::TransposeMatrix
================
*/
void idGameLocal::TransposeMatrix( vec3_t matrix[ 3 ], vec3_t transpose[ 3 ] )
{
    S32 i, j;
...
/*
================
idGameLocal::RotatePoint
================
*/
void idGameLocal::RotatePoint( vec3_t point, vec3_t matrix[ 3 ] )
{
    vec3_t tvec;
...
==================
*/
bool idGameLocal::TryPushingEntity( gentity_t* check, gentity_t* pusher, vec3_t move, vec3_t amove )
{
    ...
    if( pushed_p > &pushed[ MAX_GENTITIES ] )
        Error( "pushed_p > &pushed[MAX_GENTITIES]" );
...
============
*/
bool idGameLocal::MoverPush( gentity_t* pusher, vec3_t move, vec3_t amove, gentity_t** obstacle )
{
    S32       i, e;
    gentity_t* check;
    vec3_t    mins, maxs;
    ...
    *obstacle = NULL;
    ...
    for( e = 0 ; e < listedEntities ; e++ )
    {
        ...
        {
            Damage( check, pusher, pusher, NULL, NULL, 99999, 0, MOD_CRUSH );
            continue;
        }
...
/*
=================
idGameLocal::MoverTeam
=================
*/
void idGameLocal::MoverTeam( gentity_t* ent )
{
    vec3_t    move, amove;
...
/*
================
idGameLocal::RunMover
================
*/
void idGameLocal::RunMover( gentity_t* ent )
{
...
===============
*/
void idGameLocal::SetMoverState( gentity_t* ent, moverState_t moverState, S32 time )
{
    vec3_t delta;
    F32  f;
...
/*
================
idGameLocal::MatchTeam

All entities in a mover team will move from pos1 to pos2
in the same amount of time
================
*/
void idGameLocal::MatchTeam( gentity_t* teamLeader, S32 moverState, S32 time )
{
    gentity_t*   slave;
...
/*
================
idGameLocal::ReturnToPos1
================
*/
void idGameLocal::ReturnToPos1( gentity_t* ent )
{
...
/*
================
idGameLocal::ReturnToApos1
================
*/
void idGameLocal::ReturnToApos1( gentity_t* ent )
{
...
================
*/
void idGameLocal::Think_ClosedModelDoor( gentity_t* ent )
{
    // play sound
    if( ent->soundPos1 )
    {
...
/*
================
idGameLocal::Think_CloseModelDoor
================
*/
void idGameLocal::Think_CloseModelDoor( gentity_t* ent )
{
    S32       entityList[ MAX_GENTITIES ];
    S32       numEntities, i;
...
================
*/
void idGameLocal::Think_OpenModelDoor( gentity_t* ent )
{
    //set brush non-solid
    trap_UnlinkEntity( ent->clipBrush );
...
/*
================
idGameLocal::Reached_BinaryMover
================
*/
void idGameLocal::Reached_BinaryMover( gentity_t* ent )
{
    ...
    else if( ent->moverState == ROTATOR_2TO1 )
    {
        ...
        Error( "Reached_BinaryMover: bad moverState" );
}

/*
================
idGameLocal::Use_BinaryMover
================
*/
void idGameLocal::Use_BinaryMover( gentity_t* ent, gentity_t* other, gentity_t* activator )
{
    S32 total, partial;
...
================
*/
void idGameLocal::InitMover( gentity_t* ent )
{
    vec3_t    move;
    F32     distance;
    F32     light;
...
/*
================
idGameLocal::InitRotator

"pos1", "pos2", and "speed" should be set before calling,
so the movement delta can be calculated
================
*/
void idGameLocal::InitRotator( gentity_t* ent )
{
    vec3_t    move;
    F32     angle;
    F32     light;
...
================
*/
void idGameLocal::Blocked_Door( gentity_t* ent, gentity_t* other )
{
    ...
    if( ent->damage )
        Damage( other, ent, ent, NULL, NULL, ent->damage, 0, MOD_CRUSH );
...
================
*/
void idGameLocal::Touch_DoorTriggerSpectator( gentity_t* ent, gentity_t* other, trace_t* trace )
{
    S32     i, axis;
...
/*
================
idGameLocal::manualDoorTriggerSpectator

This effectively creates a temporary door auto trigger so manually
triggers doors can be skipped by spectators
================
*/
void idGameLocal::manualDoorTriggerSpectator( gentity_t* door, gentity_t* player )
{
    gentity_t* other;
...
/*
================
idGameLocal::manualTriggerSpectator

Trip to skip the closest door targetted by trigger
================
*/
void idGameLocal::manualTriggerSpectator( gentity_t* trigger, gentity_t* player )
{
    gentity_t* t = NULL;
    gentity_t* targets[ MAX_GENTITIES ];
    S32       i = 0, j;
    F32     minDistance = ( F32 )INFINITE;
    ...
    while( ( t = Find( t, FOFS( targetname ), trigger->target ) ) != NULL )
    {
        if( !strcmp( t->classname, "func_door" ) )
            targets[ i++ ] = t;
        else if( t == trigger )
            Printf( "WARNING: Entity used itself.\n" );
            
        if( !trigger->inuse )
        {
            Printf( "triggerity was removed while using targets\n" );
            return;
        }
    }
    ...
    if( i > 0 )
    {
        ...
        for( j = 0; j < i; j++ )
        {
            F32 d = Distance( player->r.currentOrigin, targets[ j ]->r.currentOrigin );
...
/*
================
idGameLocal::Touch_DoorTrigger
================
*/
void idGameLocal::Touch_DoorTrigger( gentity_t* ent, gentity_t* other, trace_t* trace )
{
...
/*
======================
idGameLocal::Think_SpawnNewDoorTrigger

All of the parts of a door have been spawned, so create
a trigger that encloses all of them
======================
*/
void idGameLocal::Think_SpawnNewDoorTrigger( gentity_t* ent )
{
    gentity_t* other;
    vec3_t    mins, maxs;
    S32       i, best;
...
void idGameLocal::Think_MatchTeam( gentity_t* ent )
{
    MatchTeam( ent, ent->moverState, level.time );
}
...
*/
void idGameLocal::SP_func_door( gentity_t* ent )
{
    vec3_t  abs_movedir;
    F32   distance;
    vec3_t  size;
    ...
    SpawnString( "sound2to1", "sound/movers/doors/dr1_strt.wav", &s );
    ent->sound2to1 = SoundIndex( s );
    SpawnString( "sound1to2", "sound/movers/doors/dr1_strt.wav", &s );
    ent->sound1to2 = SoundIndex( s );
    
    SpawnString( "soundPos2", "sound/movers/doors/dr1_end.wav", &s );
    ent->soundPos2 = SoundIndex( s );
    SpawnString( "soundPos1", "sound/movers/doors/dr1_end.wav", &s );
    ent->soundPos1 = SoundIndex( s );
...
void idGameLocal::SP_func_door_rotating( gentity_t* ent )
{
    UTF8*    s;
    ...
    if( !ent->rotatorAngle )
    {
        Printf( "%s at %s with no rotatorAngle set.\n", ent->classname, vtos( ent->s.origin ) );
...
"health"  if set, the door must be shot open
*/
void idGameLocal::SP_func_door_model( gentity_t* ent )
{
    UTF8*      s;
    F32     light;
    vec3_t    color;
    ...
    SpawnVector( "modelOrigin", "0 0 0", ent->s.origin );
    
    SpawnVector( "scale", "1 1 1", ent->s.origin2 );
    
    // if the "model2" key is set, use a seperate model
    // for drawing, but clip against the brushes
    if( !ent->model2 )
        Printf( S_COLOR_YELLOW "WARNING: func_door_model %d spawned with no model2 key\n", ent->s.number );
    else
        ent->s.modelindex = ModelIndex( ent->model2 );
    ...
    ent->s.misc = ( S32 )ent->animation[ 0 ];              //first frame
    ent->s.weapon = abs( ( S32 )ent->animation[ 1 ] );       //number of frames
...
/*
==============
idGameLocal::Touch_Plat

Don't allow decent if a living player is on it
===============
*/
void idGameLocal::Touch_Plat( gentity_t* ent, gentity_t* other, trace_t* trace )
{
    // DONT_WAIT
    if( ent->spawnflags & 1 )
        return;
...
/*
==============
idGameLocal::Touch_PlatCenterTrigger

If the plat is at the bottom position, start it going up
===============
*/
void idGameLocal::Touch_PlatCenterTrigger( gentity_t* ent, gentity_t* other, trace_t* trace )
{
    if( !other->client )
        return;
...
================
*/
void idGameLocal::SpawnPlatTrigger( gentity_t* ent )
{
    gentity_t* trigger;
    vec3_t tmin, tmax;
...
"light"   constantLight radius
*/
void idGameLocal::SP_func_plat( gentity_t* ent )
{
    F32 lip, height;
    UTF8*  s;
    
    SpawnString( "sound2to1", "sound/movers/plats/pt1_strt.wav", &s );
    ent->sound2to1 = SoundIndex( s );
    SpawnString( "sound1to2", "sound/movers/plats/pt1_strt.wav", &s );
    ent->sound1to2 = SoundIndex( s );
    
    SpawnString( "soundPos2", "sound/movers/plats/pt1_end.wav", &s );
    ent->soundPos2 = SoundIndex( s );
    SpawnString( "soundPos1", "sound/movers/plats/pt1_end.wav", &s );
    ent->soundPos1 = SoundIndex( s );
...
===============
*/
void idGameLocal::Touch_Button( gentity_t* ent, gentity_t* other, trace_t* trace )
{
    if( !other->client )
        return;
...
"light"   constantLight radius
*/
void idGameLocal::SP_func_button( gentity_t* ent )
{
    vec3_t  abs_movedir;
    F32   distance;
    vec3_t  size;
    ...
    SpawnString( "sound1to2", "sound/movers/switches/button1.wav", &s );
    ent->sound1to2 = SoundIndex( s );
...
/*
===============
idGameLocal::Think_BeginMoving

The wait time at a corner has completed, so start moving again
===============
*/
void idGameLocal::Think_BeginMoving( gentity_t* ent )
{
...
/*
===============
idGameLocal::Reached_Train
===============
*/
void idGameLocal::Reached_Train( gentity_t* ent )
{
    gentity_t* next;
    F32     speed;
    vec3_t    move;
...
/*
================
idGameLocal::Start_Train
================
*/
void idGameLocal::Start_Train( gentity_t* ent, gentity_t* other, gentity_t* activator )
{
    vec3_t  move;
...
/*
================
idGameLocal::Stop_Train
================
*/
void idGameLocal::Stop_Train( gentity_t* ent, gentity_t* other, gentity_t* activator )
{
    vec3_t  origin;
...
/*
================
idGameLocal::Use_Train
================
*/
void idGameLocal::Use_Train( gentity_t* ent, gentity_t* other, gentity_t* activator )
{
    if( ent->spawnflags & TRAIN_START_OFF )
    {
...
===============
*/
void idGameLocal::Think_SetupTrainTargets( gentity_t* ent )
{
    gentity_t* path, *next, *start;
    
    ent->nextTrain = Find( NULL, FOFS( targetname ), ent->target );
    
    if( !ent->nextTrain )
    {
        Printf( "func_train at %s with an unfound target\n", vtos( ent->r.absmin ) );
        return;
    }
    ...
    for( path = ent->nextTrain; path != start; path = next )
    {
        ...
        if( !path->target )
        {
            Printf( "Train corner at %s without a target\n", vtos( path->s.origin ) );
            return;
        }
        ...
        do
        {
            next = Find( next, FOFS( targetname ), path->target );
            
            if( !next )
            {
                Printf( "Train corner at %s without a target path_corner\n", vtos( path->s.origin ) );
                return;
            }
        }
...
*/
void idGameLocal::SP_path_corner( gentity_t* self )
{
    if( !self->targetname )
    {
        Printf( "path_corner with no targetname at %s\n", vtos( self->s.origin ) );
        FreeEntity( self );
        return;
    }
...
================
*/
void idGameLocal::Blocked_Train( gentity_t* self, gentity_t* other )
{
    ...
    else
    {
        if( !other->client )
        {
            ...
            //KILL!!1!!!
            Damage( other, self, self, NULL, NULL, 10000, 0, MOD_CRUSH );
            ...
            if( other->s.eType == ET_BUILDABLE && other->spawned )
            {
                ...
                if( other->buildableTeam == TEAM_ALIENS )
                {
                    ...
                    tent = TempEntity( other->s.origin, EV_ALIEN_BUILDABLE_EXPLOSION );
                    tent->s.eventParm = DirToByte( dir );
                }
                else if( other->buildableTeam == TEAM_HUMANS )
                {
                    VectorSet( dir, 0.0f, 0.0f, 1.0f );
                    tent = TempEntity( other->s.origin, EV_HUMAN_BUILDABLE_EXPLOSION );
                    tent->s.eventParm = DirToByte( dir );
                }
            }
...
"light"   constantLight radius
*/
void idGameLocal::SP_func_train( gentity_t* self )
{
    ...
    if( !self->target )
    {
        Printf( "func_train without a target at %s\n", vtos( self->r.absmin ) );
        FreeEntity( self );
        return;
    }
...
"light"   constantLight radius
*/
void idGameLocal::SP_func_static( gentity_t* ent )
{
    trap_SetBrushModel( ent, ent->model );
    InitMover( ent );
    VectorCopy( ent->s.origin, ent->s.pos.trBase );
...
"light"   constantLight radius
*/
void idGameLocal::SP_func_rotating( gentity_t* ent )
{
    if( !ent->speed )
        ent->speed = 100;
...
"light"   constantLight radius
*/
void idGameLocal::SP_func_bobbing( gentity_t* ent )
{
    F32 height, phase;
...
"light"   constantLight radius
*/
void idGameLocal::SP_func_pendulum( gentity_t* ent )
{
    F32 freq, length, phase, speed;
