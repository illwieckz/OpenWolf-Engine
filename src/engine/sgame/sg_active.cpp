...
===============
*/
void idGameLocal::DamageFeedback( gentity_t* player )
{
    gclient_t* client;
    F32 count;
    vec3_t angles;
    ...
    if( ( level.time > player->pain_debounce_time ) && !( player->flags & FL_GODMODE ) )
    {
        ...
        AddEvent( player, EV_PAIN, player->health > 255 ? 255 : player->health );
...
/*
=============
idGameLocal::WorldEffects

Check for lava / slime contents and drowning
=============
*/
void idGameLocal::WorldEffects( gentity_t* ent )
{
    S32 waterlevel;
    ...
    if( waterlevel == 3 )
    {
        ...
        if( ent->client->airOutTime < level.time )
        {
            ...
            if( ent->health > 0 )
            {
                ...
                {
                    Sound( ent, CHAN_VOICE, SoundIndex( "*drown.wav" ) );
                }
                else if( rand() & 1 )
                {
                    Sound( ent, CHAN_VOICE, SoundIndex( "sound/player/gurp1.wav" ) );
                }
                else
                {
                    Sound( ent, CHAN_VOICE, SoundIndex( "sound/player/gurp2.wav" ) );
                }
                
                // don't play a normal pain sound
                ent->pain_debounce_time = level.time + 200;
                
                Damage( ent, NULL, NULL, NULL, NULL, ent->damage, DAMAGE_NO_ARMOR, MOD_WATER );
            }
        }
    }
    else
    ...
    if( waterlevel && ( ent->watertype & ( CONTENTS_LAVA | CONTENTS_SLIME ) ) )
    {
        if( ent->health > 0 && ent->pain_debounce_time <= level.time )
        {
            ...
            {
                Damage( ent, NULL, NULL, NULL, NULL, 30 * waterlevel, 0, MOD_LAVA );
            }
            
            if( ent->watertype & CONTENTS_SLIME )
            {
                Damage( ent, NULL, NULL, NULL, NULL, 10 * waterlevel, 0, MOD_SLIME );
            }
        }
    }
}
...
===============
*/
void idGameLocal::SetClientSound( gentity_t* ent )
{
...
/*
==============
idGameLocal::ClientShove
==============
*/
S32 idGameLocal::GetClientMass( gentity_t* ent )
{
    S32 entMass = 100;
...
===============
*/
void idGameLocal::ClientShove( gentity_t* ent, gentity_t* victim )
{
...
/*
==============
idGameLocal::ClientImpacts
==============
*/
void idGameLocal::ClientImpacts( gentity_t* ent, pmove_t* pm )
{
    S32 i;
    trace_t trace;
    gentity_t* other;
...
============
*/
void idGameLocal::TouchTriggers( gentity_t* ent )
{
    S32 i, num, touch[MAX_GENTITIES];
    gentity_t* hit;
    trace_t trace;
    ...
    bggame->ClassBoundingBox( ( class_t )ent->client->ps.stats[ STAT_CLASS ], pmins, pmaxs, NULL, NULL, NULL );
...
/*
=================
idGameLocal::SpectatorThink
=================
*/
void idGameLocal::SpectatorThink( gentity_t* ent, usercmd_t* ucmd )
{
    pmove_t pm;
    gclient_t* client;
    S32 clientNum;
    ...
    {
        queued = SearchSpawnQueue( &level.alienSpawnQueue, ent - g_entities );
    }
    else if( client->pers.teamSelection == TEAM_HUMANS )
    {
        queued = SearchSpawnQueue( &level.humanSpawnQueue, ent - g_entities );
    }
    else
    {
        queued = false;
    ...
    if( attack1 && queued )
    {
        ...
        {
            RemoveFromSpawnQueue( &level.alienSpawnQueue, client->ps.clientNum );
        }
        else if( client->ps.stats[STAT_TEAM] == TEAM_HUMANS )
        {
            RemoveFromSpawnQueue( &level.humanSpawnQueue, client->ps.clientNum );
        }
    ...
    else if( attack1 )
    {
        ...
        {
            TriggerMenu( client->ps.clientNum, MN_TEAM );
        }
        else if( client->pers.teamSelection == TEAM_ALIENS )
        {
            TriggerMenu( client->ps.clientNum, MN_A_CLASS );
        }
        else if( client->pers.teamSelection == TEAM_HUMANS )
        {
            TriggerMenu( client->ps.clientNum, MN_H_SPAWN );
        }
    }
    ...
    if( !following )
    {
        ...
        if( client->ps.pm_flags & PMF_QUEUED )
        {
            if( client->ps.stats[ STAT_TEAM ] == TEAM_ALIENS )
            {
                client->ps.persistant[ PERS_QUEUEPOS ] = GetPosInSpawnQueue( &level.alienSpawnQueue, client->ps.clientNum );
                client->ps.persistant[ PERS_SPAWNS ] = level.numAlienSpawns;
            }
            else if( client->ps.stats[ STAT_TEAM ] == TEAM_HUMANS )
            {
                client->ps.persistant[ PERS_QUEUEPOS ] = GetPosInSpawnQueue( &level.humanSpawnQueue, client->ps.clientNum );
...
/*
=================
idGameLocal::ClientInactivityTimer

Returns false if the client is dropped
=================
*/
bool idGameLocal::ClientInactivityTimer( gclient_t* client )
{
...
/*
==================
idGameLocal::ClientTimerActions

Actions that happen once a second
==================
*/
void idGameLocal::ClientTimerActions( gentity_t* ent, S32 msec )
{
    gclient_t* client;
    usercmd_t* ucmd;
    S32 aForward, aRight, i;
    ...
    while( client->time100 >= 100 )
    {
        ...
        switch( weapon )
        {
            ...
            case WP_HBUILD:
                ...
                {
                    S32 dist = bggame->Class( ( class_t )ent->client->ps.stats[STAT_CLASS] )->buildDist;
                    vec3_t dummy;
                    
                    if( CanBuild( ent, ( buildable_t )( client->ps.stats[STAT_BUILDABLE] & ~SB_VALID_TOGGLEBIT ), dist, dummy ) == IBE_NONE )
                    {
        ...
        {
            S32 remainingStartupTime = MEDKIT_STARTUP_TIME - ( level.time - client->lastMedKitTime );
    ...
    while( client->time1000 >= 1000 )
    {
        ...
        if( client->ps.stats[ STAT_STATE ] & SS_POISONED )
        {
            ...
            Damage( ent, client->lastPoisonClient, client->lastPoisonClient, NULL, 0, damage, 0, MOD_POISON );
        }
        
        // turn off life support when a team admits defeat
        if( client->ps.stats[ STAT_TEAM ] == TEAM_ALIENS && level.surrenderTeam == TEAM_ALIENS )
        {
            Damage( ent, NULL, NULL, NULL, NULL, bggame->Class( ( class_t )client->ps.stats[ STAT_CLASS ] )->regenRate, DAMAGE_NO_ARMOR, MOD_SUICIDE );
        }
        else if( client->ps.stats[ STAT_TEAM ] == TEAM_HUMANS && level.surrenderTeam == TEAM_HUMANS )
        {
            Damage( ent, NULL, NULL, NULL, NULL, 5, DAMAGE_NO_ARMOR, MOD_SUICIDE );
        }
...
/*
====================
idGameLocal::ClientIntermissionThink
====================
*/
void idGameLocal::ClientIntermissionThink( gclient_t* client )
{
...
/*
================
idGameLocal::ClientEvents

Events will be passed on to the clients for presentation,
but any server game effects are handled here
================
*/
void idGameLocal::ClientEvents( gentity_t* ent, S32 oldEventSequence )
{
    S32 i, event, damage;
    gclient_t* client;
    ...
    for( i = oldEventSequence; i < client->ps.eventSequence; i++ )
    {
        ...
        switch( event )
        {
            ...
            case EV_FALL_FAR:
                ...
                fallDistance = ( ( F32 )client->ps.stats[ STAT_FALLDIST ] - MIN_FALL_DISTANCE ) / ( MAX_FALL_DISTANCE - MIN_FALL_DISTANCE );
                ...
                damage = ( S32 )( ( F32 )bggame->Class( _class )->health * bggame->Class( _class )->fallDamage * fallDistance );
                
                VectorSet( dir, 0, 0, 1 );
                bggame->ClassBoundingBox( _class, mins, NULL, NULL, NULL, NULL );
                mins[ 0 ] = mins[ 1 ] = 0.0f;
                VectorAdd( client->ps.origin, mins, point );
                
                ent->pain_debounce_time = level.time + 200; // no normal pain sound
                Damage( ent, NULL, NULL, dir, point, damage, DAMAGE_NO_LOCDAMAGE, MOD_FALLING );
                break;
...
/*
==============
idGameLocal::SendPendingPredictableEvents
==============
*/
void idGameLocal::SendPendingPredictableEvents( playerState_t* ps )
{
    gentity_t* t;
    S32 event, seq, extEvent, number;
...
This data is used by idGameLocal::UnlaggedCalc()
==============
*/
void idGameLocal::UnlaggedStore( void )
{
    S32 i = 0;
    gentity_t* ent;
    unlagged_t* save;
...
void idGameLocal::UnlaggedCalc( S32 time, gentity_t* rewindEnt )
{
    ...
    if( frameMsec > 0 )
    {
        lerp = ( F32 )( time - level.unlaggedTimes[ startIndex ] ) / ( F32 )frameMsec;
    }
...
/*
==============
idGameLocal::UnlaggedOff

Reverses the changes made to all active clients by idGameLocal::UnlaggedOn()
==============
*/
void idGameLocal::UnlaggedOff( void )
{
    S32 i = 0;
    gentity_t* ent;
...
/*
==============
idGameLocal::UnlaggedOn

Called after idGameLocal::UnlaggedCalc() to apply the calculated values to all active
clients.  Once finished tracing, idGameLocal::UnlaggedOff() must be called to restore
...
void idGameLocal::UnlaggedOn( gentity_t* attacker, vec3_t muzzle, F32 range )
{
    S32 i = 0;
    gentity_t* ent;
    unlagged_t* calc;
    ...
    for( i = 0; i < level.maxclients; i++ )
    {
        ...
        if( muzzle )
        {
            F32 r1 = Distance( calc->origin, calc->maxs );
            F32 r2 = Distance( calc->origin, calc->mins );
            F32 maxRadius = ( r1 > r2 ) ? r1 : r2;
...
/*
==============
idGameLocal::G_UnlaggedDetectCollisions
...
NOTE: this must be called after Pmove() and idGameLocal::UnlaggedCalc()
==============
*/
void idGameLocal::UnlaggedDetectCollisions( gentity_t* ent )
{
    unlagged_t* calc;
    trace_t tr;
    F32 r1, r2, range;
...
/*
==============
idGameLocal::CheckZap
==============
*/
void idGameLocal::CheckZap( gentity_t* ent )
{
    S32 i;
...
==============
*/
void idGameLocal::ClientThink_real( gentity_t* ent )
{
    gclient_t* client;
    pmove_t pm;
    S32 oldEventSequence, msec;
    usercmd_t* ucmd;
    ...
    {
        F32 regenRate = bggame->Class( ( class_t )ent->client->ps.stats[ STAT_CLASS ] )->regenRate;
        ...
        else
        {
            ...
            if( modifier == 1.0f && !FindCreep( ent ) )
            {
                client->ps.stats[ STAT_STATE ] &= ~SS_HEALING_ACTIVE;
    ...
    switch( client->ps.weapon )
    {
        ...
        case WP_ALEVEL3_UPG:
            ...
            else
            {
                ...
                AddEvent( ent, EV_FIRE_WEAPON2, 0 );
            }
            break;
    ...
    if( ( client->buttons & BUTTON_USE_EVOLVE ) && !( client->oldbuttons & BUTTON_USE_EVOLVE ) && client->ps.stats[ STAT_HEALTH ] > 0 )
    {
        ...
        if( client->ps.stats[ STAT_STATE ] & SS_HOVELING )
        {
            ...
            if( !AHovel_Blocked( hovel, ent, true ) )
            {
                //prevent lerping
                ...
                //hovel is empty
                SetBuildableAnim( hovel, BANIM_ATTACK2, false );
                hovel->active = false;
            }
            else
            {
                //exit is blocked
                TriggerMenu( ent->client->ps.clientNum, MN_A_HOVEL_BLOCKED );
            }
        }
        else
        {
#define USE_OBJECT_RANGE 64
            ...
            else
            {
                ...
                if( i == num && client->ps.stats[ STAT_TEAM ] == TEAM_ALIENS )
                {
                    if( bggame->AlienCanEvolve( ( class_t )client->ps.stats[ STAT_CLASS ], client->ps.persistant[ PERS_CREDIT ], g_alienStage.integer ) )
                    {
                        ...
                        TriggerMenu( ent->client->ps.clientNum, MN_A_INFEST );
                    }
                    else
                    {
                        //flash frags
                        AddEvent( ent, EV_ALIEN_EVOLVE_FAILED, 0 );
                    }
                }
            }
        }
    ...
    if( ent->client->lastKillTime + FREEKILL_PERIOD < level.time )
    {
        ...
        {
            AddCreditToClient( ent->client, FREEKILL_ALIEN, true );
        }
        else if( ent->client->ps.stats[STAT_TEAM] == TEAM_HUMANS )
        {
            AddCreditToClient( ent->client, FREEKILL_HUMAN, true );
        }
...
==================
*/
void idGameLocal::ClientThink( S32 clientNum )
{
    gentity_t* ent;
...
/*
==================
idGameLocal::RunClient
==================
*/
void idGameLocal::RunClient( gentity_t* ent )
{
    if( !( ent->r.svFlags & SVF_BOT ) && !g_synchronousClients.integer )
    {
        return;
...
/*
==================
idGameLocal::SpectatorClientEndFrame
==================
*/
void idGameLocal::SpectatorClientEndFrame( gentity_t* ent )
{
    gclient_t* cl;
    S32 clientNum, flags, score, ping;
...
while a slow client may have multiple idGameLocal::ClientEndFrame between idGameLocal::ClientThink.
==============
*/
void idGameLocal::ClientEndFrame( gentity_t* ent )
{
