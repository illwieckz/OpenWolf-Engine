...
static void Matrix34Multiply( F32* a, F32* b, F32* out )
{
...
static void Matrix34Multiply_OnlySetOrigin( F32* a, F32* b, F32* out )
{
    out[ 3] = a[0] * b[3] + a[1] * b[7] + a[ 2] * b[11] + a[ 3];
    out[ 7] = a[4] * b[3] + a[5] * b[7] + a[ 6] * b[11] + a[ 7];
    out[11] = a[8] * b[3] + a[9] * b[7] + a[10] * b[11] + a[11];
}
static void InterpolateMatrix( F32* a, F32* b, F32 lerp, F32* mat )
{
    F32 unLerp = 1.0f - lerp;
...
static void Matrix34Invert( F32* inMat, F32* outMat )
{
    vec3_t trans;
    F32 invSqrLen, *v;
...
bool R_LoadIQM( model_t* mod, void* buffer, S32 filesize, StringEntry mod_name )
{
    ...
    UTF8*			str;
    S32			i, j;
    F32			jointInvMats[IQM_MAX_JOINTS * 12] = {0.0f};
    F32*			mat, *matInv;
    U64			size, joint_names;
    iqmData_t*		iqmData;
    srfIQModel_t*		surface;
    UTF8			meshName[MAX_QPATH];
    U8			blendIndexesType, blendWeightsType;
    ...
    size += header->num_joints * 12 * sizeof( F32 ); // joint mats
    size += header->num_poses * header->num_frames * 12 * sizeof( F32 ); // pose mats
    if( header->ofs_bounds )
    ...
    size += header->num_vertexes * 4 * sizeof( U8 );	// blendIndexes
    size += header->num_vertexes * 4 * sizeof( U8 );	// colors
    size += header->num_joints * sizeof( S32 );		// parents
    ...
    iqmData->jointMats    = ( F32* )( iqmData->surfaces + iqmData->num_surfaces );
    ...
    iqmData->blendIndexes = ( U8* )( iqmData->tangents + 4 * header->num_vertexes );
    
    if( blendWeightsType == IQM_FLOAT )
    {
        iqmData->blendWeights.f = ( F32* )( iqmData->blendIndexes + 4 * header->num_vertexes );
        iqmData->colors		= ( U8* )( iqmData->blendWeights.f + 4 * header->num_vertexes );
    }
    else
    {
    ...
    iqmData->jointParents = ( S32* )( iqmData->colors + 4 * header->num_vertexes );
    iqmData->triangles    = iqmData->jointParents + header->num_joints;
    iqmData->names        = ( UTF8* )( iqmData->triangles + 3 * header->num_triangles );
    ...
    for( i = 0; i < header->num_vertexarrays; i++, vertexarray++ )
    {
        ...
        switch( vertexarray->type )
        {
            ...
            case IQM_BLENDINDEXES:
                ...
                {
                    S32* data = ( S32* )( ( U8* )header + vertexarray->offset );
                    for( j = 0; j < n; j++ )
                    {
                        iqmData->blendIndexes[j] = ( U8 )data[j];
                    }
                }
                else
                {
                             ...
                             F32 backlerp, F32* mat )
{
    ...
    if( data->num_poses == 0 )
    {
        for( i = 0; i < data->num_joints; i++, joint++ )
        {
            ...
            {
                ::memcpy( mat + 12 * i, identityMatrix, 12 * sizeof( F32 ) );
            }
        }
        return;
    }
    ...
    if( oldframe == frame )
    {
        ...
        for( i = 0; i < data->num_poses; i++, joint++ )
        {
            ...
            {
                ::memcpy( mat + 12 * i, mat1 + 12 * i, 12 * sizeof( F32 ) );
            }
        }
    }
    else
            ...
            i++, outXYZ++, outNormal += 4, outTexCoord++, outColor += 4 )
    {
        ...
        for( numWeights = 0; numWeights < 4; numWeights++ )
        {
            if( data->blendWeightsType == IQM_FLOAT )
                ...
                blendWeights[numWeights] = ( F32 )data->blendWeights.b[4 * vtx + numWeights] / 255.0f;
        ...
        if( data->num_poses == 0 || numWeights == 0 )
        {
            ...
            ::memcpy( vtxMat, identityMatrix, 12 * sizeof( F32 ) );
        }
        else
        {
            // compute the vertex matrix by blending the up to
            // four blend weights
            ::memset( vtxMat, 0, 12 * sizeof( F32 ) );
