...
static	void R_LoadVisibility( lump_t* l )
{
    ...
    {
        U8* dest = ( U8* )Hunk_Alloc( len - 8, h_low );
...
static void ParseFace( dsurface_t* ds, drawVert_t* verts, F32* hdrVertColors, msurface_t* surf, S32* indexes )
{
    ...
    cv->indexes = ( U32* )Hunk_Alloc( numIndexes * sizeof( cv->indexes[0] ), h_low );
...
void R_MovePatchSurfacesToHunk( void )
{
    ...
    for( i = 0; i < s_worldData.numsurfaces; i++ )
    {
        ...
        grid->widthLodError = ( F32* )Hunk_Alloc( grid->width * 4, h_low );
        ::memcpy( grid->widthLodError, copyFrom, grid->width * 4 );
        Z_Free( copyFrom );
        
        copyFrom = grid->heightLodError;
        grid->heightLodError = ( F32* )Hunk_Alloc( grid->height * 4, h_low );
        ::memcpy( grid->heightLodError, copyFrom, grid->height * 4 );
        Z_Free( copyFrom );
        
        copyFrom = grid->indexes;
        grid->indexes = ( U32* )Hunk_Alloc( grid->numIndexes * sizeof( U32 ), h_low );
        ::memcpy( grid->indexes, copyFrom, grid->numIndexes * sizeof( U32 ) );
        Z_Free( copyFrom );
        
        copyFrom = grid->verts;
        grid->verts = ( srfVert_t* )Hunk_Alloc( grid->numVerts * sizeof( srfVert_t ), h_low );
...
static	void R_LoadSurfaces( lump_t* surfs, lump_t* verts, lump_t* indexLump )
{
    ...
    s_worldData.surfacesViewCount = ( S32* )Hunk_Alloc( count * sizeof( *s_worldData.surfacesViewCount ), h_low );
    s_worldData.surfacesDlightBits = ( S32* )Hunk_Alloc( count * sizeof( *s_worldData.surfacesDlightBits ), h_low );
...
static	void R_LoadMarksurfaces( lump_t* l )
{
    ...
    if( l->filelen % sizeof( *in ) )
        Com_Error( ERR_DROP, "LoadMap: funny lump size in %s", s_worldData.name );
    count = l->filelen / sizeof( *in );
    out = ( S32* )Hunk_Alloc( count * sizeof( *out ), h_low );
...
void R_LoadLightGrid( lump_t* l )
{
    ...
    w->lightGridData = ( U8* )Hunk_Alloc( l->filelen, h_low );
    ...
    if( r_truehdr->integer )
    {
        ...
        if( hdrLightGrid )
        {
            ...
            w->lightGrid16 = ( U16* )Hunk_Alloc( sizeof( w->lightGrid16 ) * 6 * numGridPoints, h_low );
...
void R_LoadEntities( lump_t* l )
{
    ...
    w->entityString = ( UTF8* )Hunk_Alloc( l->filelen + 1, h_low );
...
void R_LoadEnvironmentJson( StringEntry baseName )
{
    UTF8 filename[MAX_QPATH];
    ...
    for( i = 0; i < tr.numCubemaps; i++ )
    {
        ...
        StringEntry cubemapJson, keyValueJson, indexes[3];
        S32 j;
...
void idRenderSystemLocal::LoadWorld( StringEntry name )
{
    ...
    startMarker = ( U8* )Hunk_Alloc( 0, h_low );
    c_gridVerts = 0;
