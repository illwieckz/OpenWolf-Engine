...
// movement parameters
F32 pm_stopspeed = 100.0f;
F32 pm_duckScale = 0.25f;
F32 pm_swimScale = 0.50f;
F32 pm_wadeScale = 0.70f;

F32 pm_accelerate = 10.0f;
F32 pm_airaccelerate = 1.0f;
F32 pm_wateraccelerate = 4.0f;
F32 pm_flyaccelerate = 4.0f;

F32 pm_friction = 6.0f;
F32 pm_waterfriction = 1.0f;
F32 pm_flightfriction = 6.0f;
F32 pm_spectatorfriction = 5.0f;
...
===============
*/
void idBothGamesLocal::AddEvent( S32 newEvent )
{
...
===============
*/
void idBothGamesLocal::AddTouchEnt( S32 entityNum )
{
    S32 i;
...
===================
*/
void idBothGamesLocal::StartTorsoAnim( S32 anim )
{
...
===================
*/
void idBothGamesLocal::StartLegsAnim( S32 anim )
{
    if( PM_Paralyzed( pm->ps->pm_type ) )
    {
        return;
...
===================
*/
void idBothGamesLocal::ContinueLegsAnim( S32 anim )
{
...
===================
*/
void idBothGamesLocal::ContinueTorsoAnim( S32 anim )
{
...
===================
*/
void idBothGamesLocal::ForceLegsAnim( S32 anim )
{
    //legsTimer is clamped too tightly for nonsegmented models
    if( !( pm->ps->persistant[PERS_STATE] & PS_NONSEGMODEL ) )
    {
...
==================
*/
void idBothGamesLocal::ClipVelocity( vec3_t in, vec3_t normal, vec3_t out, F32 overbounce )
{
    F32 backoff, change;
    S32 i;
...
void idBothGamesLocal::Friction( void )
{
    vec3_t vec;
    F32* vel;
    F32 speed, newspeed, control, drop;
    ...
    if( pm->waterlevel <= 1 )
    {
        if( ( pml.walking || pml.ladder ) && !( pml.groundTrace.surfaceFlags & SURF_SLICK ) )
        {
            ...
            {
                F32 stopSpeed = Class( ( class_t )pm->ps->stats[ STAT_CLASS ] )->stopSpeed;
                F32 friction = Class( ( class_t )pm->ps->stats[ STAT_CLASS ] )->friction;
...
==============
*/
void idBothGamesLocal::Accelerate( vec3_t wishdir, F32 wishspeed, F32 accel )
{
#if 1
    // q2 style
    S32 i;
...
============
*/
F32 idBothGamesLocal::CmdScale( usercmd_t* cmd )
{
    S32 max;
    F32 total, scale, modifier = 1.0f;
    ...
    if( pm->ps->stats[ STAT_TEAM ] == TEAM_HUMANS && pm->ps->pm_type == PM_NORMAL )
    {
        ...
        {
            modifier *= ( F32 )( pm->ps->stats[STAT_STAMINA] + 1000 ) / 500.0f;
        }
    ...
    scale = ( F32 )pm->ps->speed * max / ( 127.0 * total ) * modifier;
...
bool idBothGamesLocal::CheckWallJump( void )
{
    ...
    F32   normalFraction = 1.5f;
    F32   cmdFraction = 1.0f;
    F32   upFraction = 1.5f;
    trace_t trace;
...
{
    F32 speed, drop, friction, control, newspeed;
    S32 i;
    vec3_t  wishvel;
    F32 fmove, smove;
    vec3_t wishdir;
...
================
*/
S32 idBothGamesLocal::FootstepForSurface( void )
{
    if( pm->ps->stats[STAT_STATE] & SS_CREEPSLOWED )
    {
...
{
    F32 delta, dist, vel, acc, t, a, b, c, den;
...
=============
*/
S32 idBothGamesLocal::CorrectAllSolid( trace_t* trace )
{
    S32 i, j, k;
    vec3_t point;
...
void idBothGamesLocal::GroundClimbTrace( void )
{
    ...
    F32 traceDOTsurf, traceDOTref, surfDOTref, rTtDOTrTsTt, traceANGsurf, traceANGref, surfANGref;
...
void idBothGamesLocal::CheckDuck( void )
{
    ...
    ClassBoundingBox( ( class_t )pm->ps->stats[ STAT_CLASS ], PCmins, PCmaxs, PCcmaxs, NULL, NULL );
...
void idBothGamesLocal::Footsteps( void )
{
    ...
    old = pm->ps->bobCycle;
    pm->ps->bobCycle = ( S32 )( old + bobmove * pml.msec ) & 255;
...
===============
*/
void idBothGamesLocal::BeginWeaponChange( S32 weapon )
{
...
{
    S32 addTime = 200; //default addTime - should never be used
    S32 maxClips;
    ...
    else
    {
        if( pm->ps->weapon == WP_ALEVEL4 )
        {
            ...
            S32 num = abs( pm->ps->commandTime ) % 3;
    ...
    if( pm->ps->weapon == WP_CHAINGUN )
    {
        ...
        {
            pm->ps->delta_angles[ PITCH ] -= ANGLE2SHORT( ( ( random() * 0.5 ) - 0.125 ) * ( 30 / ( F32 )addTime ) );
            pm->ps->delta_angles[ YAW ] -= ANGLE2SHORT( ( ( random() * 0.5 ) - 0.25 ) * ( 30.0 / ( F32 )addTime ) );
        }
        else
        {
            pm->ps->delta_angles[ PITCH ] -= ANGLE2SHORT( ( ( random() * 8 ) - 2 ) * ( 30.0 / ( F32 )addTime ) );
            pm->ps->delta_angles[ YAW ] -= ANGLE2SHORT( ( ( random() * 8 ) - 4 ) * ( 30.0 / ( F32 )addTime ) );
        }
    }
...
void idBothGamesLocal::UpdateViewAngles( playerState_t* ps, const usercmd_t* cmd )
{
    ...
    if( ps->pm_type == PM_GRABBED && !InventoryContainsUpgrade( UP_BATTLESUIT, ps->stats ) )
    {
        ...
        for( i = 0; i < 3; i++ )
        {
            F32 diff = AngleSubtract( ps->viewangles[ i ], angles[ i ] );
