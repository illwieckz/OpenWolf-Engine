    ...
    while( 1 )
    {
        ...
        if( si != NULL )
        {
            ...
            if( shaderText[0] == '\0' || R_ForceGenericShader( si->shader ) )
            {
                ...
                {
                    sprintf( myShader, GenericPlayerShader, strippedName, strippedName, glowShaderAddition, strippedName );
                }
                else if( StringContainsWord( strippedName, "weapon" ) )
                {
                    sprintf( myShader, GenericWeaponShader, strippedName, strippedName, glowShaderAddition, strippedName );
                }
                else
                {
                    sprintf( myShader, GenericShader, strippedName, strippedName, glowShaderAddition, strippedName );
                }
                ...
                //if (verbose)
                Sys_Printf( "Created OWMap generic shader for %s.\n", si->shader );
            }
