...
=================
*/
void idCGameLocal::TestModel_f( void )
{
    vec3_t    angles;
    ...
    Q_strncpyz( cg.testModelName, Argv( 1 ), MAX_QPATH );
    cg.testModelEntity.hModel = trap_R_RegisterModel( cg.testModelName );
    
    Q_strncpyz( cg.testModelBarrelName, Argv( 1 ), MAX_QPATH );
    ...
    if( trap_Argc( ) == 3 )
    {
        cg.testModelEntity.backlerp = atof( Argv( 2 ) );
...
=================
*/
void idCGameLocal::TestGun_f( void )
{
    TestModel_f( );
...
void idCGameLocal::TestModelNextFrame_f( void )
{
    cg.testModelEntity.frame++;
    Printf( "frame %i\n", cg.testModelEntity.frame );
}

void idCGameLocal::TestModelPrevFrame_f( void )
{
    cg.testModelEntity.frame--;
...
void idCGameLocal::TestModelNextSkin_f( void )
{
    cg.testModelEntity.skinNum++;
    Printf( "skin %i\n", cg.testModelEntity.skinNum );
}

void idCGameLocal::TestModelPrevSkin_f( void )
{
    cg.testModelEntity.skinNum--;
...
void idCGameLocal::AddTestModel( void )
{
    S32 i;
    ...
    if( cg.testModelBarrelEntity.hModel )
    {
        PositionEntityOnTag( &cg.testModelBarrelEntity, &cg.testModelEntity, cg.testModelEntity.hModel, "tag_barrel" );
...
=================
*/
void idCGameLocal::CalcVrect( void )
{
    S32 size;
...
===============
*/
void idCGameLocal::OffsetThirdPersonView( void )
{
    S32 i;
    ...
    if( cg.demoPlayback || ( ( cg.snap->ps.pm_flags & PMF_FOLLOW ) && ( cg.predictedPlayerState.stats[ STAT_HEALTH ] > 0 ) ) )
    {
        ...
        if( !( cg.snap->ps.stats[ STAT_STATE ] & SS_WALLCLIMBING ) ||
                !bggame->RotateAxis( cg.snap->ps.grapplePoint, axis, rotaxis, false, cg.snap->ps.eFlags & EF_WALLCLIMBCEILING ) )
    ...
    if( !cg_cameraMode.integer )
    {
        ...
        Trace( &trace, cg.refdef.vieworg, mins, maxs, view, cg.predictedPlayerState.clientNum, MASK_SOLID );
...
===============
*/
void idCGameLocal::OffsetShoulderView( void )
{
    S32 i, cmdNum;
    usercmd_t cmd, oldCmd;
    vec3_t rotationAngles, axis[ 3 ], rotaxis[ 3 ];
    F32 deltaMousePitch;
    static F32 mousePitch;
    vec3_t forward, right, up;
    F32 classFwdOffset = 0, classUpOffset = 0, classRightOffset = 0;
...
void idCGameLocal::StepOffset( void )
{
    F32         steptime;
    S32           timeDelta;
    vec3_t        normal;
    ...
    {
        F32 stepChange = cg.stepChange * ( steptime - timeDelta ) / steptime;
...
/*
===============
idCGameLocal::OffsetFirstPersonView
===============
*/
void idCGameLocal::OffsetFirstPersonView( void )
{
    F32* origin;
    F32* angles;
    F32 bob, ratio, delta, speed, f;
...
#define LEVEL4_FEEDBACK  10.0f
    ...
    if( ps->weapon == WP_ALEVEL4 )
    {
        ...
        {
            F32 fraction = ( F32 )ps->stats[ STAT_MISC ] /
...
#define LEVEL3_FEEDBACK  20.0f
    ...
    {
        F32 fraction1, fraction2;
        vec3_t  forward;
        
        AngleVectors( angles, forward, NULL, NULL );
        VectorNormalize( forward );
        
        fraction1 = ( F32 )cg.predictedPlayerState.stats[ STAT_MISC ] / LEVEL3_POUNCE_TIME_UPG;
...
#define STRUGGLE_TIME 250
    ...
    if( cg.predictedPlayerState.pm_type == PM_GRABBED )
    {
        ...
        usercmd_t cmd;
        S32 cmdNum;
        F32 fFraction, rFraction, uFraction, fFraction2, rFraction2, uFraction2;
        ...
        fFraction = ( F32 )( cg.time - cg.forwardMoveTime ) / STRUGGLE_TIME;
        rFraction = ( F32 )( cg.time - cg.rightMoveTime ) / STRUGGLE_TIME;
        uFraction = ( F32 )( cg.time - cg.upMoveTime ) / STRUGGLE_TIME;
    ...
    {
        F32 scale, fraction, pitchFraction;
        
        scale = 1.0f - ( F32 )( cg.time - cg.poisonedTime ) / bggame->PlayerPoisonCloudTime( &cg.predictedPlayerState );
    ...
    if( cg.predictedPlayerState.stats[ STAT_TEAM ] == TEAM_HUMANS && ( cg.predictedPlayerState.pm_type == PM_NORMAL || cg.predictedPlayerState.pm_type == PM_JETPACK ) )
    {
        ...
        {
            F32 deltaBreath = ( cg.predictedPlayerState.stats[ STAT_STAMINA ] - STAMINA_BREATHING_LEVEL ) / -250.0;
            F32 deltaAngle = cos( ( F32 )cg.time / 150.0 ) * deltaBreath;
...
S32 idCGameLocal::CalcFov( void )
{
    F32 x, phase, v, fov_x, fov_y, zoomFov, f;
    S32 contents, cmdNum, inwater, attribFov;
    usercmd_t cmd, oldcmd;
    ...
    else
    {
        ...
        if( cg.spawnTime > ( cg.time - FOVWARPTIME ) &&
                bggame->ClassHasAbility( ( class_t )cg.predictedPlayerState.stats[ STAT_CLASS ], SCA_FOVWARPS ) )
        {
            F32 temp, temp2;
            
            temp = ( F32 )( cg.time - cg.spawnTime ) / FOVWARPTIME;
        ...
        if( bggame->Weapon( ( weapon_t )cg.predictedPlayerState.weapon )->canZoom )
        {
            if( cg.zoomed )
            {
                f = ( cg.time - cg.zoomTime ) / ( F32 )ZOOM_TIME;
    ...
    if( ( cg.predictedPlayerEntity.currentState.eFlags & EF_POISONCLOUDED ) &&
            ...
            !( cg.snap->ps.pm_flags & PMF_FOLLOW ) )
    {
        F32 scale = 1.0f - ( F32 )( cg.time - cg.poisonedTime ) / bggame->PlayerPoisonCloudTime( &cg.predictedPlayerState );
...
===============
*/
void idCGameLocal::DrawSurfNormal( void )
{
    trace_t     tr;
    vec3_t      end, temp;
    polyVert_t  normal[ 4 ];
    ...
    Trace( &tr, cg.refdef.vieworg, NULL, NULL, end, cg.predictedPlayerState.clientNum, MASK_SOLID );
...
===============
*/
void idCGameLocal::addSmoothOp( vec3_t rotAxis, F32 rotAngle, F32 timeMod )
{
    S32 i;
...
===============
*/
void idCGameLocal::smoothWWTransitions( playerState_t* ps, const vec3_t in, vec3_t out )
{
    ...
    {
        smoothTime = ( S32 )( cg_wwSmoothTime.integer * cg.sList[ i ].timeMod );
...
===============
*/
void idCGameLocal::smoothWJTransitions( playerState_t* ps, const vec3_t in, vec3_t out )
{
    S32 i;
    F32 stLocal, sFraction;
    bool  performed = false;
...
===============
*/
S32 idCGameLocal::CalcViewValues( void )
{
    playerState_t* ps;
    ...
    if( cg.snap->ps.stats[ STAT_HEALTH ] <= 0 )
    {
        if( IsParticleSystemValid( &cg.poisonCloudPS ) )
            DestroyParticleSystem( &cg.poisonCloudPS );
    }
...
=====================
*/
void idCGameLocal::AddBufferedSound( sfxHandle_t sfx )
{
    if( !sfx )
        return;
...
=====================
*/
void idCGameLocal::PlayBufferedSounds( void )
{
...
=================
*/
void idCGameLocal::DrawActiveFrame( S32 serverTime, stereoFrame_t stereoView, bool demoPlayback )
{
    S32   inwater;
    ...
    if( !cg.hyperspace )
    {
        AddPacketEntities( );     // after calcViewValues, so predicted player state is correct
        AddMarks( );
    }
    ...
    if( cg_timescale.value != cg_timescaleFadeEnd.value )
    {
        if( cg_timescale.value < cg_timescaleFadeEnd.value )
        {
            cg_timescale.value += cg_timescaleFadeSpeed.value * ( ( F32 )cg.frametime ) / 1000;
            if( cg_timescale.value > cg_timescaleFadeEnd.value )
