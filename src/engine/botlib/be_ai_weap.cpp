...
#define WEAPON_OFS(x) (S32)&(((weaponinfo_t *)0)->x)
#define PROJECTILE_OFS(x) (S32)&(((projectileinfo_t *)0)->x)
...
typedef struct bot_weaponstate_s
{
    ...
    S32* weaponweightindex;							//weapon weight index
} bot_weaponstate_t;
...
// Changes Globals:		-
//========================================================================
S32 BotValidWeaponNumber( S32 weaponnum )
{
...
// Changes Globals:		-
//========================================================================
bot_weaponstate_t* BotWeaponStateFromHandle( S32 handle )
{
...
void DumpWeaponConfig( weaponconfig_t* wc )
{
    ...
    {
        WriteStructure( fp, &projectileinfo_struct, ( UTF8* ) &wc->projectileinfo[i] );
        Log_Flush();
    } //end for
    for( i = 0; i < wc->numweapons; i++ )
    {
        WriteStructure( fp, &weaponinfo_struct, ( UTF8* ) &wc->weaponinfo[i] );
        Log_Flush();
    } //end for
...
// Changes Globals:		-
//===========================================================================
weaponconfig_t* LoadWeaponConfig( UTF8* filename )
{
    S32 max_weaponinfo, max_projectileinfo;
    token_t token;
    UTF8 path[MAX_PATH];
    S32 i, j;
    source_t* source;
    ...
    max_weaponinfo = ( S32 ) LibVarValue( "max_weaponinfo", "32" );
    if( max_weaponinfo < 0 )
    {
    ...
    max_projectileinfo = ( S32 ) LibVarValue( "max_projectileinfo", "32" );
    ...
    wc->weaponinfo = ( weaponinfo_t* )( ( UTF8* ) wc + sizeof( weaponconfig_t ) );
    wc->projectileinfo = ( projectileinfo_t* )( ( UTF8* ) wc->weaponinfo +
                         max_weaponinfo * sizeof( weaponinfo_t ) );
    ...
    while( PC_ReadToken( source, &token ) )
    {
        if( !strcmp( token.string, "weaponinfo" ) )
        {
            ...
            if( !ReadStructure( source, &weaponinfo_struct, ( UTF8* ) &weaponinfo ) )
            {
                FreeMemory( wc );
                FreeSource( source );
                return NULL;
        ...
        else if( !strcmp( token.string, "projectileinfo" ) )
        {
            ...
            if( !ReadStructure( source, &projectileinfo_struct, ( UTF8* ) &wc->projectileinfo[wc->numprojectiles] ) )
            {
                FreeMemory( wc );
                FreeSource( source );
                return NULL;
...
// Changes Globals:		-
//===========================================================================
S32* WeaponWeightIndex( weightconfig_t* wwc, weaponconfig_t* wc )
{
    S32* index, i;
    
    //initialize item weight index
    index = ( S32* ) GetClearedMemory( sizeof( S32 ) * wc->numweapons );
...
// Changes Globals:		-
//===========================================================================
void BotFreeWeaponWeights( S32 weaponstate )
{
...
{
    S32 i, index, bestweapon;
    F32 weight, bestweight;
    weaponconfig_t* wc;
    bot_weaponstate_t* ws;
...
// Changes Globals:		-
//========================================================================
void BotFreeWeaponState( S32 handle )
{
    if( handle <= 0 || handle > MAX_CLIENTS )
    {
        botimport.Print( PRT_FATAL, "move state handle %d out of range\n", handle );
