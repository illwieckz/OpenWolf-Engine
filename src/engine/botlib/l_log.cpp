...
{
    UTF8 filename[MAX_LOGFILENAMESIZE];
    FILE* fp;
    S32 numwrites;
} logfile_t;
...
void Log_WriteTimeStamped( UTF8* fmt, ... )
{
    ...
    fprintf( logfile.fp, "%d   %02d:%02d:%02d:%02d   ",
             logfile.numwrites,
             ( S32 )( botlibglobals.time / 60 / 60 ),
             ( S32 )( botlibglobals.time / 60 ),
             ( S32 )( botlibglobals.time ),
             ( S32 )( ( S32 )( botlibglobals.time * 100 ) ) -
             ( ( S32 ) botlibglobals.time ) * 100 );
    va_start( ap, fmt );
    vfprintf( logfile.fp, fmt, ap );
    va_end( ap );
    fprintf( logfile.fp, "\r\n" );
