...
S32 s_rawend[MAX_STREAMING_SOUNDS];
S32 s_rawpainted[MAX_STREAMING_SOUNDS];
...
//		::memset(snd.sfxHash, 0, sizeof(sfx_t *)*LOOP_HASH);
...
==================
*/
sfxHandle_t idSoundSystemLocal::RegisterSound( StringEntry name, bool compressed )
{
    sfx_t*   sfx;
    
    //compressed = false; // Arnout: memory corruption with compressed sounds?
...
void S_SpatializeOrigin( vec3_t origin, S32 master_vol, S32* left_vol, S32* right_vol, F32 range, S32 noAttenuation )
{
//	vec_t		dot;
    vec_t dist;
...
====================
*/
void S_ThreadStartSoundEx( vec3_t origin, S32 entityNum, S32 entchannel, sfxHandle_t sfxHandle, S32 flags, S32 volume );

void idSoundSystemLocal::StartSoundEx( vec3_t origin, S32 entityNum, S32 entchannel, sfxHandle_t sfxHandle, S32 flags, S32 volume )
{
...
==============
*/
void idSoundSystemLocal::StartSound( vec3_t origin, S32 entityNum, S32 entchannel, sfxHandle_t sfxHandle, S32 volume )
{
    StartSoundEx( origin, entityNum, entchannel, sfxHandle, 0, volume );
}
...
==================
*/
void idSoundSystemLocal::StartLocalSound( sfxHandle_t sfxHandle, S32 channelNum, S32 volume )
{
    if( !snd.s_soundStarted || snd.s_soundMute )
    {
        return;
    ...
    StartSound( NULL, listener_number, channelNum, sfxHandle, volume );
}
...
==================
*/
void idSoundSystemLocal::ClearSoundBuffer( bool killStreaming )
{
    if( !snd.s_soundStarted )
    {
        return;
...
//DAJ BUGFIX	for(i=0;i<numStreamingSounds;i++) {
    ...
    soundSystemLocal.ClearSoundBuffer( true );
...
void idSoundSystemLocal::AddLoopingSound( const vec3_t origin, const vec3_t velocity, const S32 range, sfxHandle_t sfxHandle, S32 volume, S32 soundTime )
{
    sfx_t* sfx;
    ...
    snd.loopSounds[ snd.numLoopSounds ].startSample = sfx->soundLength
            ?  s_paintedtime - ( ( S32 )( dma.speed * ( cl.serverTime - soundTime ) / 1000.0f ) )
            : 0;
    ...
    snd.loopSounds[snd.numLoopSounds].active = true;
    snd.loopSounds[snd.numLoopSounds].doppler = false;
    ...
    snd.loopSounds[snd.numLoopSounds].vol = ( S32 )( ( F32 )volume * snd.volCurrent ); //----(SA)	modified
    ...
    if( s_doppler->integer && VectorLengthSquared( velocity ) > 0.0 )
    {
        ...
        snd.loopSounds[snd.numLoopSounds].doppler = true;
        ...
        {
            snd.loopSounds[snd.numLoopSounds].doppler = false;         // don't bother doing the math
        }
    }
...
==================
*/
void idSoundSystemLocal::AddRealLoopingSound( const vec3_t origin, const vec3_t velocity, const S32 range, sfxHandle_t sfxHandle, S32 volume, S32 soundTime )
{
    sfx_t* sfx;
...
//		Com_DPrintf ("not started or muted\n");
    ...
    for( i = 0 ; i < snd.numLoopSounds ; i++ )
    {
        ...
        if( loop->kill )
        {
            S_SpatializeOrigin( loop->origin, 127, &left_total, &right_total, loop->range, false );         // 3d
        }
        else
        {
            S_SpatializeOrigin( loop->origin, 90,  &left_total, &right_total, loop->range, false );         // sphere
        }
        
        // adjust according to volume
        left_total = ( S32 )( ( F32 )loop->vol * ( F32 )left_total / 256.0 );
        right_total = ( S32 )( ( F32 )loop->vol * ( F32 )right_total / 256.0 );
        ...
        for( j = ( i + 1 ); j < MAX_GENTITIES ; j++ )
        {
            ...
            if( loop2->kill )
            {
                S_SpatializeOrigin( loop2->origin, 127, &left, &right, loop2->range, false );               // 3d
            }
            else
            {
                S_SpatializeOrigin( loop2->origin, 90,  &left, &right, loop2->range, false );               // sphere
            }
            
            // adjust according to volume
            left = ( S32 )( ( F32 )loop2->vol * ( F32 )left / 256.0 );
            right = ( S32 )( ( F32 )loop2->vol * ( F32 )right / 256.0 );
...
=================
*/
//DAJ void S_ByteSwapRawSamples( S32 samples, S32 width, S32 s_channels, const U8 *data ) {
void S_ByteSwapRawSamples( S32 samples, S32 width, S32 s_channels, S16* data )
{
    S32 i;
    ...
    do
    {
        *data = LittleShort( *data );
        data++;
//DAJ		((S16 *)data)[i] = LittleShort( ((S16 *)data)[i] );
    }
    while( --i );
}
...
============
*/
void idSoundSystemLocal::RawSamples( S32 samples, S32 rate, S32 width, S32 s_channels, const U8* data, F32 lvol, F32 rvol, S32 streamingIndex )
{
    S32 i;
    S32 src, dst;
    F32 scale;
    S32 intVolumeL, intVolumeR;
    ...
    if( s_channels == 2 && width == 2 )
    {
        if( scale == 1.0 )    // optimized case
        {
            for( i = 0; i < samples; i++ )
            {
                ...
                s_rawsamples[streamingIndex][dst].left = ( ( S16* )data )[i * 2] * intVolumeL;
                s_rawsamples[streamingIndex][dst].right = ( ( S16* )data )[i * 2 + 1] * intVolumeR;
            }
        }
        else
        {
            for( i = 0; ; i++ )
            {
                ...
                dst = s_rawend[streamingIndex] & ( MAX_RAW_SAMPLES - 1 );
                s_rawend[streamingIndex]++;
                s_rawsamples[streamingIndex][dst].left = ( ( S16* )data )[src * 2] * intVolumeL;
                s_rawsamples[streamingIndex][dst].right = ( ( S16* )data )[src * 2 + 1] * intVolumeR;
            }
        }
    }
    else if( s_channels == 1 && width == 2 )
    {
        for( i = 0; ; i++ )
        {
            ...
            dst = s_rawend[streamingIndex] & ( MAX_RAW_SAMPLES - 1 );
            s_rawend[streamingIndex]++;
            s_rawsamples[streamingIndex][dst].left = ( ( S16* )data )[src] * intVolumeL;
            s_rawsamples[streamingIndex][dst].right = ( ( S16* )data )[src] * intVolumeR;
        }
    }
    else if( s_channels == 2 && width == 1 )
    {
        ...
        for( i = 0 ; ; i++ )
        {
            ...
            dst = s_rawend[streamingIndex] & ( MAX_RAW_SAMPLES - 1 );
            s_rawend[streamingIndex]++;
            s_rawsamples[streamingIndex][dst].left = ( ( UTF8* )data )[src * 2] * intVolumeL;
            s_rawsamples[streamingIndex][dst].right = ( ( UTF8* )data )[src * 2 + 1] * intVolumeR;
        }
    }
    else if( s_channels == 1 && width == 1 )
    {
        ...
        for( i = 0; ; i++ )
        {
            ...
            dst = s_rawend[streamingIndex] & ( MAX_RAW_SAMPLES - 1 );
            s_rawend[streamingIndex]++;
            s_rawsamples[streamingIndex][dst].left = ( ( ( U8* )data )[src] - 128 ) * intVolumeL;
            s_rawsamples[streamingIndex][dst].right = ( ( ( U8* )data )[src] - 128 ) * intVolumeR;
        }
    }
    
    if( s_rawend[streamingIndex] > ( s_soundtime + MAX_RAW_SAMPLES ) )
    {
        Com_DPrintf( "idSoundSystemLocal::RawSamples: overflowed %i > %i (%i)\n",  s_rawend[streamingIndex], s_soundtime + MAX_RAW_SAMPLES, s_rawend[streamingIndex] - ( s_soundtime + MAX_RAW_SAMPLES ) );
    }
}
...
============
*/
void idSoundSystemLocal::Respatialize( S32 entityNum, const vec3_t head, vec3_t axis[3], S32 inwater )
{
...
==============
*/
S32 S_CheckForQueuedMusic( void )
{
    streamingSound_t* ss;
    UTF8* nextMusicVA;
...
void idSoundSystemLocal::ClearSounds( bool clearStreaming, bool clearMusic )
{
    ...
    // stop looping sounds
    ::memset( snd.loopSounds, 0, MAX_GENTITIES * sizeof( loopSound_t ) );
...
void S_Update_Mix( void )
{
    ...
    */
    ...
    if( s_soundtime < snd.volTime2 )       // still has fading to do
    {
        ...
        {
            snd.volFadeFrac = ( ( F32 )( s_soundtime - snd.volTime1 ) / ( F32 )( snd.volTime2 - snd.volTime1 ) );
...
//DAJ			Com_DPrintf( "master vol: %f\n", snd.volCurrent );
    ...
    else
    {
        ...
        if( snd.stopSounds )
        {
            soundSystem->StopAllSounds();  // faded out, stop playing
            snd.stopSounds = false;
        }
    }
...
void S_Play_f( void )
{
    ...
    while( i < Cmd_Argc() )
    {
        ...
        h = soundSystemLocal.RegisterSound( name, false );
        if( h )
        {
            soundSystemLocal.StartLocalSound( h, CHAN_LOCAL_SOUND, 127 );
        }
        i++;
    }
}
...
void S_QueueMusic_f( void )
{
    S32 type = -2;  // default to setting this as the next continual loop
    S32 c;
...
S32 FGetLittleLong( const fileHandle_t f )
{
    S32 v;
...
S32 FGetLittleShort( const fileHandle_t f )
{
    S16 v;
...
S32 S_FindWavChunk( const fileHandle_t f, StringEntry chunk )
{
    UTF8 name[5];
    S32 len;
    S32 r;
...
======================
*/
void idSoundSystemLocal::StartBackgroundTrack( StringEntry intro, StringEntry loop, S32 fadeupTime )
{
    S32 len;
    UTF8 dump[16];
//	UTF8	name[MAX_QPATH];
    UTF8 loopMusic[MAX_QPATH];
    streamingSound_t* ss;
    fileHandle_t fh;
...
//----(SA)	end
    ...
    if( fadeupTime )
    {
        ...
        ss->fadeEnd         = s_soundtime + ( ( ( F32 )( ss->info.rate ) / 1000.0f ) * fadeupTime );
//		ss->fadeStart		= s_paintedtime;
//		ss->fadeEnd			= s_paintedtime + (((F32)(ss->info.rate)/1000.0f ) * fadeupTime);
...
==============
*/
void idSoundSystemLocal::FadeAllSounds( F32 targetVol, S32 time, bool stopsounds )
{
    // TAT 11/15/2002
    ...
    snd.volTime2 = s_soundtime + ( ( ( F32 )( dma.speed ) / 1000.0f ) * time );
...
==============
*/
void idSoundSystemLocal::FadeStreamingSound( F32 targetVol, S32 time, S32 ssNum )
{
    streamingSound_t* ss;
...
//		if(ss->fadeEnd <= s_paintedtime)
        ...
        {
            ss->fadeStartVol = ( ( F32 )( s_soundtime - ss->fadeStart ) / ( F32 )( ss->fadeEnd - ss->fadeStart ) );
        }
//			ss->fadeStartVol = ( (F32)(s_paintedtime - ss->fadeStart)/(F32)(ss->fadeEnd - ss->fadeStart) );
    }
    
    ss->fadeStart       = s_soundtime;
    ss->fadeEnd         = s_soundtime + ( ( ( F32 )( ss->info.rate ) / 1000.0f ) * time );
//	ss->fadeStart		= s_paintedtime;
//	ss->fadeEnd			= s_paintedtime + (((F32)(ss->info.rate)/1000.0f ) * time);
...
==============
*/
F32 S_GetStreamingFade( streamingSound_t* ss )
{
    F32 oldfrac, newfrac;
...
//	if(ss->fadeEnd <= s_paintedtime) {	// it's hit it's target
    ...
    newfrac = ( F32 )( s_soundtime - ss->fadeStart ) / ( F32 )( ss->fadeEnd - ss->fadeStart );
//	newfrac = (F32)(s_paintedtime - ss->fadeStart)/(F32)(ss->fadeEnd - ss->fadeStart);
...
======================
*/
F32 idSoundSystemLocal::StartStreamingSound( StringEntry intro, StringEntry loop, S32 entnum, S32 channel, S32 attenuation )
{
    S32 len;
    UTF8 dump[16];
//	UTF8	name[MAX_QPATH];
    ...
    Com_DPrintf( "idSoundSystemLocal::StartStridSoundSystemLocal::StartStreamingSoundeamingSound( %s, %s, %i, %i, %i )\n", intro, loop, entnum, channel, attenuation );
    ...
    if( !ss )
    {
        ...
        {
            Com_Printf( "idSoundSystemLocal::StartStreamingSound: No free streaming tracks\n" );
        }
        return 0;
    }
    ...
    return ( ss->samples / ( F32 )ss->info.rate ) * 1000.f;
}
...
======================
*/
void idSoundSystemLocal::StopStreamingSound( S32 index )
{
...
==============
*/
void idSoundSystemLocal::StopEntStreamingSound( S32 entNum )
{
    S32 i;
...
//	bool looped;
    F32 lvol, rvol;
    S32 soundMixAheadTime;
    F32 streamingVol = 1.0f;
...
//	}
    ...
    soundMixAheadTime = s_soundtime; // + (S32)(0.35 * dma.speed);	// allow for talking animations
...
//		looped = false;
        ...
        while( *re < soundMixAheadTime + MAX_RAW_SAMPLES )
        {
            ...
            // U8 swap if needed
            S_ByteSwapRawSamples( fileSamples, ss->info.width, ss->info.channels, ( S16* )raw );
            ...
            else            // attenuate if required
            {
                ...
                {
                    S32 r, l;
                    S_SpatializeOrigin( snd.entityPositions[ ss->entnum ], s_volume->value * 255.0f, &l, &r, SOUND_RANGE_DEFAULT, false );
                    if( ( lvol = ( ( F32 )l / 255.0 ) ) > 1.0 )
                    {
                        lvol = 1.0;
                    }
                    if( ( rvol = ( ( F32 )r / 255.0 ) ) > 1.0 )
                    {
                        rvol = 1.0;
                    }
                    lvol *= streamingVol;
...
void S_FreeOldestSound( void )
{
    ...
    Com_DPrintf( "idSoundSystemLocal::FreeOldestSound: freeing sound %s\n", sfx->soundName );
...
S32 idSoundSystemLocal::GetSoundLength( sfxHandle_t sfxHandle )
{
    if( sfxHandle < 0 || sfxHandle >= snd.s_numSfx )
    {
        Com_DPrintf( S_COLOR_YELLOW "idSoundSystemLocal::StartSound: handle %i out of range\n", sfxHandle );
        return -1;
    }
...
S32 idSoundSystemLocal::GetCurrentSoundTime( void )
{
...
{
    snd.loopSounds[entityNum].active = false;
    snd.loopSounds[entityNum].sfx = 0;
    snd.loopSounds[entityNum].kill = false;
}
...
{
    if( handle < 0 || handle >= snd.s_numSfx )
    {
