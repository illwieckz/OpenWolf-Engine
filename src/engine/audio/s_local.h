...
typedef struct
{
    ...
    void ( *RawSamples )( S32 stream, S32 samples, S32 rate, S32 width, S32 channels, const U8* data, F32 volume );
    ...
    void ( *SoundInfo )( void );
    void ( *SoundList )( void );
    void ( *SetReverb )( S32 reverb, vec3_t pos, F32 minDist, F32 maxDist );
} soundInterface_t;
...
public:
    ...
    virtual void RawSamples( S32 stream, S32 samples, S32 rate, S32 width, S32 channels, const U8* data, F32 volume );
    ...
    virtual sfxHandle_t RegisterSound( StringEntry sample );
